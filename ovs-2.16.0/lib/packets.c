...
VLOG_DEFINE_THIS_MODULE(packet);
#define LABEL_RANGE 0xffffc000
int count_1 = 0, count_2 = 0, rcv_color = 0, T_count = 0, sndflag = -1, rcvflag = -1, pkt_count = 0, sec = 0;
int write_flag, write1_flag;
bool first_pkt = true, timer = false;
...
void signalHandler(int signo)
{
    ...
    case SIGALRM:
        if (sec % 10 == 0)
        { //进入发送周期
            // VLOG_INFO("Caught the SIGALRM signal!period_count=%d,flag=%d\n", T_count, sndflag);
            if (sndflag == 1)
            {
                write_flag = 1;
                VLOG_INFO("in pactke.c,write_flag=1,sec=%d", sec);
                sndflag = 2;
            }
            else if (sndflag == 2)
            {
        ...
        { //进入接收周期
            ...
            { //第一次进入，什么都不做
                rcvflag = 1;
                VLOG_INFO("rcv flag=-1");
            }
            else if (rcvflag == 1)
            { //第二次进入，统计第一个10秒的丢包率，重置颜色1的数量，并改变标志位
                write_flag = 3;
                VLOG_INFO("in pactke.c,write_flag=3,sec=%d", sec);
                rcvflag = 2;
            }
            else if (rcvflag == 2)
            { //第二次进入，统计第二个10秒的丢包率，重置颜色2的数量，并改变标志位
                write_flag = 4;
                VLOG_INFO("in pactke.c,write_flag=4,sec=%d", sec);
                rcvflag = 1;
            }
            else
            {
...
{
    size_t len = strlen(s);
    *dpidp = ((len == 16 && is_all_hex(s)) || (len <= 18 && s[0] == '0' && (s[1] == 'x' || s[1] == 'X') && is_all_hex(s + 2))
                  ? strtoull(s, NULL, 16)
                  : 0);
    return *dpidp != 0;
}
...
{
    return (((uint64_t)ntohs(ea.be16[0]) << 32) | ((uint64_t)ntohs(ea.be16[1]) << 16) | ntohs(ea.be16[2]));
}
...
bool eth_addr_is_reserved(const struct eth_addr ea)
{
...
{
    int n = 0;
    if (ovs_scan(s, ETH_ADDR_SCAN_FMT "%n", ETH_ADDR_SCAN_ARGS(*ea), &n) && !s[n])
    {
        return true;
    }
    else
 ...
 * desired. */
void compose_rarp(struct dp_packet *b, const struct eth_addr eth_src)
{
    struct eth_header *eth;
    struct arp_eth_header *arp;

    dp_packet_clear(b);
    dp_packet_prealloc_tailroom(b, 2 + ETH_HEADER_LEN + VLAN_HEADER_LEN + ARP_ETH_HEADER_LEN);
...
void eth_pop_vlan(struct dp_packet *packet)
{
    ...
    if (veh && dp_packet_size(packet) >= sizeof *veh && eth_type_vlan(veh->veth_type))
    {
...
void set_mpls_lse(struct dp_packet *packet, ovs_be32 mpls_lse)
{
...
void push_mpls(struct dp_packet *packet, ovs_be16 ethtype, ovs_be32 lse)
{
    ...
    // new addition
    long long int origin_time = time_wall_usec();
    long long int curr_time = origin_time;
    // label20的前18位存放着时间的二进制值
    unsigned int label20 = (curr_time & 0xffffc0) >> 4;
    unsigned int label20color = 0;
    // VLOG_INFO("origin_time=%llx,current time_send=%x\n", curr_time,label20<<6);
    //这里进行染色，18位之后的两位存放颜色
    ...
    if (sndflag == -1)
    {
        ...
        if (timer = false)
        {
            ...
            int res = setitimer(ITIMER_REAL, &new_value, &old_value);
            if (res)
                VLOG_INFO("timer start failed!\n");
            else
                VLOG_INFO("timer start");
        }
        label20color = (label20 | 0x1);
    }
    if (sndflag == 1)
    {
        label20color = (label20 | 0x1);
    ...
    //打印时间和颜色，右移12位变到最低位
    VLOG_INFO("origin time=%llx,curr_time send=%x,send time and color=%x,color=%d,send_cnt=%d\n", origin_time, label20 << 4, label20color, label20color & 0x3, pkt_count++);
    // VLOG_INFO("lse=%u",lse);
    VLOG_INFO("old lse=%x", lse);
    // lse = label20 | (lse&0xfff00000);
    unsigned int label16 = label20color & 0xffff;
    unsigned int label4 = (label20color & 0xf0000) << 4;
    lse = lse | label16 | label4;
    VLOG_INFO("lse=%x,label16=%x,label4=%x", lse, label16, label4);
 ...
 * Ethertype). */
void pop_mpls(struct dp_packet *packet, ovs_be16 ethtype)
{
    if (is_mpls(packet))
    {
        struct mpls_hdr *mh = dp_packet_l2_5(packet);
        ...
        unsigned long int rcv_label;
        unsigned long int rcv_time;
        unsigned long int curr_time;
        rcv_label = ((mh->mpls_lse.lo << 16 | mh->mpls_lse.hi) & 0x00ffffff);
        unsigned int rcv_label4 = (rcv_label & 0xf00000) >> 4;
        unsigned int rcv_label16 = rcv_label & 0xffff;
        rcv_label = rcv_label4 | rcv_label16;
        rcv_time = (rcv_label & 0xffffc) << 4;
        rcv_color = rcv_label & 0x3;
        // VLOG_INFO("RECV LABEL=%x,RCV TIME=%lx,rcv color=%d", rcv_label, rcv_time, rcv_color);
        unsigned long long int origin_time = time_wall_usec();
        curr_time = origin_time & 0xffffc0;
        if (curr_time < rcv_time)
        {
            // VLOG_INFO("overflow result=%lx", curr_time - rcv_time);
            curr_time = origin_time & 0x1ffffff;
            if (curr_time < rcv_time)
            {
                curr_time = origin_time & 0x3ffffff;
            }
        }

        diff_time = curr_time - rcv_time;
        if (curr_time < rcv_time)
        {
            /*
            diff_time=-1;
            ...
            last_time=0;
            */
            diff_time = rcv_time - curr_time;
        }
        jitter = (int)diff_time - (int)last_time;
        last_time = diff_time;
        // VLOG_INFO("origin_time=%llx,RECV LABEL=%lx,RCV color=%d,RCV TIME=%lx,curr_time=%lx,diff=%lx,jitter=%d,pkt_count=%d",
        // origin_time, rcv_label, rcv_color, rcv_time, curr_time, diff_time, jitter, ++pkt_count);
        if (first_pkt == true)
        {
            //开启定时器
            first_pkt = false;
            if (timer == false)
            {
                ...
                if (res)
                    ...
                    VLOG_INFO("timer start,7s trigle the first\n");
            }
        }
        rcv_color == 2 ? count_2++ : count_1++;
...
void eth_format_masked(const struct eth_addr eth,
...
void in6_addr_solicited_node(struct in6_addr *addr, const struct in6_addr *ip6)
{
...
void ipv6_multicast_to_ethernet(struct eth_addr *eth, const struct in6_addr *ip6)
{
    eth->ea[0] = 0x33;
    eth->ea[1] = 0x33;
...
void ip_format_masked(ovs_be32 ip, ovs_be32 mask, struct ds *s)
{
...
{
    int n = 0;
    if (ovs_scan(s, IP_PORT_SCAN_FMT "%n", IP_PORT_SCAN_ARGS(ip, port), &n) && !s[n])
    {
        return NULL;
    }
                      ...
                      struct in6_addr *mask)
{
    ...
    if (ovs_scan_len(s, n, " " IPV6_SCAN_FMT, ipv6_s) && ipv6_parse(ipv6_s, ip))
    {
 ...
 * bytes long. */
void ipv6_format_addr(const struct in6_addr *addr, struct ds *s)
{
    char *dst;
 ...
 * brackets. */
void ipv6_format_addr_bracket(const struct in6_addr *addr, struct ds *s,
                              bool bracket)
{
    if (bracket)
    {
...
void ipv6_format_mapped(const struct in6_addr *addr, struct ds *s)
{
...
void ipv6_format_masked(const struct in6_addr *addr, const struct in6_addr *mask,
                        struct ds *s)
{
    ipv6_format_addr(addr, s);
...
void packet_set_ipv4_addr(struct dp_packet *packet,
...
packet_rh_present(struct dp_packet *packet, uint8_t *nexthdr)
{
    ...
    *nexthdr = nh->ip6_nxt;
    ...
    while (1)
    {
        if ((*nexthdr != IPPROTO_HOPOPTS) && (*nexthdr != IPPROTO_ROUTING) && (*nexthdr != IPPROTO_DSTOPTS) && (*nexthdr != IPPROTO_AH) && (*nexthdr != IPPROTO_FRAGMENT))
        {
        ...
        {
            const struct ovs_16aligned_ip6_frag *frag_hdr = ALIGNED_CAST(struct ovs_16aligned_ip6_frag *, data);
 ...
 * markers. */
void packet_set_ipv4(struct dp_packet *packet, ovs_be32 src, ovs_be32 dst,
...
void packet_set_ipv6(struct dp_packet *packet, const struct in6_addr *src,
...
void packet_set_tcp_port(struct dp_packet *packet, ovs_be16 src, ovs_be16 dst)
{
    struct tcp_header *th = dp_packet_l4(packet);
 ...
 * with its l4 offset properly populated. */
void packet_set_udp_port(struct dp_packet *packet, ovs_be16 src, ovs_be16 dst)
{
    struct udp_header *uh = dp_packet_l4(packet);
 ...
 * with its l4 offset properly populated. */
void packet_set_sctp_port(struct dp_packet *packet, ovs_be16 src, ovs_be16 dst)
{
 ...
 * populated. */
void packet_set_icmp(struct dp_packet *packet, uint8_t type, uint8_t code)
{
 ...
 */
void packet_set_igmp3_query(struct dp_packet *packet, uint8_t max_resp,
...
void packet_set_nd_ext(struct dp_packet *packet, const ovs_16aligned_be32 rso_flags,
...
void packet_set_nd(struct dp_packet *packet, const struct in6_addr *target,
...
void packet_format_tcp_flags(struct ds *s, uint16_t tcp_flags)
{
    if (!tcp_flags)
    {
        ds_put_cstr(s, "none");
...
void compose_arp(struct dp_packet *b, uint16_t arp_op,
 ...
 * header. */
void compose_arp__(struct dp_packet *b)
{
    dp_packet_clear(b);
...
void compose_nd_ns(struct dp_packet *b, const struct eth_addr eth_src,
...
void compose_nd_na(struct dp_packet *b,
...
void compose_nd_ra(struct dp_packet *b,
                   const struct eth_addr eth_src, const struct eth_addr eth_dst,
                   const struct in6_addr *ipv6_src, const struct in6_addr *ipv6_dst,
                   ...
                   ovs_be32 retrans_timer, uint32_t mtu)
{
    ...
    if (with_mtu)
    {
        ...
        struct ovs_nd_mtu_opt *mtu_opt = (struct ovs_nd_mtu_opt *)(lla_opt + 1);
...
void packet_put_ra_prefix_opt(struct dp_packet *b,
