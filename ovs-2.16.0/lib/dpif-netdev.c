...
#include "tuple.h"
#include <rte_ip.h>
//#include "../include/sparse/rte_ip.h"
#include  <rte_ether.h>
#include <rte_tcp.h>
#include <rte_udp.h>
...
int arridx = 0, rcvidx1 = 0, rcvidx2 = 0;
static bool mlc1 = false;
tuple_t *flowt;
tuple_t *rflow1;
tuple_t *rflow2;
int snd_period_cnt = 0, rcv_period_cnt = 0;
//end

VLOG_DEFINE_THIS_MODULE(dpif_netdev);

#define SIGCOMM_MEASUREMENT 1
                        ...
                        unsigned core_id, int numa_id)
{
    ...
    VLOG_INFO("SIGCOMM: %s initialized.", name);//YES
#endif
    // Sigcomm: end
}
               ...
               bool md_is_valid, odp_port_t port_no)
{
    ...
    if(likely(mbuf != NULL)){
        ...
        // if(eth_hdr->ether_type == 0x4788){//判断是否是MPLS
        //     struct rte_ipv4_hdr *ip_hdr;
        //     ip_hdr = rte_pktmbuf_mtod_offset(mbuf, struct rte_ipv4_hdr *, 18);
                
        //     if(ip_hdr->next_proto_id == 0x11){//判断udp
        //         tuple_t tuple;
        ...
        //         udp_hdr = rte_pktmbuf_mtod_offset(mbuf, struct udp_hdr *, 18 + 20);
        //         tuple.key.src_port = udp_hdr->src_port;
        //         tuple.key.dst_port = udp_hdr->dst_port;

        //         // 统计时延
        //         tuple.diff_time = diff_time;
        //         // VLOG_INFO("in dpif, diff_time=%lx", tuple.diff_time);
        //         while(write_ringbuffer(pmd->rb, (void*)&tuple, sizeof(tuple_t)) != 0);
        //     }
        // }
            //flowt用于存放发送流的信息，rflow1和2用于存放接收流的信息
            
            if (mlc1 == false)
            {
                flowt = (tuple_t *)malloc(50000 * sizeof(tuple_t));
                rflow1 = (tuple_t *)malloc(50000 * sizeof(tuple_t));
                rflow2 = (tuple_t *)malloc(50000 * sizeof(tuple_t));
                mlc1 = true;
            }

            struct rte_ether_hdr *eth_hdr;
            eth_hdr = rte_pktmbuf_mtod(mbuf, struct ether_hdr *);
            if (eth_hdr->ether_type == 0x4788)
            {
                if (write_flag == 3)
                {
                    rcv_period_cnt++;
                    int idx1 = 0;
                    for (idx1 = 0; idx1 < rcvidx1; idx1++)
                    {
                        (rflow1 + idx1)->period_cnt = rcv_period_cnt;
                        VLOG_INFO("WRITE TO RB,ridx1=%d,pkt_count=%d,rcv_period_cnt=%d,color=%d", idx1, (rflow1 + idx1)->pkt_count, rcv_period_cnt, (rflow1 + idx1)->color);
                        while (write_ringbuffer(pmd->rb, (void *)(rflow1 + idx1), sizeof(tuple_t)) != 0)
                            ;
                    }
                    // while (write_ringbuffer(pmd->rb, (void *)&tuple, sizeof(tuple_t)) != 0);
                    write_flag = 0;
                ...
                else if (write_flag == 4)
                {
                    rcv_period_cnt++;
                    int idx1 = 0;
                    for (idx1 = 0; idx1 < rcvidx2; idx1++)
                    {
                        (rflow2 + idx1)->period_cnt = rcv_period_cnt;
                        //VLOG_INFO("WRITE TO RB,ridx1=%d,pkt_count=%d,rcv_period_cnt=%d,color=%d", idx1, (rflow1 + idx1)->pkt_count, rcv_period_cnt, (rflow1 + idx1)->color);
                        while (write_ringbuffer(pmd->rb, (void *)(rflow2 + idx1), sizeof(tuple_t)) != 0)
                            ;
                    }
                    // while (write_ringbuffer(pmd->rb, (void *)&tuple, sizeof(tuple_t)) != 0);
                    write_flag = 0;
                ...
                //VLOG_INFO("IT IS A MPLS PKT");
                // VLOG_INFO("in dpif, diff_time=%lx,loss=%f", tuple.diff_time,tuple.loss);
                struct rte_ipv4_hdr *ip_hdr;
                ip_hdr = rte_pktmbuf_mtod_offset(mbuf, struct rte_ipv4_hdr *, 14 + 4);
                struct mpls_hdr *mh;
                mh = rte_pktmbuf_mtod_offset(mbuf, struct mpls_hdr *, 14);
                int rcv_color;
                rcv_color = ((mh->mpls_lse.lo << 16 | mh->mpls_lse.hi) & 0x00ffffff) & 0x3;
                //VLOG_INFO("THIS PACKET'S COLOR IS %d", rcv_color);
                tuple_t tuple;
                tuple.key.src_ip = ip_hdr->src_addr;
                tuple.key.dst_ip = ip_hdr->dst_addr;
                tuple.key.proto = ip_hdr->next_proto_id;
                tuple.size = ip_hdr->total_length;
                tuple.color = rcv_color;
                tuple.diff_time = diff_time;
                if (ip_hdr->next_proto_id == 0x11)
                {
                    struct rte_udp_hdr *udp_hdr;
                    udp_hdr = rte_pktmbuf_mtod_offset(mbuf, struct rte_udp_hdr *, 14 + 20 + 4);
                    tuple.key.src_port = udp_hdr->src_port;
                    tuple.key.dst_port = udp_hdr->dst_port;
                }
                else
                {
                    ...
                    tcp_hdr = rte_pktmbuf_mtod_offset(mbuf, struct rte_tcp_hdr *, 14 + 20 + 4);
                    tuple.key.src_port = tcp_hdr->src_port;
                    tuple.key.dst_port = tcp_hdr->dst_port;
                }
                ...
                if (rcv_color == 1)
                {
                    if (rcvidx1 == 0)
                    {
                        *(rflow1 + rcvidx1) = tuple;
                        (rflow1 + rcvidx1)->pkt_count++;
                        rcvidx1++;
                        //VLOG_INFO("FIRST ADD TO ARRAY");
                    }
                    else
                    {
                        bool exist = false;
                        int idx2 = 0;
                        for (idx2 = 0; idx2 < rcvidx1; idx2++)
                        {
                            if ((tuple.key.src_ip == (rflow1 + idx2)->key.src_ip) &&
                                (tuple.key.dst_ip == (rflow1 + idx2)->key.dst_ip) &&
                                (tuple.key.src_port == (rflow1 + idx2)->key.src_port) &&
                                (tuple.key.dst_port == (rflow1 + idx2)->key.dst_port) &&
                                tuple.key.proto == (rflow1 + idx2)->key.proto &&
                                tuple.color == (rflow1 + idx2)->color)
                            {
                                (rflow1 + idx2)->pkt_count++;
                                exist = true;
                                //VLOG_INFO("FLOW HAS EXIST IN ARRAY");
                                break;
                            }
                        }
                        if (exist == false)
                ...
                else if (rcv_color == 2)
                {
                    if (rcvidx2 == 0)
                    {
                        *(rflow2 + rcvidx2) = tuple;
                        (rflow2 + rcvidx2)->pkt_count++;
                        rcvidx2++;
                        //VLOG_INFO("FIRST ADD TO ARRAY");
                    }
                    else
                    {
                        bool exist = false;
                        int idx2 = 0;
                        for (idx2 = 0; idx2 < rcvidx2; idx2++)
                        {
                            if ((tuple.key.src_ip == (rflow2 + idx2)->key.src_ip) &&
                                (tuple.key.dst_ip == (rflow2 + idx2)->key.dst_ip) &&
                                (tuple.key.src_port == (rflow2 + idx2)->key.src_port) &&
                                (tuple.key.dst_port == (rflow2 + idx2)->key.dst_port) &&
                                tuple.key.proto == (rflow2 + idx2)->key.proto &&
                                tuple.color == (rflow2 + idx2)->color)
                            {
                                (rflow2 + idx2)->pkt_count++;
                                exist = true;
                                //VLOG_INFO("FLOW HAS EXIST IN ARRAY");
                                break;
                            }
            ...
            {
                //VLOG_INFO("WRITE_FLAG=%d", write_flag);
                if (write_flag == 1)
                {
                    snd_period_cnt++;
                    int idx1 = 0;
                    for (idx1 = 0; idx1 < arridx; idx1++)
                    {
                        (flowt + idx1)->period_cnt = snd_period_cnt;
                        //VLOG_INFO("WRITE TO RB,arridx=%d,pkt_count=%d,snd_period_cnt=%d", idx1, (flowt + idx1)->pkt_count, snd_period_cnt);
                        while (write_ringbuffer(pmd->rb, (void *)(flowt + idx1), sizeof(tuple_t)) != 0)
                            ;
                    }
                    // while (write_ringbuffer(pmd->rb, (void *)&tuple, sizeof(tuple_t)) != 0);
                    write_flag = 0;
                ...
                VLOG_INFO("IT IS A ETH PKT");
                // VLOG_INFO("in dpif, diff_time=%lx,loss=%f", tuple.diff_time,tuple.loss);
                struct rte_ipv4_hdr *ip_hdr;
                ip_hdr = rte_pktmbuf_mtod_offset(mbuf, struct rte_ipv4_hdr *, 14);
                ...
                if (ip_hdr->next_proto_id == 0x11)
                {
                    // VLOG_INFO("IT IS A UDP PKT");
                    struct rte_udp_hdr *udp_hdr;
                    udp_hdr = rte_pktmbuf_mtod_offset(mbuf, struct rte_udp_hdr *, 14 + 20);
                    tuple.key.src_port = udp_hdr->src_port;
                    tuple.key.dst_port = udp_hdr->dst_port;
                }
                else
                {
                    struct rte_tcp_hdr *tcp_hdr;
                    tcp_hdr = rte_pktmbuf_mtod_offset(mbuf, struct rte_tcp_hdr *, 14 + 20);
                    tuple.key.src_port = tcp_hdr->src_port;
                    tuple.key.dst_port = tcp_hdr->dst_port;
                }
                if (arridx == 0)
                ...
                else
                {
                    bool exist = false;
                    int idx2 = 0;
                    for (idx2 = 0; idx2 < arridx; idx2++)
                    {
                        if ((tuple.key.src_ip == (flowt + idx2)->key.src_ip) &&
                            (tuple.key.dst_ip == (flowt + idx2)->key.dst_ip) &&
                            (tuple.key.src_port == (flowt + idx2)->key.src_port) &&
                            (tuple.key.dst_port == (flowt + idx2)->key.dst_port) &&
                            tuple.key.proto == (flowt + idx2)->key.proto)
                        {
                            (flowt + idx2)->pkt_count++;
                            exist = true;
                            VLOG_INFO("FLOW HAS EXIST IN ARRAY");
                            break;
                        }
