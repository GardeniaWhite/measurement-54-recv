...
#define PKT_NUM_PER_10SEC 100.0
int count_1 = 0, count_2 = 0, rcv_color = 0, dp_conut_pkt = 0, my_count = 0, myflag = -1, test_count = 0;
int loss_num=0;
int write_flag;
...
void signalHandler(int signo)
{
    ...
    case SIGALRM:
        ...
        { //第一次进入，什么都不做
            myflag = 1;
            VLOG_INFO("I am into timer");
        }
        else if (myflag == 1)
        { //第二次进入，统计第一个10秒的丢包率，重置颜色1的数量，并改变标志位
            loss = 1 - count_1 / PKT_NUM_PER_10SEC;
            write_flag=1;
	    VLOG_INFO("color 1 num=%d,loss= %f\n", count_1, loss);
            count_1 = 0;
            myflag = 2;
        }
        else if (myflag == 2)
        { //第二次进入，统计第二个10秒的丢包率，重置颜色2的数量，并改变标志位
            loss = 1 - count_2 / PKT_NUM_PER_10SEC;
            write_flag=1;
	    VLOG_INFO("color 2 num=%d,loss=%f\n", count_2, loss);
            count_2 = 0;
            myflag = 1;
        }
        else
	...
	if(write_flag==1){
	    loss_sum=(loss_num*loss_sum+loss)/(loss_num+1);
	    loss_num++;
            VLOG_INFO("loss_sum=%f\n",loss_sum);
	}
        break;
    }
}
...
void push_mpls(struct dp_packet *packet, ovs_be16 ethtype, ovs_be32 lse)
{
    ...
    // new addition
    long long int curr_time = time_wall_usec();
    // label20的前18位存放着时间的二进制值
    // unsigned int label20 = (curr_time & LABEL_RANGE);
    unsigned int label20 = curr_time & 0x3ffff;
    ...
    else if ((dp_conut_pkt / 100) % 2 == 0)
    {
        label20 = (label20 | 0x40000);
        dp_conut_pkt++;
    }
    else if ((dp_conut_pkt / 100) % 2 == 1)
    {
        label20 = (label20 | 0x80000);
        dp_conut_pkt++;
    }
    else
    {
    ...
    //打印时间和颜色，右移12位变到最低位
    VLOG_INFO("current time=%x,color=%d,send_cnt=%d\n", label20, (label20 & 0xc0000) >> 18, test_count++);
    // VLOG_INFO("lse=%u",lse);
    VLOG_INFO("old lse=%x", lse);
    // lse = label20 | (lse&0xfff00000);
    // lse=lse|0x00800000;
    unsigned int label16 = label20 & 0xffff;
    unsigned int label4 = (label20 & 0xf0000) << 4;
...
void pop_mpls(struct dp_packet *packet, ovs_be16 ethtype)
{
    if (is_mpls(packet))
    {
        ...
        // VLOG_INFO("htol=%u\n",mh->mpls_lse.hi << 16 | mh->mpls_lse.lo);
        // VLOG_INFO("ltoh=%x\n", mh->mpls_lse.lo << 16 | mh->mpls_lse.hi);
        ...
        // VLOG_INFO("RECV LABEL=%u",rcv_label);
        unsigned int rcv_label4 = (rcv_label & 0xf00000) >> 4;
        unsigned int rcv_label16 = rcv_label & 0xffff;
        rcv_label = rcv_label4 | rcv_label16;
        rcv_time = rcv_label & 0x3ffff;
        rcv_color = (rcv_label & 0xc0000) >> 18;
        VLOG_INFO("RECV LABEL=%x,RCV TIME=%lx,rcv color=%d", rcv_label, rcv_time, rcv_color);
        // rcv_color = (rcv_label & 0x3000)>>12;
        curr_time = time_wall_usec();
        long long int origin_time = curr_time;
        // curr_time = curr_time & 0xffffc000;
        curr_time = curr_time & 0x3ffff;
        if(curr_time<rcv_time){
	    VLOG_INFO("overflow result=%llx",curr_time-rcv_time);
            curr_time=origin_time&0x7ffff;
            if(curr_time<rcv_time){
                curr_time=origin_time&0xfffff;
            }
        }
	...
	last_time=diff_time;
        VLOG_INFO("origin_time=%llx,curr_time_=%llx,diff=%llx,jitter=%x", origin_time, curr_time, curr_time - rcv_time,jitter);
        if (rcv_color == 0)
        {
            //开启定时器
