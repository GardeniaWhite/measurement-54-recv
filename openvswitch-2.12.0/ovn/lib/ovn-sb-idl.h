...
struct sbrec_chassis {
	...
	/* transport_zones column. */
	char **transport_zones;
	size_t n_transport_zones;
...
#define sbrec_chassis_col_transport_zones (sbrec_chassis_columns[SBREC_CHASSIS_COL_TRANSPORT_ZONES])
...
void sbrec_chassis_verify_transport_zones(const struct sbrec_chassis *);
...
const struct ovsdb_datum *sbrec_chassis_get_transport_zones(const struct sbrec_chassis *, enum ovsdb_atomic_type key_type);
...
void sbrec_chassis_set_transport_zones(const struct sbrec_chassis *, const char **transport_zones, size_t n_transport_zones);
...
void sbrec_chassis_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void sbrec_chassis_update_transport_zones_addvalue(const struct sbrec_chassis *,  const char *);
void sbrec_chassis_update_transport_zones_delvalue(const struct sbrec_chassis *,  const char *);
void sbrec_chassis_add_clause_transport_zones(struct ovsdb_idl_condition *, enum ovsdb_function function, const char **transport_zones, size_t n_transport_zones);
void sbrec_chassis_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
...
int64_t nb_cfg);
void sbrec_chassis_index_set_transport_zones(const struct sbrec_chassis *,
const char **transport_zones, size_t n_transport_zones);
...
struct sbrec_controller_event {
	...
	/* event_info column. */
	struct smap event_info;

	/* event_type column. */
	char *event_type;	/* Always nonnull. */

	/* seq_num column. */
	int64_t seq_num;
};

enum sbrec_controller_event_column_id {
    ...
    SBREC_CONTROLLER_EVENT_N_COLUMNS
};

#define sbrec_controller_event_col_chassis (sbrec_controller_event_columns[SBREC_CONTROLLER_EVENT_COL_CHASSIS])
#define sbrec_controller_event_col_event_info (sbrec_controller_event_columns[SBREC_CONTROLLER_EVENT_COL_EVENT_INFO])
#define sbrec_controller_event_col_event_type (sbrec_controller_event_columns[SBREC_CONTROLLER_EVENT_COL_EVENT_TYPE])
#define sbrec_controller_event_col_seq_num (sbrec_controller_event_columns[SBREC_CONTROLLER_EVENT_COL_SEQ_NUM])

extern struct ovsdb_idl_column sbrec_controller_event_columns[SBREC_CONTROLLER_EVENT_N_COLUMNS];

const struct sbrec_controller_event_table *sbrec_controller_event_table_get(const struct ovsdb_idl *);
const struct sbrec_controller_event *sbrec_controller_event_table_first(const struct sbrec_controller_event_table *);

#define SBREC_CONTROLLER_EVENT_TABLE_FOR_EACH(ROW, TABLE) \
        for ((ROW) = sbrec_controller_event_table_first(TABLE); \
             (ROW); \
             (ROW) = sbrec_controller_event_next(ROW))
#define SBREC_CONTROLLER_EVENT_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = sbrec_controller_event_table_first(TABLE); \
             (ROW) ? ((NEXT) = sbrec_controller_event_next(ROW), 1) : 0; \
             (ROW) = (NEXT))

const struct sbrec_controller_event *sbrec_controller_event_get_for_uuid(const struct ovsdb_idl *, const struct uuid *);
const struct sbrec_controller_event *sbrec_controller_event_table_get_for_uuid(const struct sbrec_controller_event_table *, const struct uuid *);
const struct sbrec_controller_event *sbrec_controller_event_first(const struct ovsdb_idl *);
const struct sbrec_controller_event *sbrec_controller_event_next(const struct sbrec_controller_event *);
#define SBREC_CONTROLLER_EVENT_FOR_EACH(ROW, IDL) \
        for ((ROW) = sbrec_controller_event_first(IDL); \
             (ROW); \
             (ROW) = sbrec_controller_event_next(ROW))
#define SBREC_CONTROLLER_EVENT_FOR_EACH_SAFE(ROW, NEXT, IDL) \
        for ((ROW) = sbrec_controller_event_first(IDL); \
             (ROW) ? ((NEXT) = sbrec_controller_event_next(ROW), 1) : 0; \
             (ROW) = (NEXT))

unsigned int sbrec_controller_event_get_seqno(const struct ovsdb_idl *);
unsigned int sbrec_controller_event_row_get_seqno(const struct sbrec_controller_event *row, enum ovsdb_idl_change change);
const struct sbrec_controller_event *sbrec_controller_event_track_get_first(const struct ovsdb_idl *);
const struct sbrec_controller_event *sbrec_controller_event_track_get_next(const struct sbrec_controller_event *);
#define SBREC_CONTROLLER_EVENT_FOR_EACH_TRACKED(ROW, IDL) \
        for ((ROW) = sbrec_controller_event_track_get_first(IDL); \
             (ROW); \
             (ROW) = sbrec_controller_event_track_get_next(ROW))

const struct sbrec_controller_event *sbrec_controller_event_table_track_get_first(const struct sbrec_controller_event_table *);
#define SBREC_CONTROLLER_EVENT_TABLE_FOR_EACH_TRACKED(ROW, TABLE) \
        for ((ROW) = sbrec_controller_event_table_track_get_first(TABLE); \
             (ROW); \
             (ROW) = sbrec_controller_event_track_get_next(ROW))


/* Returns true if 'row' was inserted since the last change tracking reset. */
static inline bool sbrec_controller_event_is_new(const struct sbrec_controller_event *row)
{
    return sbrec_controller_event_row_get_seqno(row, OVSDB_IDL_CHANGE_MODIFY) == 0;
}

/* Returns true if 'row' was deleted since the last change tracking reset. */
static inline bool sbrec_controller_event_is_deleted(const struct sbrec_controller_event *row)
{
    return sbrec_controller_event_row_get_seqno(row, OVSDB_IDL_CHANGE_DELETE) > 0;
}

void sbrec_controller_event_index_destroy_row(const struct sbrec_controller_event *);

struct sbrec_controller_event *sbrec_controller_event_index_find(struct ovsdb_idl_index *, const struct sbrec_controller_event *);

int sbrec_controller_event_index_compare(
    struct ovsdb_idl_index *, 
    const struct sbrec_controller_event *, 
    const struct sbrec_controller_event *);
struct ovsdb_idl_cursor sbrec_controller_event_cursor_first(struct ovsdb_idl_index *);
struct ovsdb_idl_cursor sbrec_controller_event_cursor_first_eq(
    struct ovsdb_idl_index *, const struct sbrec_controller_event *);
struct ovsdb_idl_cursor sbrec_controller_event_cursor_first_ge(
    struct ovsdb_idl_index *, const struct sbrec_controller_event *);

struct sbrec_controller_event *sbrec_controller_event_cursor_data(struct ovsdb_idl_cursor *);

#define SBREC_CONTROLLER_EVENT_FOR_EACH_RANGE(ROW, FROM, TO, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = sbrec_controller_event_cursor_first_ge(INDEX, FROM); \
             (cursor__.position \
              && ((ROW) = sbrec_controller_event_cursor_data(&cursor__), \
                  !(TO) || sbrec_controller_event_index_compare(INDEX, ROW, TO) <= 0)); \
             ovsdb_idl_cursor_next(&cursor__))
#define SBREC_CONTROLLER_EVENT_FOR_EACH_EQUAL(ROW, KEY, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = sbrec_controller_event_cursor_first_eq(INDEX, KEY); \
             (cursor__.position \
              ? ((ROW) = sbrec_controller_event_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next_eq(&cursor__), \
                 true) \
...
#define SBREC_CONTROLLER_EVENT_FOR_EACH_BYINDEX(ROW, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = sbrec_controller_event_cursor_first(INDEX); \
             (cursor__.position \
              ? ((ROW) = sbrec_controller_event_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next(&cursor__), \
                 true) \
...
void sbrec_controller_event_init(struct sbrec_controller_event *);
void sbrec_controller_event_delete(const struct sbrec_controller_event *);
struct sbrec_controller_event *sbrec_controller_event_insert(struct ovsdb_idl_txn *);
bool sbrec_controller_event_is_updated(const struct sbrec_controller_event *, enum sbrec_controller_event_column_id);

void sbrec_controller_event_verify_chassis(const struct sbrec_controller_event *);
void sbrec_controller_event_verify_event_info(const struct sbrec_controller_event *);
void sbrec_controller_event_verify_event_type(const struct sbrec_controller_event *);
void sbrec_controller_event_verify_seq_num(const struct sbrec_controller_event *);

const struct ovsdb_datum *sbrec_controller_event_get_chassis(const struct sbrec_controller_event *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *sbrec_controller_event_get_event_info(const struct sbrec_controller_event *, enum ovsdb_atomic_type key_type, enum ovsdb_atomic_type value_type);
const struct ovsdb_datum *sbrec_controller_event_get_event_type(const struct sbrec_controller_event *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *sbrec_controller_event_get_seq_num(const struct sbrec_controller_event *, enum ovsdb_atomic_type key_type);

void sbrec_controller_event_set_chassis(const struct sbrec_controller_event *, const struct sbrec_chassis *chassis);
void sbrec_controller_event_set_event_info(const struct sbrec_controller_event *, const struct smap *);
void sbrec_controller_event_set_event_type(const struct sbrec_controller_event *, const char *event_type);
void sbrec_controller_event_set_seq_num(const struct sbrec_controller_event *, int64_t seq_num);

void sbrec_controller_event_update_chassis_addvalue(const struct sbrec_controller_event *,  const struct sbrec_chassis *);
void sbrec_controller_event_update_chassis_delvalue(const struct sbrec_controller_event *,  const struct sbrec_chassis *);
void sbrec_controller_event_add_clause_chassis(struct ovsdb_idl_condition *, enum ovsdb_function function, const struct uuid *chassis);
void sbrec_controller_event_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void sbrec_controller_event_update_event_info_setkey(const struct sbrec_controller_event *,  const char *, const char *);
void sbrec_controller_event_update_event_info_delkey(const struct sbrec_controller_event *,  const char *);
void sbrec_controller_event_add_clause_event_info(struct ovsdb_idl_condition *, enum ovsdb_function function, const struct smap *);
void sbrec_controller_event_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void sbrec_controller_event_add_clause_event_type(struct ovsdb_idl_condition *, enum ovsdb_function function, const char *event_type);
void sbrec_controller_event_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void sbrec_controller_event_add_clause_seq_num(struct ovsdb_idl_condition *, enum ovsdb_function function, int64_t seq_num);
void sbrec_controller_event_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);

struct sbrec_controller_event *sbrec_controller_event_index_init_row(struct ovsdb_idl_index *);
void sbrec_controller_event_index_set_chassis(const struct sbrec_controller_event *,
const struct sbrec_chassis *chassis);
void sbrec_controller_event_index_set_event_info(const struct sbrec_controller_event *,
const struct smap *);
void sbrec_controller_event_index_set_event_type(const struct sbrec_controller_event *,
const char *event_type);
void sbrec_controller_event_index_set_seq_num(const struct sbrec_controller_event *,
int64_t seq_num);

/* DHCP_Options table. */
...
enum sbrec_ha_chassis_column_id {
    SBREC_HA_CHASSIS_COL_CHASSIS,
    SBREC_HA_CHASSIS_COL_EXTERNAL_IDS,
    SBREC_HA_CHASSIS_COL_PRIORITY,
    SBREC_HA_CHASSIS_N_COLUMNS
};

#define sbrec_ha_chassis_col_chassis (sbrec_ha_chassis_columns[SBREC_HA_CHASSIS_COL_CHASSIS])
#define sbrec_ha_chassis_col_external_ids (sbrec_ha_chassis_columns[SBREC_HA_CHASSIS_COL_EXTERNAL_IDS])
#define sbrec_ha_chassis_col_priority (sbrec_ha_chassis_columns[SBREC_HA_CHASSIS_COL_PRIORITY])

extern struct ovsdb_idl_column sbrec_ha_chassis_columns[SBREC_HA_CHASSIS_N_COLUMNS];

const struct sbrec_ha_chassis_table *sbrec_ha_chassis_table_get(const struct ovsdb_idl *);
const struct sbrec_ha_chassis *sbrec_ha_chassis_table_first(const struct sbrec_ha_chassis_table *);

#define SBREC_HA_CHASSIS_TABLE_FOR_EACH(ROW, TABLE) \
        for ((ROW) = sbrec_ha_chassis_table_first(TABLE); \
             (ROW); \
             (ROW) = sbrec_ha_chassis_next(ROW))
#define SBREC_HA_CHASSIS_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = sbrec_ha_chassis_table_first(TABLE); \
             (ROW) ? ((NEXT) = sbrec_ha_chassis_next(ROW), 1) : 0; \
             (ROW) = (NEXT))

const struct sbrec_ha_chassis *sbrec_ha_chassis_get_for_uuid(const struct ovsdb_idl *, const struct uuid *);
const struct sbrec_ha_chassis *sbrec_ha_chassis_table_get_for_uuid(const struct sbrec_ha_chassis_table *, const struct uuid *);
const struct sbrec_ha_chassis *sbrec_ha_chassis_first(const struct ovsdb_idl *);
const struct sbrec_ha_chassis *sbrec_ha_chassis_next(const struct sbrec_ha_chassis *);
#define SBREC_HA_CHASSIS_FOR_EACH(ROW, IDL) \
        for ((ROW) = sbrec_ha_chassis_first(IDL); \
             (ROW); \
             (ROW) = sbrec_ha_chassis_next(ROW))
#define SBREC_HA_CHASSIS_FOR_EACH_SAFE(ROW, NEXT, IDL) \
        for ((ROW) = sbrec_ha_chassis_first(IDL); \
             (ROW) ? ((NEXT) = sbrec_ha_chassis_next(ROW), 1) : 0; \
             (ROW) = (NEXT))

unsigned int sbrec_ha_chassis_get_seqno(const struct ovsdb_idl *);
unsigned int sbrec_ha_chassis_row_get_seqno(const struct sbrec_ha_chassis *row, enum ovsdb_idl_change change);
const struct sbrec_ha_chassis *sbrec_ha_chassis_track_get_first(const struct ovsdb_idl *);
const struct sbrec_ha_chassis *sbrec_ha_chassis_track_get_next(const struct sbrec_ha_chassis *);
#define SBREC_HA_CHASSIS_FOR_EACH_TRACKED(ROW, IDL) \
        for ((ROW) = sbrec_ha_chassis_track_get_first(IDL); \
             (ROW); \
             (ROW) = sbrec_ha_chassis_track_get_next(ROW))

const struct sbrec_ha_chassis *sbrec_ha_chassis_table_track_get_first(const struct sbrec_ha_chassis_table *);
#define SBREC_HA_CHASSIS_TABLE_FOR_EACH_TRACKED(ROW, TABLE) \
        for ((ROW) = sbrec_ha_chassis_table_track_get_first(TABLE); \
             (ROW); \
             (ROW) = sbrec_ha_chassis_track_get_next(ROW))


/* Returns true if 'row' was inserted since the last change tracking reset. */
static inline bool sbrec_ha_chassis_is_new(const struct sbrec_ha_chassis *row)
{
    return sbrec_ha_chassis_row_get_seqno(row, OVSDB_IDL_CHANGE_MODIFY) == 0;
}

/* Returns true if 'row' was deleted since the last change tracking reset. */
static inline bool sbrec_ha_chassis_is_deleted(const struct sbrec_ha_chassis *row)
{
    return sbrec_ha_chassis_row_get_seqno(row, OVSDB_IDL_CHANGE_DELETE) > 0;
}

void sbrec_ha_chassis_index_destroy_row(const struct sbrec_ha_chassis *);

struct sbrec_ha_chassis *sbrec_ha_chassis_index_find(struct ovsdb_idl_index *, const struct sbrec_ha_chassis *);

int sbrec_ha_chassis_index_compare(
    struct ovsdb_idl_index *, 
    const struct sbrec_ha_chassis *, 
    const struct sbrec_ha_chassis *);
struct ovsdb_idl_cursor sbrec_ha_chassis_cursor_first(struct ovsdb_idl_index *);
struct ovsdb_idl_cursor sbrec_ha_chassis_cursor_first_eq(
    struct ovsdb_idl_index *, const struct sbrec_ha_chassis *);
struct ovsdb_idl_cursor sbrec_ha_chassis_cursor_first_ge(
    struct ovsdb_idl_index *, const struct sbrec_ha_chassis *);

struct sbrec_ha_chassis *sbrec_ha_chassis_cursor_data(struct ovsdb_idl_cursor *);

#define SBREC_HA_CHASSIS_FOR_EACH_RANGE(ROW, FROM, TO, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = sbrec_ha_chassis_cursor_first_ge(INDEX, FROM); \
             (cursor__.position \
              && ((ROW) = sbrec_ha_chassis_cursor_data(&cursor__), \
                  !(TO) || sbrec_ha_chassis_index_compare(INDEX, ROW, TO) <= 0)); \
             ovsdb_idl_cursor_next(&cursor__))
#define SBREC_HA_CHASSIS_FOR_EACH_EQUAL(ROW, KEY, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = sbrec_ha_chassis_cursor_first_eq(INDEX, KEY); \
             (cursor__.position \
              ? ((ROW) = sbrec_ha_chassis_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next_eq(&cursor__), \
                 true) \
...
#define SBREC_HA_CHASSIS_FOR_EACH_BYINDEX(ROW, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = sbrec_ha_chassis_cursor_first(INDEX); \
             (cursor__.position \
              ? ((ROW) = sbrec_ha_chassis_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next(&cursor__), \
                 true) \
...
void sbrec_ha_chassis_init(struct sbrec_ha_chassis *);
void sbrec_ha_chassis_delete(const struct sbrec_ha_chassis *);
struct sbrec_ha_chassis *sbrec_ha_chassis_insert(struct ovsdb_idl_txn *);
bool sbrec_ha_chassis_is_updated(const struct sbrec_ha_chassis *, enum sbrec_ha_chassis_column_id);

void sbrec_ha_chassis_verify_chassis(const struct sbrec_ha_chassis *);
void sbrec_ha_chassis_verify_external_ids(const struct sbrec_ha_chassis *);
void sbrec_ha_chassis_verify_priority(const struct sbrec_ha_chassis *);

const struct ovsdb_datum *sbrec_ha_chassis_get_chassis(const struct sbrec_ha_chassis *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *sbrec_ha_chassis_get_external_ids(const struct sbrec_ha_chassis *, enum ovsdb_atomic_type key_type, enum ovsdb_atomic_type value_type);
const struct ovsdb_datum *sbrec_ha_chassis_get_priority(const struct sbrec_ha_chassis *, enum ovsdb_atomic_type key_type);

void sbrec_ha_chassis_set_chassis(const struct sbrec_ha_chassis *, const struct sbrec_chassis *chassis);
void sbrec_ha_chassis_set_external_ids(const struct sbrec_ha_chassis *, const struct smap *);
void sbrec_ha_chassis_set_priority(const struct sbrec_ha_chassis *, int64_t priority);

void sbrec_ha_chassis_update_chassis_addvalue(const struct sbrec_ha_chassis *,  const struct sbrec_chassis *);
void sbrec_ha_chassis_update_chassis_delvalue(const struct sbrec_ha_chassis *,  const struct sbrec_chassis *);
void sbrec_ha_chassis_add_clause_chassis(struct ovsdb_idl_condition *, enum ovsdb_function function, const struct uuid *chassis);
void sbrec_ha_chassis_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void sbrec_ha_chassis_update_external_ids_setkey(const struct sbrec_ha_chassis *,  const char *, const char *);
void sbrec_ha_chassis_update_external_ids_delkey(const struct sbrec_ha_chassis *,  const char *);
void sbrec_ha_chassis_add_clause_external_ids(struct ovsdb_idl_condition *, enum ovsdb_function function, const struct smap *);
void sbrec_ha_chassis_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void sbrec_ha_chassis_add_clause_priority(struct ovsdb_idl_condition *, enum ovsdb_function function, int64_t priority);
void sbrec_ha_chassis_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);

struct sbrec_ha_chassis *sbrec_ha_chassis_index_init_row(struct ovsdb_idl_index *);
void sbrec_ha_chassis_index_set_chassis(const struct sbrec_ha_chassis *,
const struct sbrec_chassis *chassis);
void sbrec_ha_chassis_index_set_external_ids(const struct sbrec_ha_chassis *,
const struct smap *);
void sbrec_ha_chassis_index_set_priority(const struct sbrec_ha_chassis *,
int64_t priority);

...
struct sbrec_ha_chassis_group {
	...
	/* ha_chassis column. */
	struct sbrec_ha_chassis **ha_chassis;
	size_t n_ha_chassis;
...
enum sbrec_ha_chassis_group_column_id {
    ...
    SBREC_HA_CHASSIS_GROUP_N_COLUMNS
};

#define sbrec_ha_chassis_group_col_external_ids (sbrec_ha_chassis_group_columns[SBREC_HA_CHASSIS_GROUP_COL_EXTERNAL_IDS])
#define sbrec_ha_chassis_group_col_ha_chassis (sbrec_ha_chassis_group_columns[SBREC_HA_CHASSIS_GROUP_COL_HA_CHASSIS])
#define sbrec_ha_chassis_group_col_name (sbrec_ha_chassis_group_columns[SBREC_HA_CHASSIS_GROUP_COL_NAME])
#define sbrec_ha_chassis_group_col_ref_chassis (sbrec_ha_chassis_group_columns[SBREC_HA_CHASSIS_GROUP_COL_REF_CHASSIS])

extern struct ovsdb_idl_column sbrec_ha_chassis_group_columns[SBREC_HA_CHASSIS_GROUP_N_COLUMNS];

const struct sbrec_ha_chassis_group_table *sbrec_ha_chassis_group_table_get(const struct ovsdb_idl *);
const struct sbrec_ha_chassis_group *sbrec_ha_chassis_group_table_first(const struct sbrec_ha_chassis_group_table *);

#define SBREC_HA_CHASSIS_GROUP_TABLE_FOR_EACH(ROW, TABLE) \
        for ((ROW) = sbrec_ha_chassis_group_table_first(TABLE); \
             (ROW); \
             (ROW) = sbrec_ha_chassis_group_next(ROW))
#define SBREC_HA_CHASSIS_GROUP_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = sbrec_ha_chassis_group_table_first(TABLE); \
             (ROW) ? ((NEXT) = sbrec_ha_chassis_group_next(ROW), 1) : 0; \
             (ROW) = (NEXT))

const struct sbrec_ha_chassis_group *sbrec_ha_chassis_group_get_for_uuid(const struct ovsdb_idl *, const struct uuid *);
const struct sbrec_ha_chassis_group *sbrec_ha_chassis_group_table_get_for_uuid(const struct sbrec_ha_chassis_group_table *, const struct uuid *);
const struct sbrec_ha_chassis_group *sbrec_ha_chassis_group_first(const struct ovsdb_idl *);
const struct sbrec_ha_chassis_group *sbrec_ha_chassis_group_next(const struct sbrec_ha_chassis_group *);
#define SBREC_HA_CHASSIS_GROUP_FOR_EACH(ROW, IDL) \
        for ((ROW) = sbrec_ha_chassis_group_first(IDL); \
             (ROW); \
             (ROW) = sbrec_ha_chassis_group_next(ROW))
#define SBREC_HA_CHASSIS_GROUP_FOR_EACH_SAFE(ROW, NEXT, IDL) \
        for ((ROW) = sbrec_ha_chassis_group_first(IDL); \
             (ROW) ? ((NEXT) = sbrec_ha_chassis_group_next(ROW), 1) : 0; \
             (ROW) = (NEXT))

unsigned int sbrec_ha_chassis_group_get_seqno(const struct ovsdb_idl *);
unsigned int sbrec_ha_chassis_group_row_get_seqno(const struct sbrec_ha_chassis_group *row, enum ovsdb_idl_change change);
const struct sbrec_ha_chassis_group *sbrec_ha_chassis_group_track_get_first(const struct ovsdb_idl *);
const struct sbrec_ha_chassis_group *sbrec_ha_chassis_group_track_get_next(const struct sbrec_ha_chassis_group *);
#define SBREC_HA_CHASSIS_GROUP_FOR_EACH_TRACKED(ROW, IDL) \
        for ((ROW) = sbrec_ha_chassis_group_track_get_first(IDL); \
             (ROW); \
             (ROW) = sbrec_ha_chassis_group_track_get_next(ROW))

const struct sbrec_ha_chassis_group *sbrec_ha_chassis_group_table_track_get_first(const struct sbrec_ha_chassis_group_table *);
#define SBREC_HA_CHASSIS_GROUP_TABLE_FOR_EACH_TRACKED(ROW, TABLE) \
        for ((ROW) = sbrec_ha_chassis_group_table_track_get_first(TABLE); \
             (ROW); \
             (ROW) = sbrec_ha_chassis_group_track_get_next(ROW))


/* Returns true if 'row' was inserted since the last change tracking reset. */
static inline bool sbrec_ha_chassis_group_is_new(const struct sbrec_ha_chassis_group *row)
{
    return sbrec_ha_chassis_group_row_get_seqno(row, OVSDB_IDL_CHANGE_MODIFY) == 0;
}

/* Returns true if 'row' was deleted since the last change tracking reset. */
static inline bool sbrec_ha_chassis_group_is_deleted(const struct sbrec_ha_chassis_group *row)
{
    return sbrec_ha_chassis_group_row_get_seqno(row, OVSDB_IDL_CHANGE_DELETE) > 0;
}

void sbrec_ha_chassis_group_index_destroy_row(const struct sbrec_ha_chassis_group *);

struct sbrec_ha_chassis_group *sbrec_ha_chassis_group_index_find(struct ovsdb_idl_index *, const struct sbrec_ha_chassis_group *);

int sbrec_ha_chassis_group_index_compare(
    struct ovsdb_idl_index *, 
    const struct sbrec_ha_chassis_group *, 
    const struct sbrec_ha_chassis_group *);
struct ovsdb_idl_cursor sbrec_ha_chassis_group_cursor_first(struct ovsdb_idl_index *);
struct ovsdb_idl_cursor sbrec_ha_chassis_group_cursor_first_eq(
    struct ovsdb_idl_index *, const struct sbrec_ha_chassis_group *);
struct ovsdb_idl_cursor sbrec_ha_chassis_group_cursor_first_ge(
    struct ovsdb_idl_index *, const struct sbrec_ha_chassis_group *);

struct sbrec_ha_chassis_group *sbrec_ha_chassis_group_cursor_data(struct ovsdb_idl_cursor *);

#define SBREC_HA_CHASSIS_GROUP_FOR_EACH_RANGE(ROW, FROM, TO, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = sbrec_ha_chassis_group_cursor_first_ge(INDEX, FROM); \
             (cursor__.position \
              && ((ROW) = sbrec_ha_chassis_group_cursor_data(&cursor__), \
                  !(TO) || sbrec_ha_chassis_group_index_compare(INDEX, ROW, TO) <= 0)); \
             ovsdb_idl_cursor_next(&cursor__))
#define SBREC_HA_CHASSIS_GROUP_FOR_EACH_EQUAL(ROW, KEY, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = sbrec_ha_chassis_group_cursor_first_eq(INDEX, KEY); \
             (cursor__.position \
              ? ((ROW) = sbrec_ha_chassis_group_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next_eq(&cursor__), \
                 true) \
...
#define SBREC_HA_CHASSIS_GROUP_FOR_EACH_BYINDEX(ROW, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = sbrec_ha_chassis_group_cursor_first(INDEX); \
             (cursor__.position \
              ? ((ROW) = sbrec_ha_chassis_group_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next(&cursor__), \
                 true) \
...
void sbrec_ha_chassis_group_init(struct sbrec_ha_chassis_group *);
void sbrec_ha_chassis_group_delete(const struct sbrec_ha_chassis_group *);
struct sbrec_ha_chassis_group *sbrec_ha_chassis_group_insert(struct ovsdb_idl_txn *);
bool sbrec_ha_chassis_group_is_updated(const struct sbrec_ha_chassis_group *, enum sbrec_ha_chassis_group_column_id);

void sbrec_ha_chassis_group_verify_external_ids(const struct sbrec_ha_chassis_group *);
void sbrec_ha_chassis_group_verify_ha_chassis(const struct sbrec_ha_chassis_group *);
void sbrec_ha_chassis_group_verify_name(const struct sbrec_ha_chassis_group *);
void sbrec_ha_chassis_group_verify_ref_chassis(const struct sbrec_ha_chassis_group *);

const struct ovsdb_datum *sbrec_ha_chassis_group_get_external_ids(const struct sbrec_ha_chassis_group *, enum ovsdb_atomic_type key_type, enum ovsdb_atomic_type value_type);
const struct ovsdb_datum *sbrec_ha_chassis_group_get_ha_chassis(const struct sbrec_ha_chassis_group *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *sbrec_ha_chassis_group_get_name(const struct sbrec_ha_chassis_group *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *sbrec_ha_chassis_group_get_ref_chassis(const struct sbrec_ha_chassis_group *, enum ovsdb_atomic_type key_type);

void sbrec_ha_chassis_group_set_external_ids(const struct sbrec_ha_chassis_group *, const struct smap *);
void sbrec_ha_chassis_group_set_ha_chassis(const struct sbrec_ha_chassis_group *, struct sbrec_ha_chassis **ha_chassis, size_t n_ha_chassis);
void sbrec_ha_chassis_group_set_name(const struct sbrec_ha_chassis_group *, const char *name);
void sbrec_ha_chassis_group_set_ref_chassis(const struct sbrec_ha_chassis_group *, struct sbrec_chassis **ref_chassis, size_t n_ref_chassis);

void sbrec_ha_chassis_group_update_external_ids_setkey(const struct sbrec_ha_chassis_group *,  const char *, const char *);
void sbrec_ha_chassis_group_update_external_ids_delkey(const struct sbrec_ha_chassis_group *,  const char *);
void sbrec_ha_chassis_group_add_clause_external_ids(struct ovsdb_idl_condition *, enum ovsdb_function function, const struct smap *);
void sbrec_ha_chassis_group_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void sbrec_ha_chassis_group_update_ha_chassis_addvalue(const struct sbrec_ha_chassis_group *,  const struct sbrec_ha_chassis *);
void sbrec_ha_chassis_group_update_ha_chassis_delvalue(const struct sbrec_ha_chassis_group *,  const struct sbrec_ha_chassis *);
void sbrec_ha_chassis_group_add_clause_ha_chassis(struct ovsdb_idl_condition *, enum ovsdb_function function, struct uuid **ha_chassis, size_t n_ha_chassis);
void sbrec_ha_chassis_group_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void sbrec_ha_chassis_group_add_clause_name(struct ovsdb_idl_condition *, enum ovsdb_function function, const char *name);
void sbrec_ha_chassis_group_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void sbrec_ha_chassis_group_update_ref_chassis_addvalue(const struct sbrec_ha_chassis_group *,  const struct sbrec_chassis *);
void sbrec_ha_chassis_group_update_ref_chassis_delvalue(const struct sbrec_ha_chassis_group *,  const struct sbrec_chassis *);
void sbrec_ha_chassis_group_add_clause_ref_chassis(struct ovsdb_idl_condition *, enum ovsdb_function function, struct uuid **ref_chassis, size_t n_ref_chassis);
void sbrec_ha_chassis_group_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);

struct sbrec_ha_chassis_group *sbrec_ha_chassis_group_index_init_row(struct ovsdb_idl_index *);
void sbrec_ha_chassis_group_index_set_external_ids(const struct sbrec_ha_chassis_group *,
const struct smap *);
void sbrec_ha_chassis_group_index_set_ha_chassis(const struct sbrec_ha_chassis_group *,
struct sbrec_ha_chassis **ha_chassis, size_t n_ha_chassis);
void sbrec_ha_chassis_group_index_set_name(const struct sbrec_ha_chassis_group *,
const char *name);
void sbrec_ha_chassis_group_index_set_ref_chassis(const struct sbrec_ha_chassis_group *,
struct sbrec_chassis **ref_chassis, size_t n_ref_chassis);

/* IGMP_Group table. */
...
enum sbrec_igmp_group_column_id {
    SBREC_IGMP_GROUP_COL_ADDRESS,
    SBREC_IGMP_GROUP_COL_CHASSIS,
    SBREC_IGMP_GROUP_COL_DATAPATH,
    SBREC_IGMP_GROUP_COL_PORTS,
    SBREC_IGMP_GROUP_N_COLUMNS
};

#define sbrec_igmp_group_col_address (sbrec_igmp_group_columns[SBREC_IGMP_GROUP_COL_ADDRESS])
#define sbrec_igmp_group_col_chassis (sbrec_igmp_group_columns[SBREC_IGMP_GROUP_COL_CHASSIS])
#define sbrec_igmp_group_col_datapath (sbrec_igmp_group_columns[SBREC_IGMP_GROUP_COL_DATAPATH])
#define sbrec_igmp_group_col_ports (sbrec_igmp_group_columns[SBREC_IGMP_GROUP_COL_PORTS])

extern struct ovsdb_idl_column sbrec_igmp_group_columns[SBREC_IGMP_GROUP_N_COLUMNS];

const struct sbrec_igmp_group_table *sbrec_igmp_group_table_get(const struct ovsdb_idl *);
const struct sbrec_igmp_group *sbrec_igmp_group_table_first(const struct sbrec_igmp_group_table *);

#define SBREC_IGMP_GROUP_TABLE_FOR_EACH(ROW, TABLE) \
        for ((ROW) = sbrec_igmp_group_table_first(TABLE); \
             (ROW); \
             (ROW) = sbrec_igmp_group_next(ROW))
#define SBREC_IGMP_GROUP_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = sbrec_igmp_group_table_first(TABLE); \
             (ROW) ? ((NEXT) = sbrec_igmp_group_next(ROW), 1) : 0; \
             (ROW) = (NEXT))

const struct sbrec_igmp_group *sbrec_igmp_group_get_for_uuid(const struct ovsdb_idl *, const struct uuid *);
const struct sbrec_igmp_group *sbrec_igmp_group_table_get_for_uuid(const struct sbrec_igmp_group_table *, const struct uuid *);
const struct sbrec_igmp_group *sbrec_igmp_group_first(const struct ovsdb_idl *);
const struct sbrec_igmp_group *sbrec_igmp_group_next(const struct sbrec_igmp_group *);
#define SBREC_IGMP_GROUP_FOR_EACH(ROW, IDL) \
        for ((ROW) = sbrec_igmp_group_first(IDL); \
             (ROW); \
             (ROW) = sbrec_igmp_group_next(ROW))
#define SBREC_IGMP_GROUP_FOR_EACH_SAFE(ROW, NEXT, IDL) \
        for ((ROW) = sbrec_igmp_group_first(IDL); \
             (ROW) ? ((NEXT) = sbrec_igmp_group_next(ROW), 1) : 0; \
             (ROW) = (NEXT))

unsigned int sbrec_igmp_group_get_seqno(const struct ovsdb_idl *);
unsigned int sbrec_igmp_group_row_get_seqno(const struct sbrec_igmp_group *row, enum ovsdb_idl_change change);
const struct sbrec_igmp_group *sbrec_igmp_group_track_get_first(const struct ovsdb_idl *);
const struct sbrec_igmp_group *sbrec_igmp_group_track_get_next(const struct sbrec_igmp_group *);
#define SBREC_IGMP_GROUP_FOR_EACH_TRACKED(ROW, IDL) \
        for ((ROW) = sbrec_igmp_group_track_get_first(IDL); \
             (ROW); \
             (ROW) = sbrec_igmp_group_track_get_next(ROW))

const struct sbrec_igmp_group *sbrec_igmp_group_table_track_get_first(const struct sbrec_igmp_group_table *);
#define SBREC_IGMP_GROUP_TABLE_FOR_EACH_TRACKED(ROW, TABLE) \
        for ((ROW) = sbrec_igmp_group_table_track_get_first(TABLE); \
             (ROW); \
             (ROW) = sbrec_igmp_group_track_get_next(ROW))


/* Returns true if 'row' was inserted since the last change tracking reset. */
static inline bool sbrec_igmp_group_is_new(const struct sbrec_igmp_group *row)
{
    return sbrec_igmp_group_row_get_seqno(row, OVSDB_IDL_CHANGE_MODIFY) == 0;
}

/* Returns true if 'row' was deleted since the last change tracking reset. */
static inline bool sbrec_igmp_group_is_deleted(const struct sbrec_igmp_group *row)
{
    return sbrec_igmp_group_row_get_seqno(row, OVSDB_IDL_CHANGE_DELETE) > 0;
}

void sbrec_igmp_group_index_destroy_row(const struct sbrec_igmp_group *);

struct sbrec_igmp_group *sbrec_igmp_group_index_find(struct ovsdb_idl_index *, const struct sbrec_igmp_group *);

int sbrec_igmp_group_index_compare(
    struct ovsdb_idl_index *, 
    const struct sbrec_igmp_group *, 
    const struct sbrec_igmp_group *);
struct ovsdb_idl_cursor sbrec_igmp_group_cursor_first(struct ovsdb_idl_index *);
struct ovsdb_idl_cursor sbrec_igmp_group_cursor_first_eq(
    struct ovsdb_idl_index *, const struct sbrec_igmp_group *);
struct ovsdb_idl_cursor sbrec_igmp_group_cursor_first_ge(
    struct ovsdb_idl_index *, const struct sbrec_igmp_group *);

struct sbrec_igmp_group *sbrec_igmp_group_cursor_data(struct ovsdb_idl_cursor *);

#define SBREC_IGMP_GROUP_FOR_EACH_RANGE(ROW, FROM, TO, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = sbrec_igmp_group_cursor_first_ge(INDEX, FROM); \
             (cursor__.position \
              && ((ROW) = sbrec_igmp_group_cursor_data(&cursor__), \
                  !(TO) || sbrec_igmp_group_index_compare(INDEX, ROW, TO) <= 0)); \
             ovsdb_idl_cursor_next(&cursor__))
#define SBREC_IGMP_GROUP_FOR_EACH_EQUAL(ROW, KEY, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = sbrec_igmp_group_cursor_first_eq(INDEX, KEY); \
             (cursor__.position \
              ? ((ROW) = sbrec_igmp_group_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next_eq(&cursor__), \
                 true) \
...
#define SBREC_IGMP_GROUP_FOR_EACH_BYINDEX(ROW, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = sbrec_igmp_group_cursor_first(INDEX); \
             (cursor__.position \
              ? ((ROW) = sbrec_igmp_group_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next(&cursor__), \
                 true) \
...
void sbrec_igmp_group_init(struct sbrec_igmp_group *);
void sbrec_igmp_group_delete(const struct sbrec_igmp_group *);
struct sbrec_igmp_group *sbrec_igmp_group_insert(struct ovsdb_idl_txn *);
bool sbrec_igmp_group_is_updated(const struct sbrec_igmp_group *, enum sbrec_igmp_group_column_id);

void sbrec_igmp_group_verify_address(const struct sbrec_igmp_group *);
void sbrec_igmp_group_verify_chassis(const struct sbrec_igmp_group *);
void sbrec_igmp_group_verify_datapath(const struct sbrec_igmp_group *);
void sbrec_igmp_group_verify_ports(const struct sbrec_igmp_group *);

const struct ovsdb_datum *sbrec_igmp_group_get_address(const struct sbrec_igmp_group *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *sbrec_igmp_group_get_chassis(const struct sbrec_igmp_group *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *sbrec_igmp_group_get_datapath(const struct sbrec_igmp_group *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *sbrec_igmp_group_get_ports(const struct sbrec_igmp_group *, enum ovsdb_atomic_type key_type);

void sbrec_igmp_group_set_address(const struct sbrec_igmp_group *, const char *address);
void sbrec_igmp_group_set_chassis(const struct sbrec_igmp_group *, const struct sbrec_chassis *chassis);
void sbrec_igmp_group_set_datapath(const struct sbrec_igmp_group *, const struct sbrec_datapath_binding *datapath);
void sbrec_igmp_group_set_ports(const struct sbrec_igmp_group *, struct sbrec_port_binding **ports, size_t n_ports);

void sbrec_igmp_group_add_clause_address(struct ovsdb_idl_condition *, enum ovsdb_function function, const char *address);
void sbrec_igmp_group_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void sbrec_igmp_group_update_chassis_addvalue(const struct sbrec_igmp_group *,  const struct sbrec_chassis *);
void sbrec_igmp_group_update_chassis_delvalue(const struct sbrec_igmp_group *,  const struct sbrec_chassis *);
void sbrec_igmp_group_add_clause_chassis(struct ovsdb_idl_condition *, enum ovsdb_function function, const struct uuid *chassis);
void sbrec_igmp_group_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void sbrec_igmp_group_update_datapath_addvalue(const struct sbrec_igmp_group *,  const struct sbrec_datapath_binding *);
void sbrec_igmp_group_update_datapath_delvalue(const struct sbrec_igmp_group *,  const struct sbrec_datapath_binding *);
void sbrec_igmp_group_add_clause_datapath(struct ovsdb_idl_condition *, enum ovsdb_function function, const struct uuid *datapath);
void sbrec_igmp_group_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void sbrec_igmp_group_update_ports_addvalue(const struct sbrec_igmp_group *,  const struct sbrec_port_binding *);
void sbrec_igmp_group_update_ports_delvalue(const struct sbrec_igmp_group *,  const struct sbrec_port_binding *);
void sbrec_igmp_group_add_clause_ports(struct ovsdb_idl_condition *, enum ovsdb_function function, struct uuid **ports, size_t n_ports);
void sbrec_igmp_group_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);

struct sbrec_igmp_group *sbrec_igmp_group_index_init_row(struct ovsdb_idl_index *);
void sbrec_igmp_group_index_set_address(const struct sbrec_igmp_group *,
const char *address);
void sbrec_igmp_group_index_set_chassis(const struct sbrec_igmp_group *,
const struct sbrec_chassis *chassis);
void sbrec_igmp_group_index_set_datapath(const struct sbrec_igmp_group *,
const struct sbrec_datapath_binding *datapath);
void sbrec_igmp_group_index_set_ports(const struct sbrec_igmp_group *,
...
struct sbrec_ip_multicast {
	...
	/* eth_src column. */
	char *eth_src;	/* Always nonnull. */

	/* idle_timeout column. */
	int64_t *idle_timeout;
	size_t n_idle_timeout;

	/* ip4_src column. */
	char *ip4_src;	/* Always nonnull. */
	...
	size_t n_query_interval;

	/* query_max_resp column. */
	int64_t *query_max_resp;
	size_t n_query_max_resp;
...
enum sbrec_ip_multicast_column_id {
    ...
    SBREC_IP_MULTICAST_N_COLUMNS
};

#define sbrec_ip_multicast_col_datapath (sbrec_ip_multicast_columns[SBREC_IP_MULTICAST_COL_DATAPATH])
#define sbrec_ip_multicast_col_enabled (sbrec_ip_multicast_columns[SBREC_IP_MULTICAST_COL_ENABLED])
#define sbrec_ip_multicast_col_eth_src (sbrec_ip_multicast_columns[SBREC_IP_MULTICAST_COL_ETH_SRC])
#define sbrec_ip_multicast_col_idle_timeout (sbrec_ip_multicast_columns[SBREC_IP_MULTICAST_COL_IDLE_TIMEOUT])
#define sbrec_ip_multicast_col_ip4_src (sbrec_ip_multicast_columns[SBREC_IP_MULTICAST_COL_IP4_SRC])
#define sbrec_ip_multicast_col_querier (sbrec_ip_multicast_columns[SBREC_IP_MULTICAST_COL_QUERIER])
#define sbrec_ip_multicast_col_query_interval (sbrec_ip_multicast_columns[SBREC_IP_MULTICAST_COL_QUERY_INTERVAL])
#define sbrec_ip_multicast_col_query_max_resp (sbrec_ip_multicast_columns[SBREC_IP_MULTICAST_COL_QUERY_MAX_RESP])
#define sbrec_ip_multicast_col_seq_no (sbrec_ip_multicast_columns[SBREC_IP_MULTICAST_COL_SEQ_NO])
#define sbrec_ip_multicast_col_table_size (sbrec_ip_multicast_columns[SBREC_IP_MULTICAST_COL_TABLE_SIZE])

extern struct ovsdb_idl_column sbrec_ip_multicast_columns[SBREC_IP_MULTICAST_N_COLUMNS];

const struct sbrec_ip_multicast_table *sbrec_ip_multicast_table_get(const struct ovsdb_idl *);
const struct sbrec_ip_multicast *sbrec_ip_multicast_table_first(const struct sbrec_ip_multicast_table *);

#define SBREC_IP_MULTICAST_TABLE_FOR_EACH(ROW, TABLE) \
        for ((ROW) = sbrec_ip_multicast_table_first(TABLE); \
             (ROW); \
             (ROW) = sbrec_ip_multicast_next(ROW))
#define SBREC_IP_MULTICAST_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = sbrec_ip_multicast_table_first(TABLE); \
             (ROW) ? ((NEXT) = sbrec_ip_multicast_next(ROW), 1) : 0; \
             (ROW) = (NEXT))

const struct sbrec_ip_multicast *sbrec_ip_multicast_get_for_uuid(const struct ovsdb_idl *, const struct uuid *);
const struct sbrec_ip_multicast *sbrec_ip_multicast_table_get_for_uuid(const struct sbrec_ip_multicast_table *, const struct uuid *);
const struct sbrec_ip_multicast *sbrec_ip_multicast_first(const struct ovsdb_idl *);
const struct sbrec_ip_multicast *sbrec_ip_multicast_next(const struct sbrec_ip_multicast *);
#define SBREC_IP_MULTICAST_FOR_EACH(ROW, IDL) \
        for ((ROW) = sbrec_ip_multicast_first(IDL); \
             (ROW); \
             (ROW) = sbrec_ip_multicast_next(ROW))
#define SBREC_IP_MULTICAST_FOR_EACH_SAFE(ROW, NEXT, IDL) \
        for ((ROW) = sbrec_ip_multicast_first(IDL); \
             (ROW) ? ((NEXT) = sbrec_ip_multicast_next(ROW), 1) : 0; \
             (ROW) = (NEXT))

unsigned int sbrec_ip_multicast_get_seqno(const struct ovsdb_idl *);
unsigned int sbrec_ip_multicast_row_get_seqno(const struct sbrec_ip_multicast *row, enum ovsdb_idl_change change);
const struct sbrec_ip_multicast *sbrec_ip_multicast_track_get_first(const struct ovsdb_idl *);
const struct sbrec_ip_multicast *sbrec_ip_multicast_track_get_next(const struct sbrec_ip_multicast *);
#define SBREC_IP_MULTICAST_FOR_EACH_TRACKED(ROW, IDL) \
        for ((ROW) = sbrec_ip_multicast_track_get_first(IDL); \
             (ROW); \
             (ROW) = sbrec_ip_multicast_track_get_next(ROW))

const struct sbrec_ip_multicast *sbrec_ip_multicast_table_track_get_first(const struct sbrec_ip_multicast_table *);
#define SBREC_IP_MULTICAST_TABLE_FOR_EACH_TRACKED(ROW, TABLE) \
        for ((ROW) = sbrec_ip_multicast_table_track_get_first(TABLE); \
             (ROW); \
             (ROW) = sbrec_ip_multicast_track_get_next(ROW))


/* Returns true if 'row' was inserted since the last change tracking reset. */
static inline bool sbrec_ip_multicast_is_new(const struct sbrec_ip_multicast *row)
{
    return sbrec_ip_multicast_row_get_seqno(row, OVSDB_IDL_CHANGE_MODIFY) == 0;
}

/* Returns true if 'row' was deleted since the last change tracking reset. */
static inline bool sbrec_ip_multicast_is_deleted(const struct sbrec_ip_multicast *row)
{
    return sbrec_ip_multicast_row_get_seqno(row, OVSDB_IDL_CHANGE_DELETE) > 0;
}

void sbrec_ip_multicast_index_destroy_row(const struct sbrec_ip_multicast *);

struct sbrec_ip_multicast *sbrec_ip_multicast_index_find(struct ovsdb_idl_index *, const struct sbrec_ip_multicast *);

int sbrec_ip_multicast_index_compare(
    struct ovsdb_idl_index *, 
    const struct sbrec_ip_multicast *, 
    const struct sbrec_ip_multicast *);
struct ovsdb_idl_cursor sbrec_ip_multicast_cursor_first(struct ovsdb_idl_index *);
struct ovsdb_idl_cursor sbrec_ip_multicast_cursor_first_eq(
    struct ovsdb_idl_index *, const struct sbrec_ip_multicast *);
struct ovsdb_idl_cursor sbrec_ip_multicast_cursor_first_ge(
    struct ovsdb_idl_index *, const struct sbrec_ip_multicast *);

struct sbrec_ip_multicast *sbrec_ip_multicast_cursor_data(struct ovsdb_idl_cursor *);

#define SBREC_IP_MULTICAST_FOR_EACH_RANGE(ROW, FROM, TO, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = sbrec_ip_multicast_cursor_first_ge(INDEX, FROM); \
             (cursor__.position \
              && ((ROW) = sbrec_ip_multicast_cursor_data(&cursor__), \
                  !(TO) || sbrec_ip_multicast_index_compare(INDEX, ROW, TO) <= 0)); \
             ovsdb_idl_cursor_next(&cursor__))
#define SBREC_IP_MULTICAST_FOR_EACH_EQUAL(ROW, KEY, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = sbrec_ip_multicast_cursor_first_eq(INDEX, KEY); \
             (cursor__.position \
              ? ((ROW) = sbrec_ip_multicast_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next_eq(&cursor__), \
                 true) \
...
#define SBREC_IP_MULTICAST_FOR_EACH_BYINDEX(ROW, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = sbrec_ip_multicast_cursor_first(INDEX); \
             (cursor__.position \
              ? ((ROW) = sbrec_ip_multicast_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next(&cursor__), \
                 true) \
...
void sbrec_ip_multicast_init(struct sbrec_ip_multicast *);
void sbrec_ip_multicast_delete(const struct sbrec_ip_multicast *);
struct sbrec_ip_multicast *sbrec_ip_multicast_insert(struct ovsdb_idl_txn *);
bool sbrec_ip_multicast_is_updated(const struct sbrec_ip_multicast *, enum sbrec_ip_multicast_column_id);

void sbrec_ip_multicast_verify_datapath(const struct sbrec_ip_multicast *);
void sbrec_ip_multicast_verify_enabled(const struct sbrec_ip_multicast *);
void sbrec_ip_multicast_verify_eth_src(const struct sbrec_ip_multicast *);
void sbrec_ip_multicast_verify_idle_timeout(const struct sbrec_ip_multicast *);
void sbrec_ip_multicast_verify_ip4_src(const struct sbrec_ip_multicast *);
void sbrec_ip_multicast_verify_querier(const struct sbrec_ip_multicast *);
void sbrec_ip_multicast_verify_query_interval(const struct sbrec_ip_multicast *);
void sbrec_ip_multicast_verify_query_max_resp(const struct sbrec_ip_multicast *);
void sbrec_ip_multicast_verify_seq_no(const struct sbrec_ip_multicast *);
void sbrec_ip_multicast_verify_table_size(const struct sbrec_ip_multicast *);

const struct ovsdb_datum *sbrec_ip_multicast_get_datapath(const struct sbrec_ip_multicast *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *sbrec_ip_multicast_get_enabled(const struct sbrec_ip_multicast *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *sbrec_ip_multicast_get_eth_src(const struct sbrec_ip_multicast *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *sbrec_ip_multicast_get_idle_timeout(const struct sbrec_ip_multicast *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *sbrec_ip_multicast_get_ip4_src(const struct sbrec_ip_multicast *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *sbrec_ip_multicast_get_querier(const struct sbrec_ip_multicast *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *sbrec_ip_multicast_get_query_interval(const struct sbrec_ip_multicast *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *sbrec_ip_multicast_get_query_max_resp(const struct sbrec_ip_multicast *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *sbrec_ip_multicast_get_seq_no(const struct sbrec_ip_multicast *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *sbrec_ip_multicast_get_table_size(const struct sbrec_ip_multicast *, enum ovsdb_atomic_type key_type);

void sbrec_ip_multicast_set_datapath(const struct sbrec_ip_multicast *, const struct sbrec_datapath_binding *datapath);
void sbrec_ip_multicast_set_enabled(const struct sbrec_ip_multicast *, const bool *enabled, size_t n_enabled);
void sbrec_ip_multicast_set_eth_src(const struct sbrec_ip_multicast *, const char *eth_src);
void sbrec_ip_multicast_set_idle_timeout(const struct sbrec_ip_multicast *, const int64_t *idle_timeout, size_t n_idle_timeout);
void sbrec_ip_multicast_set_ip4_src(const struct sbrec_ip_multicast *, const char *ip4_src);
void sbrec_ip_multicast_set_querier(const struct sbrec_ip_multicast *, const bool *querier, size_t n_querier);
void sbrec_ip_multicast_set_query_interval(const struct sbrec_ip_multicast *, const int64_t *query_interval, size_t n_query_interval);
void sbrec_ip_multicast_set_query_max_resp(const struct sbrec_ip_multicast *, const int64_t *query_max_resp, size_t n_query_max_resp);
void sbrec_ip_multicast_set_seq_no(const struct sbrec_ip_multicast *, int64_t seq_no);
void sbrec_ip_multicast_set_table_size(const struct sbrec_ip_multicast *, const int64_t *table_size, size_t n_table_size);

void sbrec_ip_multicast_add_clause_datapath(struct ovsdb_idl_condition *, enum ovsdb_function function, const struct uuid *datapath);
void sbrec_ip_multicast_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void sbrec_ip_multicast_update_enabled_addvalue(const struct sbrec_ip_multicast *,  bool );
void sbrec_ip_multicast_update_enabled_delvalue(const struct sbrec_ip_multicast *,  bool );
void sbrec_ip_multicast_add_clause_enabled(struct ovsdb_idl_condition *, enum ovsdb_function function, const bool *enabled, size_t n_enabled);
void sbrec_ip_multicast_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void sbrec_ip_multicast_add_clause_eth_src(struct ovsdb_idl_condition *, enum ovsdb_function function, const char *eth_src);
void sbrec_ip_multicast_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void sbrec_ip_multicast_update_idle_timeout_addvalue(const struct sbrec_ip_multicast *,  int64_t );
void sbrec_ip_multicast_update_idle_timeout_delvalue(const struct sbrec_ip_multicast *,  int64_t );
void sbrec_ip_multicast_add_clause_idle_timeout(struct ovsdb_idl_condition *, enum ovsdb_function function, const int64_t *idle_timeout, size_t n_idle_timeout);
void sbrec_ip_multicast_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void sbrec_ip_multicast_add_clause_ip4_src(struct ovsdb_idl_condition *, enum ovsdb_function function, const char *ip4_src);
void sbrec_ip_multicast_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void sbrec_ip_multicast_update_querier_addvalue(const struct sbrec_ip_multicast *,  bool );
void sbrec_ip_multicast_update_querier_delvalue(const struct sbrec_ip_multicast *,  bool );
void sbrec_ip_multicast_add_clause_querier(struct ovsdb_idl_condition *, enum ovsdb_function function, const bool *querier, size_t n_querier);
void sbrec_ip_multicast_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void sbrec_ip_multicast_update_query_interval_addvalue(const struct sbrec_ip_multicast *,  int64_t );
void sbrec_ip_multicast_update_query_interval_delvalue(const struct sbrec_ip_multicast *,  int64_t );
void sbrec_ip_multicast_add_clause_query_interval(struct ovsdb_idl_condition *, enum ovsdb_function function, const int64_t *query_interval, size_t n_query_interval);
void sbrec_ip_multicast_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void sbrec_ip_multicast_update_query_max_resp_addvalue(const struct sbrec_ip_multicast *,  int64_t );
void sbrec_ip_multicast_update_query_max_resp_delvalue(const struct sbrec_ip_multicast *,  int64_t );
void sbrec_ip_multicast_add_clause_query_max_resp(struct ovsdb_idl_condition *, enum ovsdb_function function, const int64_t *query_max_resp, size_t n_query_max_resp);
void sbrec_ip_multicast_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void sbrec_ip_multicast_add_clause_seq_no(struct ovsdb_idl_condition *, enum ovsdb_function function, int64_t seq_no);
void sbrec_ip_multicast_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void sbrec_ip_multicast_update_table_size_addvalue(const struct sbrec_ip_multicast *,  int64_t );
void sbrec_ip_multicast_update_table_size_delvalue(const struct sbrec_ip_multicast *,  int64_t );
void sbrec_ip_multicast_add_clause_table_size(struct ovsdb_idl_condition *, enum ovsdb_function function, const int64_t *table_size, size_t n_table_size);
void sbrec_ip_multicast_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);

struct sbrec_ip_multicast *sbrec_ip_multicast_index_init_row(struct ovsdb_idl_index *);
void sbrec_ip_multicast_index_set_datapath(const struct sbrec_ip_multicast *,
const struct sbrec_datapath_binding *datapath);
void sbrec_ip_multicast_index_set_enabled(const struct sbrec_ip_multicast *,
const bool *enabled, size_t n_enabled);
void sbrec_ip_multicast_index_set_eth_src(const struct sbrec_ip_multicast *,
const char *eth_src);
void sbrec_ip_multicast_index_set_idle_timeout(const struct sbrec_ip_multicast *,
const int64_t *idle_timeout, size_t n_idle_timeout);
void sbrec_ip_multicast_index_set_ip4_src(const struct sbrec_ip_multicast *,
const char *ip4_src);
void sbrec_ip_multicast_index_set_querier(const struct sbrec_ip_multicast *,
const bool *querier, size_t n_querier);
void sbrec_ip_multicast_index_set_query_interval(const struct sbrec_ip_multicast *,
const int64_t *query_interval, size_t n_query_interval);
void sbrec_ip_multicast_index_set_query_max_resp(const struct sbrec_ip_multicast *,
const int64_t *query_max_resp, size_t n_query_max_resp);
void sbrec_ip_multicast_index_set_seq_no(const struct sbrec_ip_multicast *,
int64_t seq_no);
void sbrec_ip_multicast_index_set_table_size(const struct sbrec_ip_multicast *,
const int64_t *table_size, size_t n_table_size);

/* Logical_Flow table. */
...
#define sbrec_port_binding_col_ha_chassis_group (sbrec_port_binding_columns[SBREC_PORT_BINDING_COL_HA_CHASSIS_GROUP])
...
#define sbrec_port_binding_col_virtual_parent (sbrec_port_binding_columns[SBREC_PORT_BINDING_COL_VIRTUAL_PARENT])
...
void sbrec_port_binding_verify_ha_chassis_group(const struct sbrec_port_binding *);
...
void sbrec_port_binding_verify_virtual_parent(const struct sbrec_port_binding *);
...
const struct ovsdb_datum *sbrec_port_binding_get_ha_chassis_group(const struct sbrec_port_binding *, enum ovsdb_atomic_type key_type);
...
const struct ovsdb_datum *sbrec_port_binding_get_virtual_parent(const struct sbrec_port_binding *, enum ovsdb_atomic_type key_type);
...
void sbrec_port_binding_set_ha_chassis_group(const struct sbrec_port_binding *, const struct sbrec_ha_chassis_group *ha_chassis_group);
...
void sbrec_port_binding_set_virtual_parent(const struct sbrec_port_binding *, const char *virtual_parent);
...
void sbrec_port_binding_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void sbrec_port_binding_update_ha_chassis_group_addvalue(const struct sbrec_port_binding *,  const struct sbrec_ha_chassis_group *);
void sbrec_port_binding_update_ha_chassis_group_delvalue(const struct sbrec_port_binding *,  const struct sbrec_ha_chassis_group *);
void sbrec_port_binding_add_clause_ha_chassis_group(struct ovsdb_idl_condition *, enum ovsdb_function function, const struct uuid *ha_chassis_group);
void sbrec_port_binding_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
...
void sbrec_port_binding_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void sbrec_port_binding_update_virtual_parent_addvalue(const struct sbrec_port_binding *,  const char *);
void sbrec_port_binding_update_virtual_parent_delvalue(const struct sbrec_port_binding *,  const char *);
void sbrec_port_binding_add_clause_virtual_parent(struct ovsdb_idl_condition *, enum ovsdb_function function, const char *virtual_parent);
void sbrec_port_binding_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
...
void sbrec_port_binding_index_set_ha_chassis_group(const struct sbrec_port_binding *,
...
const char *type);
void sbrec_port_binding_index_set_virtual_parent(const struct sbrec_port_binding *,
const char *virtual_parent);

/* Port_Group table. */
...
enum sbrec_table_id {
    ...
    SBREC_TABLE_HA_CHASSIS,
    SBREC_TABLE_HA_CHASSIS_GROUP,
    SBREC_TABLE_IGMP_GROUP,
...
#define sbrec_table_controller_event (sbrec_table_classes[SBREC_TABLE_CONTROLLER_EVENT])
...
#define sbrec_table_ha_chassis (sbrec_table_classes[SBREC_TABLE_HA_CHASSIS])
#define sbrec_table_ha_chassis_group (sbrec_table_classes[SBREC_TABLE_HA_CHASSIS_GROUP])
#define sbrec_table_igmp_group (sbrec_table_classes[SBREC_TABLE_IGMP_GROUP])
#define sbrec_table_ip_multicast (sbrec_table_classes[SBREC_TABLE_IP_MULTICAST])
