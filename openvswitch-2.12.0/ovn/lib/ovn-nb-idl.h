...
enum nbrec_ha_chassis_column_id {
    NBREC_HA_CHASSIS_COL_CHASSIS_NAME,
    NBREC_HA_CHASSIS_COL_EXTERNAL_IDS,
    NBREC_HA_CHASSIS_COL_PRIORITY,
    NBREC_HA_CHASSIS_N_COLUMNS
};

#define nbrec_ha_chassis_col_chassis_name (nbrec_ha_chassis_columns[NBREC_HA_CHASSIS_COL_CHASSIS_NAME])
#define nbrec_ha_chassis_col_external_ids (nbrec_ha_chassis_columns[NBREC_HA_CHASSIS_COL_EXTERNAL_IDS])
#define nbrec_ha_chassis_col_priority (nbrec_ha_chassis_columns[NBREC_HA_CHASSIS_COL_PRIORITY])

extern struct ovsdb_idl_column nbrec_ha_chassis_columns[NBREC_HA_CHASSIS_N_COLUMNS];

const struct nbrec_ha_chassis_table *nbrec_ha_chassis_table_get(const struct ovsdb_idl *);
const struct nbrec_ha_chassis *nbrec_ha_chassis_table_first(const struct nbrec_ha_chassis_table *);

#define NBREC_HA_CHASSIS_TABLE_FOR_EACH(ROW, TABLE) \
        for ((ROW) = nbrec_ha_chassis_table_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_ha_chassis_next(ROW))
#define NBREC_HA_CHASSIS_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = nbrec_ha_chassis_table_first(TABLE); \
             (ROW) ? ((NEXT) = nbrec_ha_chassis_next(ROW), 1) : 0; \
             (ROW) = (NEXT))

const struct nbrec_ha_chassis *nbrec_ha_chassis_get_for_uuid(const struct ovsdb_idl *, const struct uuid *);
const struct nbrec_ha_chassis *nbrec_ha_chassis_table_get_for_uuid(const struct nbrec_ha_chassis_table *, const struct uuid *);
const struct nbrec_ha_chassis *nbrec_ha_chassis_first(const struct ovsdb_idl *);
const struct nbrec_ha_chassis *nbrec_ha_chassis_next(const struct nbrec_ha_chassis *);
#define NBREC_HA_CHASSIS_FOR_EACH(ROW, IDL) \
        for ((ROW) = nbrec_ha_chassis_first(IDL); \
             (ROW); \
             (ROW) = nbrec_ha_chassis_next(ROW))
#define NBREC_HA_CHASSIS_FOR_EACH_SAFE(ROW, NEXT, IDL) \
        for ((ROW) = nbrec_ha_chassis_first(IDL); \
             (ROW) ? ((NEXT) = nbrec_ha_chassis_next(ROW), 1) : 0; \
             (ROW) = (NEXT))

unsigned int nbrec_ha_chassis_get_seqno(const struct ovsdb_idl *);
unsigned int nbrec_ha_chassis_row_get_seqno(const struct nbrec_ha_chassis *row, enum ovsdb_idl_change change);
const struct nbrec_ha_chassis *nbrec_ha_chassis_track_get_first(const struct ovsdb_idl *);
const struct nbrec_ha_chassis *nbrec_ha_chassis_track_get_next(const struct nbrec_ha_chassis *);
#define NBREC_HA_CHASSIS_FOR_EACH_TRACKED(ROW, IDL) \
        for ((ROW) = nbrec_ha_chassis_track_get_first(IDL); \
             (ROW); \
             (ROW) = nbrec_ha_chassis_track_get_next(ROW))

const struct nbrec_ha_chassis *nbrec_ha_chassis_table_track_get_first(const struct nbrec_ha_chassis_table *);
#define NBREC_HA_CHASSIS_TABLE_FOR_EACH_TRACKED(ROW, TABLE) \
        for ((ROW) = nbrec_ha_chassis_table_track_get_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_ha_chassis_track_get_next(ROW))


/* Returns true if 'row' was inserted since the last change tracking reset. */
static inline bool nbrec_ha_chassis_is_new(const struct nbrec_ha_chassis *row)
{
    return nbrec_ha_chassis_row_get_seqno(row, OVSDB_IDL_CHANGE_MODIFY) == 0;
}

/* Returns true if 'row' was deleted since the last change tracking reset. */
static inline bool nbrec_ha_chassis_is_deleted(const struct nbrec_ha_chassis *row)
{
    return nbrec_ha_chassis_row_get_seqno(row, OVSDB_IDL_CHANGE_DELETE) > 0;
}

void nbrec_ha_chassis_index_destroy_row(const struct nbrec_ha_chassis *);

struct nbrec_ha_chassis *nbrec_ha_chassis_index_find(struct ovsdb_idl_index *, const struct nbrec_ha_chassis *);

int nbrec_ha_chassis_index_compare(
    struct ovsdb_idl_index *, 
    const struct nbrec_ha_chassis *, 
    const struct nbrec_ha_chassis *);
struct ovsdb_idl_cursor nbrec_ha_chassis_cursor_first(struct ovsdb_idl_index *);
struct ovsdb_idl_cursor nbrec_ha_chassis_cursor_first_eq(
    struct ovsdb_idl_index *, const struct nbrec_ha_chassis *);
struct ovsdb_idl_cursor nbrec_ha_chassis_cursor_first_ge(
    struct ovsdb_idl_index *, const struct nbrec_ha_chassis *);

struct nbrec_ha_chassis *nbrec_ha_chassis_cursor_data(struct ovsdb_idl_cursor *);

#define NBREC_HA_CHASSIS_FOR_EACH_RANGE(ROW, FROM, TO, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_ha_chassis_cursor_first_ge(INDEX, FROM); \
             (cursor__.position \
              && ((ROW) = nbrec_ha_chassis_cursor_data(&cursor__), \
                  !(TO) || nbrec_ha_chassis_index_compare(INDEX, ROW, TO) <= 0)); \
             ovsdb_idl_cursor_next(&cursor__))
#define NBREC_HA_CHASSIS_FOR_EACH_EQUAL(ROW, KEY, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_ha_chassis_cursor_first_eq(INDEX, KEY); \
             (cursor__.position \
              ? ((ROW) = nbrec_ha_chassis_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next_eq(&cursor__), \
                 true) \
...
#define NBREC_HA_CHASSIS_FOR_EACH_BYINDEX(ROW, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_ha_chassis_cursor_first(INDEX); \
             (cursor__.position \
              ? ((ROW) = nbrec_ha_chassis_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next(&cursor__), \
                 true) \
...
void nbrec_ha_chassis_init(struct nbrec_ha_chassis *);
void nbrec_ha_chassis_delete(const struct nbrec_ha_chassis *);
struct nbrec_ha_chassis *nbrec_ha_chassis_insert(struct ovsdb_idl_txn *);
bool nbrec_ha_chassis_is_updated(const struct nbrec_ha_chassis *, enum nbrec_ha_chassis_column_id);

void nbrec_ha_chassis_verify_chassis_name(const struct nbrec_ha_chassis *);
void nbrec_ha_chassis_verify_external_ids(const struct nbrec_ha_chassis *);
void nbrec_ha_chassis_verify_priority(const struct nbrec_ha_chassis *);

const struct ovsdb_datum *nbrec_ha_chassis_get_chassis_name(const struct nbrec_ha_chassis *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *nbrec_ha_chassis_get_external_ids(const struct nbrec_ha_chassis *, enum ovsdb_atomic_type key_type, enum ovsdb_atomic_type value_type);
const struct ovsdb_datum *nbrec_ha_chassis_get_priority(const struct nbrec_ha_chassis *, enum ovsdb_atomic_type key_type);

void nbrec_ha_chassis_set_chassis_name(const struct nbrec_ha_chassis *, const char *chassis_name);
void nbrec_ha_chassis_set_external_ids(const struct nbrec_ha_chassis *, const struct smap *);
void nbrec_ha_chassis_set_priority(const struct nbrec_ha_chassis *, int64_t priority);

void nbrec_ha_chassis_add_clause_chassis_name(struct ovsdb_idl_condition *, enum ovsdb_function function, const char *chassis_name);
void nbrec_ha_chassis_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void nbrec_ha_chassis_update_external_ids_setkey(const struct nbrec_ha_chassis *,  const char *, const char *);
void nbrec_ha_chassis_update_external_ids_delkey(const struct nbrec_ha_chassis *,  const char *);
void nbrec_ha_chassis_add_clause_external_ids(struct ovsdb_idl_condition *, enum ovsdb_function function, const struct smap *);
void nbrec_ha_chassis_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void nbrec_ha_chassis_add_clause_priority(struct ovsdb_idl_condition *, enum ovsdb_function function, int64_t priority);
void nbrec_ha_chassis_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);

struct nbrec_ha_chassis *nbrec_ha_chassis_index_init_row(struct ovsdb_idl_index *);
void nbrec_ha_chassis_index_set_chassis_name(const struct nbrec_ha_chassis *,
const char *chassis_name);
void nbrec_ha_chassis_index_set_external_ids(const struct nbrec_ha_chassis *,
const struct smap *);
void nbrec_ha_chassis_index_set_priority(const struct nbrec_ha_chassis *,
int64_t priority);

...
struct nbrec_ha_chassis_group {
	...
	/* ha_chassis column. */
	struct nbrec_ha_chassis **ha_chassis;
	size_t n_ha_chassis;
...
enum nbrec_ha_chassis_group_column_id {
    NBREC_HA_CHASSIS_GROUP_COL_EXTERNAL_IDS,
    NBREC_HA_CHASSIS_GROUP_COL_HA_CHASSIS,
    NBREC_HA_CHASSIS_GROUP_COL_NAME,
    NBREC_HA_CHASSIS_GROUP_N_COLUMNS
};

#define nbrec_ha_chassis_group_col_external_ids (nbrec_ha_chassis_group_columns[NBREC_HA_CHASSIS_GROUP_COL_EXTERNAL_IDS])
#define nbrec_ha_chassis_group_col_ha_chassis (nbrec_ha_chassis_group_columns[NBREC_HA_CHASSIS_GROUP_COL_HA_CHASSIS])
#define nbrec_ha_chassis_group_col_name (nbrec_ha_chassis_group_columns[NBREC_HA_CHASSIS_GROUP_COL_NAME])

extern struct ovsdb_idl_column nbrec_ha_chassis_group_columns[NBREC_HA_CHASSIS_GROUP_N_COLUMNS];

const struct nbrec_ha_chassis_group_table *nbrec_ha_chassis_group_table_get(const struct ovsdb_idl *);
const struct nbrec_ha_chassis_group *nbrec_ha_chassis_group_table_first(const struct nbrec_ha_chassis_group_table *);

#define NBREC_HA_CHASSIS_GROUP_TABLE_FOR_EACH(ROW, TABLE) \
        for ((ROW) = nbrec_ha_chassis_group_table_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_ha_chassis_group_next(ROW))
#define NBREC_HA_CHASSIS_GROUP_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = nbrec_ha_chassis_group_table_first(TABLE); \
             (ROW) ? ((NEXT) = nbrec_ha_chassis_group_next(ROW), 1) : 0; \
             (ROW) = (NEXT))

const struct nbrec_ha_chassis_group *nbrec_ha_chassis_group_get_for_uuid(const struct ovsdb_idl *, const struct uuid *);
const struct nbrec_ha_chassis_group *nbrec_ha_chassis_group_table_get_for_uuid(const struct nbrec_ha_chassis_group_table *, const struct uuid *);
const struct nbrec_ha_chassis_group *nbrec_ha_chassis_group_first(const struct ovsdb_idl *);
const struct nbrec_ha_chassis_group *nbrec_ha_chassis_group_next(const struct nbrec_ha_chassis_group *);
#define NBREC_HA_CHASSIS_GROUP_FOR_EACH(ROW, IDL) \
        for ((ROW) = nbrec_ha_chassis_group_first(IDL); \
             (ROW); \
             (ROW) = nbrec_ha_chassis_group_next(ROW))
#define NBREC_HA_CHASSIS_GROUP_FOR_EACH_SAFE(ROW, NEXT, IDL) \
        for ((ROW) = nbrec_ha_chassis_group_first(IDL); \
             (ROW) ? ((NEXT) = nbrec_ha_chassis_group_next(ROW), 1) : 0; \
             (ROW) = (NEXT))

unsigned int nbrec_ha_chassis_group_get_seqno(const struct ovsdb_idl *);
unsigned int nbrec_ha_chassis_group_row_get_seqno(const struct nbrec_ha_chassis_group *row, enum ovsdb_idl_change change);
const struct nbrec_ha_chassis_group *nbrec_ha_chassis_group_track_get_first(const struct ovsdb_idl *);
const struct nbrec_ha_chassis_group *nbrec_ha_chassis_group_track_get_next(const struct nbrec_ha_chassis_group *);
#define NBREC_HA_CHASSIS_GROUP_FOR_EACH_TRACKED(ROW, IDL) \
        for ((ROW) = nbrec_ha_chassis_group_track_get_first(IDL); \
             (ROW); \
             (ROW) = nbrec_ha_chassis_group_track_get_next(ROW))

const struct nbrec_ha_chassis_group *nbrec_ha_chassis_group_table_track_get_first(const struct nbrec_ha_chassis_group_table *);
#define NBREC_HA_CHASSIS_GROUP_TABLE_FOR_EACH_TRACKED(ROW, TABLE) \
        for ((ROW) = nbrec_ha_chassis_group_table_track_get_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_ha_chassis_group_track_get_next(ROW))


/* Returns true if 'row' was inserted since the last change tracking reset. */
static inline bool nbrec_ha_chassis_group_is_new(const struct nbrec_ha_chassis_group *row)
{
    return nbrec_ha_chassis_group_row_get_seqno(row, OVSDB_IDL_CHANGE_MODIFY) == 0;
}

/* Returns true if 'row' was deleted since the last change tracking reset. */
static inline bool nbrec_ha_chassis_group_is_deleted(const struct nbrec_ha_chassis_group *row)
{
    return nbrec_ha_chassis_group_row_get_seqno(row, OVSDB_IDL_CHANGE_DELETE) > 0;
}

void nbrec_ha_chassis_group_index_destroy_row(const struct nbrec_ha_chassis_group *);

struct nbrec_ha_chassis_group *nbrec_ha_chassis_group_index_find(struct ovsdb_idl_index *, const struct nbrec_ha_chassis_group *);

int nbrec_ha_chassis_group_index_compare(
    struct ovsdb_idl_index *, 
    const struct nbrec_ha_chassis_group *, 
    const struct nbrec_ha_chassis_group *);
struct ovsdb_idl_cursor nbrec_ha_chassis_group_cursor_first(struct ovsdb_idl_index *);
struct ovsdb_idl_cursor nbrec_ha_chassis_group_cursor_first_eq(
    struct ovsdb_idl_index *, const struct nbrec_ha_chassis_group *);
struct ovsdb_idl_cursor nbrec_ha_chassis_group_cursor_first_ge(
    struct ovsdb_idl_index *, const struct nbrec_ha_chassis_group *);

struct nbrec_ha_chassis_group *nbrec_ha_chassis_group_cursor_data(struct ovsdb_idl_cursor *);

#define NBREC_HA_CHASSIS_GROUP_FOR_EACH_RANGE(ROW, FROM, TO, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_ha_chassis_group_cursor_first_ge(INDEX, FROM); \
             (cursor__.position \
              && ((ROW) = nbrec_ha_chassis_group_cursor_data(&cursor__), \
                  !(TO) || nbrec_ha_chassis_group_index_compare(INDEX, ROW, TO) <= 0)); \
             ovsdb_idl_cursor_next(&cursor__))
#define NBREC_HA_CHASSIS_GROUP_FOR_EACH_EQUAL(ROW, KEY, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_ha_chassis_group_cursor_first_eq(INDEX, KEY); \
             (cursor__.position \
              ? ((ROW) = nbrec_ha_chassis_group_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next_eq(&cursor__), \
                 true) \
...
#define NBREC_HA_CHASSIS_GROUP_FOR_EACH_BYINDEX(ROW, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_ha_chassis_group_cursor_first(INDEX); \
             (cursor__.position \
              ? ((ROW) = nbrec_ha_chassis_group_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next(&cursor__), \
                 true) \
...
void nbrec_ha_chassis_group_init(struct nbrec_ha_chassis_group *);
void nbrec_ha_chassis_group_delete(const struct nbrec_ha_chassis_group *);
struct nbrec_ha_chassis_group *nbrec_ha_chassis_group_insert(struct ovsdb_idl_txn *);
bool nbrec_ha_chassis_group_is_updated(const struct nbrec_ha_chassis_group *, enum nbrec_ha_chassis_group_column_id);

void nbrec_ha_chassis_group_verify_external_ids(const struct nbrec_ha_chassis_group *);
void nbrec_ha_chassis_group_verify_ha_chassis(const struct nbrec_ha_chassis_group *);
void nbrec_ha_chassis_group_verify_name(const struct nbrec_ha_chassis_group *);

const struct ovsdb_datum *nbrec_ha_chassis_group_get_external_ids(const struct nbrec_ha_chassis_group *, enum ovsdb_atomic_type key_type, enum ovsdb_atomic_type value_type);
const struct ovsdb_datum *nbrec_ha_chassis_group_get_ha_chassis(const struct nbrec_ha_chassis_group *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *nbrec_ha_chassis_group_get_name(const struct nbrec_ha_chassis_group *, enum ovsdb_atomic_type key_type);

void nbrec_ha_chassis_group_set_external_ids(const struct nbrec_ha_chassis_group *, const struct smap *);
void nbrec_ha_chassis_group_set_ha_chassis(const struct nbrec_ha_chassis_group *, struct nbrec_ha_chassis **ha_chassis, size_t n_ha_chassis);
void nbrec_ha_chassis_group_set_name(const struct nbrec_ha_chassis_group *, const char *name);

void nbrec_ha_chassis_group_update_external_ids_setkey(const struct nbrec_ha_chassis_group *,  const char *, const char *);
void nbrec_ha_chassis_group_update_external_ids_delkey(const struct nbrec_ha_chassis_group *,  const char *);
void nbrec_ha_chassis_group_add_clause_external_ids(struct ovsdb_idl_condition *, enum ovsdb_function function, const struct smap *);
void nbrec_ha_chassis_group_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void nbrec_ha_chassis_group_update_ha_chassis_addvalue(const struct nbrec_ha_chassis_group *,  const struct nbrec_ha_chassis *);
void nbrec_ha_chassis_group_update_ha_chassis_delvalue(const struct nbrec_ha_chassis_group *,  const struct nbrec_ha_chassis *);
void nbrec_ha_chassis_group_add_clause_ha_chassis(struct ovsdb_idl_condition *, enum ovsdb_function function, struct uuid **ha_chassis, size_t n_ha_chassis);
void nbrec_ha_chassis_group_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void nbrec_ha_chassis_group_add_clause_name(struct ovsdb_idl_condition *, enum ovsdb_function function, const char *name);
void nbrec_ha_chassis_group_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);

struct nbrec_ha_chassis_group *nbrec_ha_chassis_group_index_init_row(struct ovsdb_idl_index *);
void nbrec_ha_chassis_group_index_set_external_ids(const struct nbrec_ha_chassis_group *,
const struct smap *);
void nbrec_ha_chassis_group_index_set_ha_chassis(const struct nbrec_ha_chassis_group *,
struct nbrec_ha_chassis **ha_chassis, size_t n_ha_chassis);
void nbrec_ha_chassis_group_index_set_name(const struct nbrec_ha_chassis_group *,
const char *name);

/* Load_Balancer table. */
...
struct nbrec_logical_router {
	...
	/* policies column. */
	struct nbrec_logical_router_policy **policies;
	size_t n_policies;
...
#define nbrec_logical_router_col_policies (nbrec_logical_router_columns[NBREC_LOGICAL_ROUTER_COL_POLICIES])
...
void nbrec_logical_router_verify_policies(const struct nbrec_logical_router *);
...
const struct ovsdb_datum *nbrec_logical_router_get_policies(const struct nbrec_logical_router *, enum ovsdb_atomic_type key_type);
...
void nbrec_logical_router_set_policies(const struct nbrec_logical_router *, struct nbrec_logical_router_policy **policies, size_t n_policies);
...
void nbrec_logical_router_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void nbrec_logical_router_update_policies_addvalue(const struct nbrec_logical_router *,  const struct nbrec_logical_router_policy *);
void nbrec_logical_router_update_policies_delvalue(const struct nbrec_logical_router *,  const struct nbrec_logical_router_policy *);
void nbrec_logical_router_add_clause_policies(struct ovsdb_idl_condition *, enum ovsdb_function function, struct uuid **policies, size_t n_policies);
void nbrec_logical_router_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
...
const struct smap *);
void nbrec_logical_router_index_set_policies(const struct nbrec_logical_router *,
struct nbrec_logical_router_policy **policies, size_t n_policies);
...
struct nbrec_logical_router_policy {
	struct ovsdb_idl_row header_;
...
enum nbrec_logical_router_policy_column_id {
    NBREC_LOGICAL_ROUTER_POLICY_COL_ACTION,
    NBREC_LOGICAL_ROUTER_POLICY_COL_MATCH,
    NBREC_LOGICAL_ROUTER_POLICY_COL_NEXTHOP,
    NBREC_LOGICAL_ROUTER_POLICY_COL_PRIORITY,
    NBREC_LOGICAL_ROUTER_POLICY_N_COLUMNS
};

#define nbrec_logical_router_policy_col_action (nbrec_logical_router_policy_columns[NBREC_LOGICAL_ROUTER_POLICY_COL_ACTION])
#define nbrec_logical_router_policy_col_match (nbrec_logical_router_policy_columns[NBREC_LOGICAL_ROUTER_POLICY_COL_MATCH])
#define nbrec_logical_router_policy_col_nexthop (nbrec_logical_router_policy_columns[NBREC_LOGICAL_ROUTER_POLICY_COL_NEXTHOP])
#define nbrec_logical_router_policy_col_priority (nbrec_logical_router_policy_columns[NBREC_LOGICAL_ROUTER_POLICY_COL_PRIORITY])

extern struct ovsdb_idl_column nbrec_logical_router_policy_columns[NBREC_LOGICAL_ROUTER_POLICY_N_COLUMNS];

const struct nbrec_logical_router_policy_table *nbrec_logical_router_policy_table_get(const struct ovsdb_idl *);
const struct nbrec_logical_router_policy *nbrec_logical_router_policy_table_first(const struct nbrec_logical_router_policy_table *);

#define NBREC_LOGICAL_ROUTER_POLICY_TABLE_FOR_EACH(ROW, TABLE) \
        for ((ROW) = nbrec_logical_router_policy_table_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_logical_router_policy_next(ROW))
#define NBREC_LOGICAL_ROUTER_POLICY_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = nbrec_logical_router_policy_table_first(TABLE); \
             (ROW) ? ((NEXT) = nbrec_logical_router_policy_next(ROW), 1) : 0; \
             (ROW) = (NEXT))

const struct nbrec_logical_router_policy *nbrec_logical_router_policy_get_for_uuid(const struct ovsdb_idl *, const struct uuid *);
const struct nbrec_logical_router_policy *nbrec_logical_router_policy_table_get_for_uuid(const struct nbrec_logical_router_policy_table *, const struct uuid *);
const struct nbrec_logical_router_policy *nbrec_logical_router_policy_first(const struct ovsdb_idl *);
const struct nbrec_logical_router_policy *nbrec_logical_router_policy_next(const struct nbrec_logical_router_policy *);
#define NBREC_LOGICAL_ROUTER_POLICY_FOR_EACH(ROW, IDL) \
        for ((ROW) = nbrec_logical_router_policy_first(IDL); \
             (ROW); \
             (ROW) = nbrec_logical_router_policy_next(ROW))
#define NBREC_LOGICAL_ROUTER_POLICY_FOR_EACH_SAFE(ROW, NEXT, IDL) \
        for ((ROW) = nbrec_logical_router_policy_first(IDL); \
             (ROW) ? ((NEXT) = nbrec_logical_router_policy_next(ROW), 1) : 0; \
             (ROW) = (NEXT))

unsigned int nbrec_logical_router_policy_get_seqno(const struct ovsdb_idl *);
unsigned int nbrec_logical_router_policy_row_get_seqno(const struct nbrec_logical_router_policy *row, enum ovsdb_idl_change change);
const struct nbrec_logical_router_policy *nbrec_logical_router_policy_track_get_first(const struct ovsdb_idl *);
const struct nbrec_logical_router_policy *nbrec_logical_router_policy_track_get_next(const struct nbrec_logical_router_policy *);
#define NBREC_LOGICAL_ROUTER_POLICY_FOR_EACH_TRACKED(ROW, IDL) \
        for ((ROW) = nbrec_logical_router_policy_track_get_first(IDL); \
             (ROW); \
             (ROW) = nbrec_logical_router_policy_track_get_next(ROW))

const struct nbrec_logical_router_policy *nbrec_logical_router_policy_table_track_get_first(const struct nbrec_logical_router_policy_table *);
#define NBREC_LOGICAL_ROUTER_POLICY_TABLE_FOR_EACH_TRACKED(ROW, TABLE) \
        for ((ROW) = nbrec_logical_router_policy_table_track_get_first(TABLE); \
             (ROW); \
             (ROW) = nbrec_logical_router_policy_track_get_next(ROW))


/* Returns true if 'row' was inserted since the last change tracking reset. */
static inline bool nbrec_logical_router_policy_is_new(const struct nbrec_logical_router_policy *row)
{
    return nbrec_logical_router_policy_row_get_seqno(row, OVSDB_IDL_CHANGE_MODIFY) == 0;
}

/* Returns true if 'row' was deleted since the last change tracking reset. */
static inline bool nbrec_logical_router_policy_is_deleted(const struct nbrec_logical_router_policy *row)
{
    return nbrec_logical_router_policy_row_get_seqno(row, OVSDB_IDL_CHANGE_DELETE) > 0;
}

void nbrec_logical_router_policy_index_destroy_row(const struct nbrec_logical_router_policy *);

struct nbrec_logical_router_policy *nbrec_logical_router_policy_index_find(struct ovsdb_idl_index *, const struct nbrec_logical_router_policy *);

int nbrec_logical_router_policy_index_compare(
    struct ovsdb_idl_index *, 
    const struct nbrec_logical_router_policy *, 
    const struct nbrec_logical_router_policy *);
struct ovsdb_idl_cursor nbrec_logical_router_policy_cursor_first(struct ovsdb_idl_index *);
struct ovsdb_idl_cursor nbrec_logical_router_policy_cursor_first_eq(
    struct ovsdb_idl_index *, const struct nbrec_logical_router_policy *);
struct ovsdb_idl_cursor nbrec_logical_router_policy_cursor_first_ge(
    struct ovsdb_idl_index *, const struct nbrec_logical_router_policy *);

struct nbrec_logical_router_policy *nbrec_logical_router_policy_cursor_data(struct ovsdb_idl_cursor *);

#define NBREC_LOGICAL_ROUTER_POLICY_FOR_EACH_RANGE(ROW, FROM, TO, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_logical_router_policy_cursor_first_ge(INDEX, FROM); \
             (cursor__.position \
              && ((ROW) = nbrec_logical_router_policy_cursor_data(&cursor__), \
                  !(TO) || nbrec_logical_router_policy_index_compare(INDEX, ROW, TO) <= 0)); \
             ovsdb_idl_cursor_next(&cursor__))
#define NBREC_LOGICAL_ROUTER_POLICY_FOR_EACH_EQUAL(ROW, KEY, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_logical_router_policy_cursor_first_eq(INDEX, KEY); \
             (cursor__.position \
              ? ((ROW) = nbrec_logical_router_policy_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next_eq(&cursor__), \
                 true) \
...
#define NBREC_LOGICAL_ROUTER_POLICY_FOR_EACH_BYINDEX(ROW, INDEX) \
        for (struct ovsdb_idl_cursor cursor__ = nbrec_logical_router_policy_cursor_first(INDEX); \
             (cursor__.position \
              ? ((ROW) = nbrec_logical_router_policy_cursor_data(&cursor__), \
                 ovsdb_idl_cursor_next(&cursor__), \
                 true) \
...
void nbrec_logical_router_policy_init(struct nbrec_logical_router_policy *);
void nbrec_logical_router_policy_delete(const struct nbrec_logical_router_policy *);
struct nbrec_logical_router_policy *nbrec_logical_router_policy_insert(struct ovsdb_idl_txn *);
bool nbrec_logical_router_policy_is_updated(const struct nbrec_logical_router_policy *, enum nbrec_logical_router_policy_column_id);

void nbrec_logical_router_policy_verify_action(const struct nbrec_logical_router_policy *);
void nbrec_logical_router_policy_verify_match(const struct nbrec_logical_router_policy *);
void nbrec_logical_router_policy_verify_nexthop(const struct nbrec_logical_router_policy *);
void nbrec_logical_router_policy_verify_priority(const struct nbrec_logical_router_policy *);

const struct ovsdb_datum *nbrec_logical_router_policy_get_action(const struct nbrec_logical_router_policy *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *nbrec_logical_router_policy_get_match(const struct nbrec_logical_router_policy *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *nbrec_logical_router_policy_get_nexthop(const struct nbrec_logical_router_policy *, enum ovsdb_atomic_type key_type);
const struct ovsdb_datum *nbrec_logical_router_policy_get_priority(const struct nbrec_logical_router_policy *, enum ovsdb_atomic_type key_type);

void nbrec_logical_router_policy_set_action(const struct nbrec_logical_router_policy *, const char *action);
void nbrec_logical_router_policy_set_match(const struct nbrec_logical_router_policy *, const char *match);
void nbrec_logical_router_policy_set_nexthop(const struct nbrec_logical_router_policy *, const char *nexthop);
void nbrec_logical_router_policy_set_priority(const struct nbrec_logical_router_policy *, int64_t priority);

void nbrec_logical_router_policy_add_clause_action(struct ovsdb_idl_condition *, enum ovsdb_function function, const char *action);
void nbrec_logical_router_policy_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void nbrec_logical_router_policy_add_clause_match(struct ovsdb_idl_condition *, enum ovsdb_function function, const char *match);
void nbrec_logical_router_policy_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void nbrec_logical_router_policy_update_nexthop_addvalue(const struct nbrec_logical_router_policy *,  const char *);
void nbrec_logical_router_policy_update_nexthop_delvalue(const struct nbrec_logical_router_policy *,  const char *);
void nbrec_logical_router_policy_add_clause_nexthop(struct ovsdb_idl_condition *, enum ovsdb_function function, const char *nexthop);
void nbrec_logical_router_policy_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void nbrec_logical_router_policy_add_clause_priority(struct ovsdb_idl_condition *, enum ovsdb_function function, int64_t priority);
void nbrec_logical_router_policy_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);

struct nbrec_logical_router_policy *nbrec_logical_router_policy_index_init_row(struct ovsdb_idl_index *);
void nbrec_logical_router_policy_index_set_action(const struct nbrec_logical_router_policy *,
const char *action);
void nbrec_logical_router_policy_index_set_match(const struct nbrec_logical_router_policy *,
const char *match);
void nbrec_logical_router_policy_index_set_nexthop(const struct nbrec_logical_router_policy *,
const char *nexthop);
void nbrec_logical_router_policy_index_set_priority(const struct nbrec_logical_router_policy *,
int64_t priority);

...
struct nbrec_logical_router_port {
	...
	struct nbrec_ha_chassis_group *ha_chassis_group;
...
#define nbrec_logical_router_port_col_ha_chassis_group (nbrec_logical_router_port_columns[NBREC_LOGICAL_ROUTER_PORT_COL_HA_CHASSIS_GROUP])
...
void nbrec_logical_router_port_verify_ha_chassis_group(const struct nbrec_logical_router_port *);
...
const struct ovsdb_datum *nbrec_logical_router_port_get_ha_chassis_group(const struct nbrec_logical_router_port *, enum ovsdb_atomic_type key_type);
...
void nbrec_logical_router_port_set_ha_chassis_group(const struct nbrec_logical_router_port *, const struct nbrec_ha_chassis_group *ha_chassis_group);
...
void nbrec_logical_router_port_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void nbrec_logical_router_port_update_ha_chassis_group_addvalue(const struct nbrec_logical_router_port *,  const struct nbrec_ha_chassis_group *);
void nbrec_logical_router_port_update_ha_chassis_group_delvalue(const struct nbrec_logical_router_port *,  const struct nbrec_ha_chassis_group *);
void nbrec_logical_router_port_add_clause_ha_chassis_group(struct ovsdb_idl_condition *, enum ovsdb_function function, const struct uuid *ha_chassis_group);
void nbrec_logical_router_port_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
...
void nbrec_logical_router_port_index_set_ha_chassis_group(const struct nbrec_logical_router_port *,
const struct nbrec_ha_chassis_group *ha_chassis_group);
...
enum nbrec_logical_switch_port_column_id {
    ...
    NBREC_LOGICAL_SWITCH_PORT_COL_HA_CHASSIS_GROUP,
...
#define nbrec_logical_switch_port_col_ha_chassis_group (nbrec_logical_switch_port_columns[NBREC_LOGICAL_SWITCH_PORT_COL_HA_CHASSIS_GROUP])
...
void nbrec_logical_switch_port_verify_ha_chassis_group(const struct nbrec_logical_switch_port *);
...
const struct ovsdb_datum *nbrec_logical_switch_port_get_ha_chassis_group(const struct nbrec_logical_switch_port *, enum ovsdb_atomic_type key_type);
...
void nbrec_logical_switch_port_set_ha_chassis_group(const struct nbrec_logical_switch_port *, const struct nbrec_ha_chassis_group *ha_chassis_group);
...
void nbrec_logical_switch_port_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
void nbrec_logical_switch_port_update_ha_chassis_group_addvalue(const struct nbrec_logical_switch_port *,  const struct nbrec_ha_chassis_group *);
void nbrec_logical_switch_port_update_ha_chassis_group_delvalue(const struct nbrec_logical_switch_port *,  const struct nbrec_ha_chassis_group *);
void nbrec_logical_switch_port_add_clause_ha_chassis_group(struct ovsdb_idl_condition *, enum ovsdb_function function, const struct uuid *ha_chassis_group);
void nbrec_logical_switch_port_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
...
const struct smap *);
void nbrec_logical_switch_port_index_set_ha_chassis_group(const struct nbrec_logical_switch_port *,
const struct nbrec_ha_chassis_group *ha_chassis_group);
...
enum nbrec_table_id {
    ...
    NBREC_TABLE_HA_CHASSIS,
    NBREC_TABLE_HA_CHASSIS_GROUP,
    NBREC_TABLE_LOAD_BALANCER,
    NBREC_TABLE_LOGICAL_ROUTER,
    NBREC_TABLE_LOGICAL_ROUTER_POLICY,
...
#define nbrec_table_ha_chassis (nbrec_table_classes[NBREC_TABLE_HA_CHASSIS])
#define nbrec_table_ha_chassis_group (nbrec_table_classes[NBREC_TABLE_HA_CHASSIS_GROUP])
...
#define nbrec_table_logical_router_policy (nbrec_table_classes[NBREC_TABLE_LOGICAL_ROUTER_POLICY])
