               ...
               bool md_is_valid, odp_port_t port_no)
{
    ...
    //struct rte_mbuf *mbuf = &(packet->mbuf);
    /**
     * @brief 对于发送端，每次来一个包，都会在这里进行判断属于哪一条流，
     * 在要写入rb的时间段，会写入rb，在rb里面写入数据库
     *
     * 对于接收端，每次过来一个包，都会获得五元组，并写入rb。并且会写入含有很多
     * 流的结构体中，在定时器响的时候，将结构体中的包数写入到rb中。在rb读取的时候，
     * 分两部分读取，如果有包数，则放入DB中，没有的话，则放入sketch中。
     */
    if (likely(mbuf != NULL))
    {
        ...
        if (eth_hdr->ether_type == 0x4788)
        {
            ...
            ip_hdr = rte_pktmbuf_mtod_offset(mbuf, struct ipv4_hdr *, 14 + 4);
            if (ip_hdr->next_proto_id == 0x11)
            {
                ...
                udp_hdr = rte_pktmbuf_mtod_offset(mbuf, struct udp_hdr *, 14 + 20 + 4);
        ...
        if(likely(mbuf != NULL)){
            ...
            if (write_flag == 1) {
                tuple.loss=loss;
                tuple.loss_sum=loss_sum;
                write_flag=0;
                VLOG_INFO("START CHANGE LOSS");
            } else {
                tuple.loss=-1;
                tuple.loss_sum=-1;
            }
        ...
        if(likely(mbuf != NULL)){
            ...
            if(eth_hdr->ether_type == 0x4788){
                struct ipv4_hdr *ip_hdr;
                ip_hdr = rte_pktmbuf_mtod_offset(mbuf, struct ipv4_hdr *, 18);
