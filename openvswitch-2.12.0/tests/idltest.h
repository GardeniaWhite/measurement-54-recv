...
#define IDLTEST_LINK1_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = idltest_link1_table_first(TABLE); \
...
#define IDLTEST_LINK2_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = idltest_link2_table_first(TABLE); \
...
#define IDLTEST_SIMPLE_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = idltest_simple_table_first(TABLE); \
...
#define IDLTEST_SIMPLE2_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = idltest_simple2_table_first(TABLE); \
...
#define IDLTEST_SIMPLE3_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = idltest_simple3_table_first(TABLE); \
...
#define IDLTEST_SIMPLE4_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = idltest_simple4_table_first(TABLE); \
...
#define IDLTEST_SINGLETON_TABLE_FOR_EACH_SAFE(ROW, NEXT, TABLE) \
        for ((ROW) = idltest_singleton_table_first(TABLE); \
...
#define IDLTEST_SINGLETON_FOR_EACH(ROW, IDL) \
        for ((ROW) = idltest_singleton_first(IDL); \
             (ROW); \
             (ROW) = idltest_singleton_next(ROW))
#define IDLTEST_SINGLETON_FOR_EACH_SAFE(ROW, NEXT, IDL) \
        for ((ROW) = idltest_singleton_first(IDL); \
             (ROW) ? ((NEXT) = idltest_singleton_next(ROW), 1) : 0; \
             (ROW) = (NEXT))
...
{
    return idltest_singleton_row_get_seqno(row, OVSDB_IDL_CHANGE_MODIFY) == 0;
}
...
void idltest_singleton_set_condition(struct ovsdb_idl *, struct ovsdb_idl_condition *);
