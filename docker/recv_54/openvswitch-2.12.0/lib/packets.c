...
int count_1 = 0, count_2 = 0, rcv_color = 0, dp_conut_pkt = 0, my_count = 0, myflag = -1, test_count = 0, pkt_count=0;
int loss_num=0;
int write_flag;
...
void pop_mpls(struct dp_packet *packet, ovs_be16 ethtype)
{
	...
	last_time=diff_time;
        // VLOG_INFO("origin_time=%llx,RECV LABEL=%x,RCV color=%d,RCV TIME=%lx,curr_time=%llx,diff=%llx,jitter=%d,pkt_count=%d", origin_time, rcv_label, rcv_color,rcv_time, curr_time, diff_time, jitter,++pkt_count);
        if (first_pkt=true)
        {
            //开启定时器
	    first_pkt=false;
