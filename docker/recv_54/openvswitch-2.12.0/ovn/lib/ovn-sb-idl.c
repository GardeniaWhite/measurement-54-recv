...
static struct sbrec_controller_event *
sbrec_controller_event_cast(const struct ovsdb_idl_row *row)
{
    return row ? CONTAINER_OF(row, struct sbrec_controller_event, header_) : NULL;
}
...
sbrec_ha_chassis_cast(const struct ovsdb_idl_row *row)
{
    return row ? CONTAINER_OF(row, struct sbrec_ha_chassis, header_) : NULL;
}

static struct sbrec_ha_chassis_group *
sbrec_ha_chassis_group_cast(const struct ovsdb_idl_row *row)
{
    return row ? CONTAINER_OF(row, struct sbrec_ha_chassis_group, header_) : NULL;
}

static struct sbrec_igmp_group *
sbrec_igmp_group_cast(const struct ovsdb_idl_row *row)
{
    return row ? CONTAINER_OF(row, struct sbrec_igmp_group, header_) : NULL;
}

static struct sbrec_ip_multicast *
sbrec_ip_multicast_cast(const struct ovsdb_idl_row *row)
{
    return row ? CONTAINER_OF(row, struct sbrec_ip_multicast, header_) : NULL;
}
...
static void
sbrec_chassis_parse_transport_zones(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct sbrec_chassis *row = sbrec_chassis_cast(row_);
    row->transport_zones = NULL;
    row->n_transport_zones = 0;
    for (size_t i = 0; i < datum->n; i++) {
        if (!row->n_transport_zones) {
            row->transport_zones = xmalloc(datum->n * sizeof *row->transport_zones);
        }
        row->transport_zones[row->n_transport_zones] = datum->keys[i].string;
        row->n_transport_zones++;
    }
}
...
static void
sbrec_chassis_unparse_transport_zones(struct ovsdb_idl_row *row_)
{
    struct sbrec_chassis *row = sbrec_chassis_cast(row_);
    free(row->transport_zones);
}
...
/* Causes the original contents of column "transport_zones" in 'row' to be
 * verified as a prerequisite to completing the transaction.  That is, if
 * "transport_zones" in 'row' changed (or if 'row' was deleted) between the
 * time that the IDL originally read its contents and the time that the
 * transaction aborts and ovsdb_idl_txn_commit() returns TXN_TRY_AGAIN.
 *
 * The intention is that, to ensure that no transaction commits based on dirty
 * reads, an application should call this function any time "transport_zones" is
 * read as part of a read-modify-write operation.
 *
 * In some cases this function reduces to a no-op, because the current value
 * of "transport_zones" is already known:
 *
 *   - If 'row' is a row created by the current transaction (returned by
 *     sbrec_chassis_insert()).
 *
 *   - If "transport_zones" has already been modified (with
 *     sbrec_chassis_set_transport_zones()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * sbrec_chassis_set_transport_zones() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_chassis_verify_transport_zones(const struct sbrec_chassis *row)
{
    ovsdb_idl_txn_verify(&row->header_, &sbrec_chassis_col_transport_zones);
}
...
/* Returns the "transport_zones" column's value from the "Chassis" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 *
 * 'key_type' must be OVSDB_TYPE_STRING.
 * (This helps to avoid silent bugs if someone changes transport_zones's
 * type without updating the caller.)
 *
 * The caller must not modify or free the returned value.
 *
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "transport_zones" member in sbrec_chassis. */
const struct ovsdb_datum *
sbrec_chassis_get_transport_zones(const struct sbrec_chassis *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_STRING);
    return ovsdb_idl_read(&row->header_, &sbrec_chassis_col_transport_zones);
}
...
/* Sets the "transport_zones" column from the "Chassis" table in 'row' to
 * the 'transport_zones' set with 'n_transport_zones' entries.
 *
 * The caller retains ownership of the arguments. */
void
sbrec_chassis_set_transport_zones(const struct sbrec_chassis *row, const char **transport_zones, size_t n_transport_zones)
{
    struct ovsdb_datum datum;

    datum.n = n_transport_zones;
    datum.keys = n_transport_zones ? xmalloc(n_transport_zones * sizeof *datum.keys) : NULL;
    datum.values = NULL;
    for (size_t i = 0; i < n_transport_zones; i++) {
        datum.keys[i].string = xstrdup(transport_zones[i]);
    }
    ovsdb_idl_txn_write(&row->header_, &sbrec_chassis_col_transport_zones, &datum);
}
...
/* Adds the value 'new_value' to the "transport_zones" set column from the "Chassis" table
 * in 'row'.
 *
 */
void
sbrec_chassis_update_transport_zones_addvalue(const struct sbrec_chassis *row, const char *new_value)
{
    struct ovsdb_datum *datum;
    ...
    ovsdb_idl_txn_write_partial_set(&row->header_,
                                    &sbrec_chassis_col_transport_zones,
                                    datum);
}

/* Deletes the value 'delete_value' from the "transport_zones" set column from the
 * "Chassis" table in 'row'.
 *
 */
void
sbrec_chassis_update_transport_zones_delvalue(const struct sbrec_chassis *row, const char *delete_value)
{
    struct ovsdb_datum *datum;
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_chassis_add_clause_transport_zones(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const char **transport_zones, size_t n_transport_zones)
{
    struct ovsdb_datum datum;
    datum.n = n_transport_zones;
    datum.keys = n_transport_zones ? xmalloc(n_transport_zones * sizeof *datum.keys) : NULL;
    datum.values = NULL;
    for (size_t i = 0; i < n_transport_zones; i++) {
        datum.keys[i].string = CONST_CAST(char *, transport_zones[i]);
    }
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_STRING, OVSDB_TYPE_VOID);
    ovsdb_idl_condition_add_clause(cond,
                          function,
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_chassis_index_set_transport_zones(const struct sbrec_chassis *row, const char **transport_zones, size_t n_transport_zones)
{
    struct ovsdb_datum datum;
    size_t i;
    ...
    datum.keys = n_transport_zones ? xmalloc(n_transport_zones * sizeof *datum.keys) : NULL;
    datum.values = NULL;
    for (i = 0; i < n_transport_zones; i++) {
        datum.keys[i].string = xstrdup(transport_zones[i]);
    }
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_STRING, OVSDB_TYPE_VOID);
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &sbrec_chassis_columns[ SBREC_CHASSIS_COL_TRANSPORT_ZONES ], &datum, &sbrec_table_classes[SBREC_TABLE_CHASSIS]);
}
...
struct ovsdb_idl_column sbrec_chassis_columns[SBREC_CHASSIS_N_COLUMNS] = {
    ...
    [SBREC_CHASSIS_COL_TRANSPORT_ZONES] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = sbrec_chassis_parse_transport_zones,
         .unparse = sbrec_chassis_unparse_transport_zones,
    },
...
sbrec_controller_event_table_get(const struct ovsdb_idl *idl)
{
    return (const struct sbrec_controller_event_table *) idl;
}

const struct sbrec_controller_event *
sbrec_controller_event_table_first(const struct sbrec_controller_event_table *table)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return sbrec_controller_event_first(idl);
}


const struct sbrec_controller_event *
sbrec_controller_event_table_track_get_first(const struct sbrec_controller_event_table *table)
{
    const struct ovsdb_idl *idl =  (const struct ovsdb_idl *) table;
    return sbrec_controller_event_track_get_first(idl);
}


static void
sbrec_controller_event_parse_chassis(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct sbrec_controller_event *row = sbrec_controller_event_cast(row_);
...
static void
sbrec_controller_event_parse_event_info(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct sbrec_controller_event *row = sbrec_controller_event_cast(row_);
    smap_init(&row->event_info);
    for (size_t i = 0; i < datum->n; i++) {
        smap_add(&row->event_info,
                 datum->keys[i].string,
                 datum->values[i].string);
    }
}
...
static void
sbrec_controller_event_parse_event_type(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct sbrec_controller_event *row = sbrec_controller_event_cast(row_);

    if (datum->n >= 1) {
        row->event_type = datum->keys[0].string;
    } else {
        row->event_type = "";
    }
}
...
static void
sbrec_controller_event_parse_seq_num(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct sbrec_controller_event *row = sbrec_controller_event_cast(row_);

    if (datum->n >= 1) {
        row->seq_num = datum->keys[0].integer;
    } else {
        row->seq_num = 0;
    }
}
...
static void
sbrec_controller_event_unparse_chassis(struct ovsdb_idl_row *row OVS_UNUSED)
{
    /* Nothing to do. */
}

static void
sbrec_controller_event_unparse_event_info(struct ovsdb_idl_row *row_)
{
    struct sbrec_controller_event *row = sbrec_controller_event_cast(row_);
    smap_destroy(&row->event_info);
}

static void
sbrec_controller_event_unparse_event_type(struct ovsdb_idl_row *row OVS_UNUSED)
{
    /* Nothing to do. */
}

static void
sbrec_controller_event_unparse_seq_num(struct ovsdb_idl_row *row OVS_UNUSED)
{
    /* Nothing to do. */
}

static void
sbrec_controller_event_init__(struct ovsdb_idl_row *row)
{
    sbrec_controller_event_init(sbrec_controller_event_cast(row));
}

/* Clears the contents of 'row' in table "Controller_Event". */
void
sbrec_controller_event_init(struct sbrec_controller_event *row)
{
    memset(row, 0, sizeof *row); 
    smap_init(&row->event_info);
    row->event_type = "";
...
/* Searches table "Controller_Event" in 'idl' for a row with UUID 'uuid'.  Returns
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct sbrec_controller_event *
sbrec_controller_event_get_for_uuid(const struct ovsdb_idl *idl, const struct uuid *uuid)
{
    return sbrec_controller_event_cast(ovsdb_idl_get_row_for_uuid(idl, &sbrec_table_controller_event, uuid));
}

/* Searches table "Controller_Event" for a row with UUID 'uuid'.  Returns
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct sbrec_controller_event *
sbrec_controller_event_table_get_for_uuid(const struct sbrec_controller_event_table *table, const struct uuid *uuid)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return sbrec_controller_event_get_for_uuid(idl, uuid);
}

/* Returns a row in table "Controller_Event" in 'idl', or a null pointer if that
 * table is empty.
 *
 * Database tables are internally maintained as hash tables, so adding or
 * removing rows while traversing the same table can cause some rows to be
 * visited twice or not at apply. */
const struct sbrec_controller_event *
sbrec_controller_event_first(const struct ovsdb_idl *idl)
{
    return sbrec_controller_event_cast(ovsdb_idl_first_row(idl, &sbrec_table_controller_event));
}

/* Returns a row following 'row' within its table, or a null pointer if 'row'
 * is the last row in its table. */
const struct sbrec_controller_event *
sbrec_controller_event_next(const struct sbrec_controller_event *row)
{
    return sbrec_controller_event_cast(ovsdb_idl_next_row(&row->header_));
}

unsigned int sbrec_controller_event_get_seqno(const struct ovsdb_idl *idl)
{
    return ovsdb_idl_table_get_seqno(idl, &sbrec_table_controller_event);
}

unsigned int sbrec_controller_event_row_get_seqno(const struct sbrec_controller_event *row, enum ovsdb_idl_change change)
{
    return ovsdb_idl_row_get_seqno(&row->header_, change);
}

const struct sbrec_controller_event *
sbrec_controller_event_track_get_first(const struct ovsdb_idl *idl)
{
    return sbrec_controller_event_cast(ovsdb_idl_track_get_first(idl, &sbrec_table_controller_event));
}

const struct sbrec_controller_event
*sbrec_controller_event_track_get_next(const struct sbrec_controller_event *row)
{
    return sbrec_controller_event_cast(ovsdb_idl_track_get_next(&row->header_));
}


/* Deletes 'row' from table "Controller_Event".  'row' may be freed, so it must not be
 * accessed afterward.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_controller_event_delete(const struct sbrec_controller_event *row)
{
    ovsdb_idl_txn_delete(&row->header_);
}

/* Inserts and returns a new row in the table "Controller_Event" in the database
 * with open transaction 'txn'.
 *
 * The new row is assigned a randomly generated provisional UUID.
 * ovsdb-server will assign a different UUID when 'txn' is committed,
 * but the IDL will replace any uses of the provisional UUID in the
 * data to be to be committed by the UUID assigned by ovsdb-server. */
struct sbrec_controller_event *
sbrec_controller_event_insert(struct ovsdb_idl_txn *txn)
{
    return sbrec_controller_event_cast(ovsdb_idl_txn_insert(txn, &sbrec_table_controller_event, NULL));
}

bool
sbrec_controller_event_is_updated(const struct sbrec_controller_event *row, enum sbrec_controller_event_column_id column)
{
    return ovsdb_idl_track_is_updated(&row->header_, &sbrec_controller_event_columns[column]);
}
 ...
 *
 *   - If 'row' is a row created by the current transaction (returned by
 *     sbrec_controller_event_insert()).
 *
 *   - If "chassis" has already been modified (with
 *     sbrec_controller_event_set_chassis()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * sbrec_controller_event_set_chassis() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_controller_event_verify_chassis(const struct sbrec_controller_event *row)
{
    ovsdb_idl_txn_verify(&row->header_, &sbrec_controller_event_col_chassis);
}

/* Causes the original contents of column "event_info" in 'row' to be
 * verified as a prerequisite to completing the transaction.  That is, if
 * "event_info" in 'row' changed (or if 'row' was deleted) between the
 * time that the IDL originally read its contents and the time that the
 * transaction aborts and ovsdb_idl_txn_commit() returns TXN_TRY_AGAIN.
 *
 * The intention is that, to ensure that no transaction commits based on dirty
 * reads, an application should call this function any time "event_info" is
 * read as part of a read-modify-write operation.
 *
 * In some cases this function reduces to a no-op, because the current value
 * of "event_info" is already known:
 *
 *   - If 'row' is a row created by the current transaction (returned by
 *     sbrec_controller_event_insert()).
 *
 *   - If "event_info" has already been modified (with
 *     sbrec_controller_event_set_event_info()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * sbrec_controller_event_set_event_info() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_controller_event_verify_event_info(const struct sbrec_controller_event *row)
{
    ovsdb_idl_txn_verify(&row->header_, &sbrec_controller_event_col_event_info);
}

/* Causes the original contents of column "event_type" in 'row' to be
 * verified as a prerequisite to completing the transaction.  That is, if
 * "event_type" in 'row' changed (or if 'row' was deleted) between the
 * time that the IDL originally read its contents and the time that the
 * transaction aborts and ovsdb_idl_txn_commit() returns TXN_TRY_AGAIN.
 *
 * The intention is that, to ensure that no transaction commits based on dirty
 * reads, an application should call this function any time "event_type" is
 * read as part of a read-modify-write operation.
 *
 * In some cases this function reduces to a no-op, because the current value
 * of "event_type" is already known:
 *
 *   - If 'row' is a row created by the current transaction (returned by
 *     sbrec_controller_event_insert()).
 *
 *   - If "event_type" has already been modified (with
 *     sbrec_controller_event_set_event_type()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * sbrec_controller_event_set_event_type() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_controller_event_verify_event_type(const struct sbrec_controller_event *row)
{
    ovsdb_idl_txn_verify(&row->header_, &sbrec_controller_event_col_event_type);
}

/* Causes the original contents of column "seq_num" in 'row' to be
 * verified as a prerequisite to completing the transaction.  That is, if
 * "seq_num" in 'row' changed (or if 'row' was deleted) between the
 * time that the IDL originally read its contents and the time that the
 * transaction aborts and ovsdb_idl_txn_commit() returns TXN_TRY_AGAIN.
 *
 * The intention is that, to ensure that no transaction commits based on dirty
 * reads, an application should call this function any time "seq_num" is
 * read as part of a read-modify-write operation.
 *
 * In some cases this function reduces to a no-op, because the current value
 * of "seq_num" is already known:
 *
 *   - If 'row' is a row created by the current transaction (returned by
 *     sbrec_controller_event_insert()).
 *
 *   - If "seq_num" has already been modified (with
 *     sbrec_controller_event_set_seq_num()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * sbrec_controller_event_set_seq_num() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_controller_event_verify_seq_num(const struct sbrec_controller_event *row)
{
    ovsdb_idl_txn_verify(&row->header_, &sbrec_controller_event_col_seq_num);
}

/* Returns the "chassis" column's value from the "Controller_Event" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "chassis" member in sbrec_controller_event. */
const struct ovsdb_datum *
sbrec_controller_event_get_chassis(const struct sbrec_controller_event *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_UUID);
    return ovsdb_idl_read(&row->header_, &sbrec_controller_event_col_chassis);
}

/* Returns the "event_info" column's value from the "Controller_Event" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 ...
 * 'key_type' must be OVSDB_TYPE_STRING.
 * 'value_type' must be OVSDB_TYPE_STRING.
 * (This helps to avoid silent bugs if someone changes event_info's
 * type without updating the caller.)
 *
 * The caller must not modify or free the returned value.
 *
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "event_info" member in sbrec_controller_event. */
const struct ovsdb_datum *
sbrec_controller_event_get_event_info(const struct sbrec_controller_event *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED,
	enum ovsdb_atomic_type value_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_STRING);
    ovs_assert(value_type == OVSDB_TYPE_STRING);
    return ovsdb_idl_read(&row->header_, &sbrec_controller_event_col_event_info);
}

/* Returns the "event_type" column's value from the "Controller_Event" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 *
 * 'key_type' must be OVSDB_TYPE_STRING.
 * (This helps to avoid silent bugs if someone changes event_type's
 * type without updating the caller.)
 *
 * The caller must not modify or free the returned value.
 *
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "event_type" member in sbrec_controller_event. */
const struct ovsdb_datum *
sbrec_controller_event_get_event_type(const struct sbrec_controller_event *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_STRING);
    return ovsdb_idl_read(&row->header_, &sbrec_controller_event_col_event_type);
}

/* Returns the "seq_num" column's value from the "Controller_Event" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 *
 * 'key_type' must be OVSDB_TYPE_INTEGER.
 * (This helps to avoid silent bugs if someone changes seq_num's
 * type without updating the caller.)
 *
 * The caller must not modify or free the returned value.
 *
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "seq_num" member in sbrec_controller_event. */
const struct ovsdb_datum *
sbrec_controller_event_get_seq_num(const struct sbrec_controller_event *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_INTEGER);
    return ovsdb_idl_read(&row->header_, &sbrec_controller_event_col_seq_num);
}

/* Sets the "chassis" column from the "Controller_Event" table in 'row' to
 * the 'chassis' set.
 *
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_controller_event_set_chassis(const struct sbrec_controller_event *row, const struct sbrec_chassis *chassis)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &sbrec_controller_event_col_chassis, &datum);
}

/* Sets the "event_info" column's value from the "Controller_Event" table in 'row'
 * to 'event_info'.
 *
 * The caller retains ownership of 'event_info' and everything in it. */
void
sbrec_controller_event_set_event_info(const struct sbrec_controller_event *row, const struct smap *event_info)
{
    struct ovsdb_datum datum;

    if (event_info) {
        ovsdb_datum_from_smap(&datum, event_info);
    } else {
        ovsdb_datum_init_empty(&datum);
    }
    ovsdb_idl_txn_write(&row->header_,
                        &sbrec_controller_event_col_event_info,
                        &datum);
}


/* Sets the "event_type" column from the "Controller_Event" table in 'row' to
 * 'event_type'.
 *
 * Argument constraints: must be "empty_lb_backends"
 *
 * The caller retains ownership of the arguments. */
void
sbrec_controller_event_set_event_type(const struct sbrec_controller_event *row, const char *event_type)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    datum.n = 1;
    datum.keys = &key;
    key.string = CONST_CAST(char *, event_type);
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &sbrec_controller_event_col_event_type, &datum);
}

/* Sets the "seq_num" column from the "Controller_Event" table in 'row' to
 * 'seq_num'.
 *
 * The caller retains ownership of the arguments. */
void
sbrec_controller_event_set_seq_num(const struct sbrec_controller_event *row, int64_t seq_num)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    key.integer = seq_num;
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &sbrec_controller_event_col_seq_num, &datum);
}

/* Adds the value 'new_value' to the "chassis" set column from the "Controller_Event" table
 * in 'row'.
 *
 */
void
sbrec_controller_event_update_chassis_addvalue(const struct sbrec_controller_event *row, const struct sbrec_chassis *new_value)
{
    struct ovsdb_datum *datum;
    ...
    ovsdb_idl_txn_write_partial_set(&row->header_,
                                    &sbrec_controller_event_col_chassis,
                                    datum);
}

/* Deletes the value 'delete_value' from the "chassis" set column from the
 * "Controller_Event" table in 'row'.
 *
 */
void
sbrec_controller_event_update_chassis_delvalue(const struct sbrec_controller_event *row, const struct sbrec_chassis *delete_value)
{
    struct ovsdb_datum *datum;
...
/* Sets an element of the "event_info" map column from the "Controller_Event" table in 'row'
 * to 'new_value' given the key value 'new_key'.
 *
 */
void
sbrec_controller_event_update_event_info_setkey(const struct sbrec_controller_event *row, const char *new_key, const char *new_value)
{
    struct ovsdb_datum *datum;
...
/* Deletes an element of the "event_info" map column from the "Controller_Event" table in 'row'
 * given the key value 'delete_key'.
 *
 */
void
sbrec_controller_event_update_event_info_delkey(const struct sbrec_controller_event *row, const char *delete_key)
{
    struct ovsdb_datum *datum;
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_controller_event_add_clause_chassis(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const struct uuid *chassis)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
 ...
 * The caller retains ownership of 'event_info' and everything in it. */
void
sbrec_controller_event_add_clause_event_info(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const struct smap *event_info)
{
    struct ovsdb_datum datum;
    ...
    ovsdb_datum_destroy(&datum, &sbrec_controller_event_col_event_info.type);
}
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_controller_event_add_clause_event_type(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const char *event_type)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    ovsdb_idl_condition_add_clause(cond,
                          function,
                          &sbrec_controller_event_col_event_type,
                          &datum);
}
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_controller_event_add_clause_seq_num(struct ovsdb_idl_condition *cond, enum ovsdb_function function, int64_t seq_num)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    ovsdb_idl_condition_add_clause(cond,
                          function,
                          &sbrec_controller_event_col_seq_num,
                          &datum);
}

/* Destroy 'row' of kind "Controller_Event". The row must have been
 * created with ovsdb_idl_index_init_row.
 */
void
sbrec_controller_event_index_destroy_row(const struct sbrec_controller_event *row)
{
    ovsdb_idl_index_destroy_row(&row->header_);
}
        

/* Creates a new row of kind "Controller_Event". */
struct sbrec_controller_event *
sbrec_controller_event_index_init_row(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &sbrec_table_controller_event);
    return ALIGNED_CAST(struct sbrec_controller_event *, ovsdb_idl_index_init_row(index));
}

struct sbrec_controller_event *
sbrec_controller_event_index_find(struct ovsdb_idl_index *index, const struct sbrec_controller_event *target)
{
    ovs_assert(index->table->class_ == &sbrec_table_controller_event);
    return sbrec_controller_event_cast(ovsdb_idl_index_find(index, &target->header_));
}

/* Compares 'a' to 'b' and returns a strcmp()-type result. */
int
sbrec_controller_event_index_compare(
    struct ovsdb_idl_index *index, 
    const struct sbrec_controller_event *a, 
    const struct sbrec_controller_event *b)
{
    return ovsdb_idl_index_compare(index, &a->header_, &b->header_);
}

struct ovsdb_idl_cursor
sbrec_controller_event_cursor_first(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &sbrec_table_controller_event);
    return ovsdb_idl_cursor_first(index);
}
...
struct ovsdb_idl_cursor
sbrec_controller_event_cursor_first_eq(
    struct ovsdb_idl_index *index, const struct sbrec_controller_event *target)
{
    ovs_assert(index->table->class_ == &sbrec_table_controller_event);
    return ovsdb_idl_cursor_first_eq(index, &target->header_);
}
...
struct ovsdb_idl_cursor
sbrec_controller_event_cursor_first_ge(
    struct ovsdb_idl_index *index, const struct sbrec_controller_event *target)
{
    ovs_assert(index->table->class_ == &sbrec_table_controller_event);
    return ovsdb_idl_cursor_first_ge(index, &target->header_);
...
struct sbrec_controller_event *
sbrec_controller_event_cursor_data(struct ovsdb_idl_cursor *cursor)
{
    return sbrec_controller_event_cast(ovsdb_idl_cursor_data(cursor));
}
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_controller_event_index_set_chassis(const struct sbrec_controller_event *row, const struct sbrec_chassis *chassis)
{
    struct ovsdb_datum datum;
    union ovsdb_atom *key = xmalloc(sizeof (union ovsdb_atom));
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &sbrec_controller_event_columns[ SBREC_CONTROLLER_EVENT_COL_CHASSIS ], &datum, &sbrec_table_classes[SBREC_TABLE_CONTROLLER_EVENT]);
}
 ...
 * The caller retains ownership of 'event_info' and everything in it. */
void
sbrec_controller_event_index_set_event_info(const struct sbrec_controller_event *row, const struct smap *event_info)
{
    struct ovsdb_datum *datum = xmalloc(sizeof(struct ovsdb_datum));
    ...
    if (event_info) {
        ...
        datum->n = smap_count(event_info);
        datum->keys = xmalloc(datum->n * sizeof *datum->keys);
        datum->values = xmalloc(datum->n * sizeof *datum->values);

        i = 0;
        SMAP_FOR_EACH (node, event_info) {
            datum->keys[i].string = node->key;
            datum->values[i].string = node->value;
            i++;
        }
    ...
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_),
                          &sbrec_controller_event_columns[SBREC_CONTROLLER_EVENT_COL_EVENT_INFO],
                          datum,
                          &sbrec_table_classes[SBREC_TABLE_CONTROLLER_EVENT]);
}
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_controller_event_index_set_event_type(const struct sbrec_controller_event *row, const char *event_type)
{
    struct ovsdb_datum datum;
    union ovsdb_atom *key = xmalloc(sizeof(union ovsdb_atom));
    ...
    key->string = CONST_CAST(char *, event_type);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &sbrec_controller_event_columns[ SBREC_CONTROLLER_EVENT_COL_EVENT_TYPE ], &datum, &sbrec_table_classes[SBREC_TABLE_CONTROLLER_EVENT]);
}
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_controller_event_index_set_seq_num(const struct sbrec_controller_event *row, int64_t seq_num)
{
    struct ovsdb_datum datum;
    union ovsdb_atom *key = xmalloc(sizeof(union ovsdb_atom));
    ...
    key->integer = seq_num;
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &sbrec_controller_event_columns[ SBREC_CONTROLLER_EVENT_COL_SEQ_NUM ], &datum, &sbrec_table_classes[SBREC_TABLE_CONTROLLER_EVENT]);
}

struct ovsdb_idl_column sbrec_controller_event_columns[SBREC_CONTROLLER_EVENT_N_COLUMNS];

void
sbrec_controller_event_set_condition(struct ovsdb_idl *idl, struct ovsdb_idl_condition *condition)
{
    ovsdb_idl_set_condition(idl, &sbrec_table_controller_event, condition);
}
static union ovsdb_atom Controller_Event_col_event_type_key_enum_keys[1] = {
    { .string = "empty_lb_backends" },
};
static struct ovsdb_datum Controller_Event_col_event_type_key_enum = {
    .n = 1,
    .keys = Controller_Event_col_event_type_key_enum_keys,
};
...
struct ovsdb_idl_column sbrec_controller_event_columns[SBREC_CONTROLLER_EVENT_N_COLUMNS] = {
    [SBREC_CONTROLLER_EVENT_COL_CHASSIS] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = sbrec_controller_event_parse_chassis,
         .unparse = sbrec_controller_event_unparse_chassis,
    },
    ...
    [SBREC_CONTROLLER_EVENT_COL_EVENT_INFO] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = sbrec_controller_event_parse_event_info,
         .unparse = sbrec_controller_event_unparse_event_info,
    },
    ...
    [SBREC_CONTROLLER_EVENT_COL_EVENT_TYPE] = {
         ...
         .type = {
            .key = {
               .type = OVSDB_TYPE_STRING,
               .enum_ = &Controller_Event_col_event_type_key_enum,
               .string = { .minLen = 0, .maxLen = UINT_MAX },
            },
            .value = OVSDB_BASE_VOID_INIT,
            .n_min = 1,
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = sbrec_controller_event_parse_event_type,
         .unparse = sbrec_controller_event_unparse_event_type,
    },
    ...
    [SBREC_CONTROLLER_EVENT_COL_SEQ_NUM] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = sbrec_controller_event_parse_seq_num,
         .unparse = sbrec_controller_event_unparse_seq_num,
    },
...
const struct sbrec_ha_chassis_table *
sbrec_ha_chassis_table_get(const struct ovsdb_idl *idl)
{
    return (const struct sbrec_ha_chassis_table *) idl;
}

const struct sbrec_ha_chassis *
sbrec_ha_chassis_table_first(const struct sbrec_ha_chassis_table *table)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return sbrec_ha_chassis_first(idl);
}


const struct sbrec_ha_chassis *
sbrec_ha_chassis_table_track_get_first(const struct sbrec_ha_chassis_table *table)
{
    const struct ovsdb_idl *idl =  (const struct ovsdb_idl *) table;
    return sbrec_ha_chassis_track_get_first(idl);
}


static void
sbrec_ha_chassis_parse_chassis(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct sbrec_ha_chassis *row = sbrec_ha_chassis_cast(row_);
...
static void
sbrec_ha_chassis_parse_external_ids(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct sbrec_ha_chassis *row = sbrec_ha_chassis_cast(row_);
    smap_init(&row->external_ids);
    for (size_t i = 0; i < datum->n; i++) {
...
static void
sbrec_ha_chassis_parse_priority(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct sbrec_ha_chassis *row = sbrec_ha_chassis_cast(row_);
...
static void
sbrec_ha_chassis_unparse_chassis(struct ovsdb_idl_row *row OVS_UNUSED)
{
    /* Nothing to do. */
}

static void
sbrec_ha_chassis_unparse_external_ids(struct ovsdb_idl_row *row_)
{
    struct sbrec_ha_chassis *row = sbrec_ha_chassis_cast(row_);
    smap_destroy(&row->external_ids);
}
...
static void
sbrec_ha_chassis_unparse_priority(struct ovsdb_idl_row *row OVS_UNUSED)
{
    /* Nothing to do. */
}

static void
sbrec_ha_chassis_init__(struct ovsdb_idl_row *row)
{
    sbrec_ha_chassis_init(sbrec_ha_chassis_cast(row));
}

/* Clears the contents of 'row' in table "HA_Chassis". */
void
sbrec_ha_chassis_init(struct sbrec_ha_chassis *row)
{
    memset(row, 0, sizeof *row); 
    smap_init(&row->external_ids);
}

/* Searches table "HA_Chassis" in 'idl' for a row with UUID 'uuid'.  Returns
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct sbrec_ha_chassis *
sbrec_ha_chassis_get_for_uuid(const struct ovsdb_idl *idl, const struct uuid *uuid)
{
    return sbrec_ha_chassis_cast(ovsdb_idl_get_row_for_uuid(idl, &sbrec_table_ha_chassis, uuid));
}

/* Searches table "HA_Chassis" for a row with UUID 'uuid'.  Returns
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct sbrec_ha_chassis *
sbrec_ha_chassis_table_get_for_uuid(const struct sbrec_ha_chassis_table *table, const struct uuid *uuid)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return sbrec_ha_chassis_get_for_uuid(idl, uuid);
}

/* Returns a row in table "HA_Chassis" in 'idl', or a null pointer if that
 * table is empty.
 *
 * Database tables are internally maintained as hash tables, so adding or
 * removing rows while traversing the same table can cause some rows to be
 * visited twice or not at apply. */
const struct sbrec_ha_chassis *
sbrec_ha_chassis_first(const struct ovsdb_idl *idl)
{
    return sbrec_ha_chassis_cast(ovsdb_idl_first_row(idl, &sbrec_table_ha_chassis));
}

/* Returns a row following 'row' within its table, or a null pointer if 'row'
 * is the last row in its table. */
const struct sbrec_ha_chassis *
sbrec_ha_chassis_next(const struct sbrec_ha_chassis *row)
{
    return sbrec_ha_chassis_cast(ovsdb_idl_next_row(&row->header_));
}

unsigned int sbrec_ha_chassis_get_seqno(const struct ovsdb_idl *idl)
{
    return ovsdb_idl_table_get_seqno(idl, &sbrec_table_ha_chassis);
}

unsigned int sbrec_ha_chassis_row_get_seqno(const struct sbrec_ha_chassis *row, enum ovsdb_idl_change change)
{
    return ovsdb_idl_row_get_seqno(&row->header_, change);
}

const struct sbrec_ha_chassis *
sbrec_ha_chassis_track_get_first(const struct ovsdb_idl *idl)
{
    return sbrec_ha_chassis_cast(ovsdb_idl_track_get_first(idl, &sbrec_table_ha_chassis));
}

const struct sbrec_ha_chassis
*sbrec_ha_chassis_track_get_next(const struct sbrec_ha_chassis *row)
{
    return sbrec_ha_chassis_cast(ovsdb_idl_track_get_next(&row->header_));
}


/* Deletes 'row' from table "HA_Chassis".  'row' may be freed, so it must not be
 * accessed afterward.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_ha_chassis_delete(const struct sbrec_ha_chassis *row)
{
    ovsdb_idl_txn_delete(&row->header_);
}

/* Inserts and returns a new row in the table "HA_Chassis" in the database
 * with open transaction 'txn'.
 *
 * The new row is assigned a randomly generated provisional UUID.
 * ovsdb-server will assign a different UUID when 'txn' is committed,
 * but the IDL will replace any uses of the provisional UUID in the
 * data to be to be committed by the UUID assigned by ovsdb-server. */
struct sbrec_ha_chassis *
sbrec_ha_chassis_insert(struct ovsdb_idl_txn *txn)
{
    return sbrec_ha_chassis_cast(ovsdb_idl_txn_insert(txn, &sbrec_table_ha_chassis, NULL));
}

bool
sbrec_ha_chassis_is_updated(const struct sbrec_ha_chassis *row, enum sbrec_ha_chassis_column_id column)
{
    return ovsdb_idl_track_is_updated(&row->header_, &sbrec_ha_chassis_columns[column]);
}
 ...
 *
 *   - If 'row' is a row created by the current transaction (returned by
 *     sbrec_ha_chassis_insert()).
 *
 *   - If "chassis" has already been modified (with
 *     sbrec_ha_chassis_set_chassis()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * sbrec_ha_chassis_set_chassis() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_ha_chassis_verify_chassis(const struct sbrec_ha_chassis *row)
{
    ovsdb_idl_txn_verify(&row->header_, &sbrec_ha_chassis_col_chassis);
}
 ...
 *
 *   - If "external_ids" has already been modified (with
 *     sbrec_ha_chassis_set_external_ids()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * sbrec_ha_chassis_set_external_ids() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_ha_chassis_verify_external_ids(const struct sbrec_ha_chassis *row)
{
    ovsdb_idl_txn_verify(&row->header_, &sbrec_ha_chassis_col_external_ids);
}
 ...
 *
 *   - If "priority" has already been modified (with
 *     sbrec_ha_chassis_set_priority()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * sbrec_ha_chassis_set_priority() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_ha_chassis_verify_priority(const struct sbrec_ha_chassis *row)
{
    ovsdb_idl_txn_verify(&row->header_, &sbrec_ha_chassis_col_priority);
}

/* Returns the "chassis" column's value from the "HA_Chassis" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "chassis" member in sbrec_ha_chassis. */
const struct ovsdb_datum *
sbrec_ha_chassis_get_chassis(const struct sbrec_ha_chassis *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_UUID);
    return ovsdb_idl_read(&row->header_, &sbrec_ha_chassis_col_chassis);
}

/* Returns the "external_ids" column's value from the "HA_Chassis" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "external_ids" member in sbrec_ha_chassis. */
const struct ovsdb_datum *
sbrec_ha_chassis_get_external_ids(const struct sbrec_ha_chassis *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED,
	enum ovsdb_atomic_type value_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_STRING);
    ovs_assert(value_type == OVSDB_TYPE_STRING);
    return ovsdb_idl_read(&row->header_, &sbrec_ha_chassis_col_external_ids);
}

/* Returns the "priority" column's value from the "HA_Chassis" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "priority" member in sbrec_ha_chassis. */
const struct ovsdb_datum *
sbrec_ha_chassis_get_priority(const struct sbrec_ha_chassis *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_INTEGER);
    return ovsdb_idl_read(&row->header_, &sbrec_ha_chassis_col_priority);
}

/* Sets the "chassis" column from the "HA_Chassis" table in 'row' to
 * the 'chassis' set.
 *
 * If "chassis" is null, the column will be the empty set,
 * otherwise it will contain the specified value.
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ha_chassis_set_chassis(const struct sbrec_ha_chassis *row, const struct sbrec_chassis *chassis)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &sbrec_ha_chassis_col_chassis, &datum);
}

/* Sets the "external_ids" column's value from the "HA_Chassis" table in 'row'
 * to 'external_ids'.
 *
 * The caller retains ownership of 'external_ids' and everything in it. */
void
sbrec_ha_chassis_set_external_ids(const struct sbrec_ha_chassis *row, const struct smap *external_ids)
{
    struct ovsdb_datum datum;
...
/* Sets the "priority" column from the "HA_Chassis" table in 'row' to
 * 'priority'.
 *
 * Argument constraints: in range 0 to 32,767
 *
 * The caller retains ownership of the arguments. */
void
sbrec_ha_chassis_set_priority(const struct sbrec_ha_chassis *row, int64_t priority)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    key.integer = priority;
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &sbrec_ha_chassis_col_priority, &datum);
}

/* Adds the value 'new_value' to the "chassis" set column from the "HA_Chassis" table
 * in 'row'.
 *
 */
void
sbrec_ha_chassis_update_chassis_addvalue(const struct sbrec_ha_chassis *row, const struct sbrec_chassis *new_value)
{
    struct ovsdb_datum *datum;
...
/* Deletes the value 'delete_value' from the "chassis" set column from the
 * "HA_Chassis" table in 'row'.
 *
 */
void
sbrec_ha_chassis_update_chassis_delvalue(const struct sbrec_ha_chassis *row, const struct sbrec_chassis *delete_value)
{
    struct ovsdb_datum *datum;
...
/* Sets an element of the "external_ids" map column from the "HA_Chassis" table in 'row'
 * to 'new_value' given the key value 'new_key'.
 *
 */
void
sbrec_ha_chassis_update_external_ids_setkey(const struct sbrec_ha_chassis *row, const char *new_key, const char *new_value)
{
    struct ovsdb_datum *datum;
...
/* Deletes an element of the "external_ids" map column from the "HA_Chassis" table in 'row'
 * given the key value 'delete_key'.
 *
 */
void
sbrec_ha_chassis_update_external_ids_delkey(const struct sbrec_ha_chassis *row, const char *delete_key)
{
    struct ovsdb_datum *datum;
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ha_chassis_add_clause_chassis(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const struct uuid *chassis)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
 ...
 * The caller retains ownership of 'external_ids' and everything in it. */
void
sbrec_ha_chassis_add_clause_external_ids(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const struct smap *external_ids)
{
    struct ovsdb_datum datum;
    ...
    ovsdb_datum_destroy(&datum, &sbrec_ha_chassis_col_external_ids.type);
}
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ha_chassis_add_clause_priority(struct ovsdb_idl_condition *cond, enum ovsdb_function function, int64_t priority)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
...
/* Destroy 'row' of kind "HA_Chassis". The row must have been
 * created with ovsdb_idl_index_init_row.
 */
void
sbrec_ha_chassis_index_destroy_row(const struct sbrec_ha_chassis *row)
{
    ovsdb_idl_index_destroy_row(&row->header_);
}
        

/* Creates a new row of kind "HA_Chassis". */
struct sbrec_ha_chassis *
sbrec_ha_chassis_index_init_row(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &sbrec_table_ha_chassis);
    return ALIGNED_CAST(struct sbrec_ha_chassis *, ovsdb_idl_index_init_row(index));
}

struct sbrec_ha_chassis *
sbrec_ha_chassis_index_find(struct ovsdb_idl_index *index, const struct sbrec_ha_chassis *target)
{
    ovs_assert(index->table->class_ == &sbrec_table_ha_chassis);
    return sbrec_ha_chassis_cast(ovsdb_idl_index_find(index, &target->header_));
}

/* Compares 'a' to 'b' and returns a strcmp()-type result. */
int
sbrec_ha_chassis_index_compare(
    struct ovsdb_idl_index *index, 
    const struct sbrec_ha_chassis *a, 
    const struct sbrec_ha_chassis *b)
{
    return ovsdb_idl_index_compare(index, &a->header_, &b->header_);
}

struct ovsdb_idl_cursor
sbrec_ha_chassis_cursor_first(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &sbrec_table_ha_chassis);
    return ovsdb_idl_cursor_first(index);
}
...
struct ovsdb_idl_cursor
sbrec_ha_chassis_cursor_first_eq(
    struct ovsdb_idl_index *index, const struct sbrec_ha_chassis *target)
{
    ovs_assert(index->table->class_ == &sbrec_table_ha_chassis);
    return ovsdb_idl_cursor_first_eq(index, &target->header_);
}
...
struct ovsdb_idl_cursor
sbrec_ha_chassis_cursor_first_ge(
    struct ovsdb_idl_index *index, const struct sbrec_ha_chassis *target)
{
    ovs_assert(index->table->class_ == &sbrec_table_ha_chassis);
    return ovsdb_idl_cursor_first_ge(index, &target->header_);
...
struct sbrec_ha_chassis *
sbrec_ha_chassis_cursor_data(struct ovsdb_idl_cursor *cursor)
{
    return sbrec_ha_chassis_cast(ovsdb_idl_cursor_data(cursor));
}
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ha_chassis_index_set_chassis(const struct sbrec_ha_chassis *row, const struct sbrec_chassis *chassis)
{
    struct ovsdb_datum datum;
    union ovsdb_atom *key = xmalloc(sizeof (union ovsdb_atom));
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &sbrec_ha_chassis_columns[ SBREC_HA_CHASSIS_COL_CHASSIS ], &datum, &sbrec_table_classes[SBREC_TABLE_HA_CHASSIS]);
}
 ...
 * The caller retains ownership of 'external_ids' and everything in it. */
void
sbrec_ha_chassis_index_set_external_ids(const struct sbrec_ha_chassis *row, const struct smap *external_ids)
{
    struct ovsdb_datum *datum = xmalloc(sizeof(struct ovsdb_datum));
    ...
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_),
                          &sbrec_ha_chassis_columns[SBREC_HA_CHASSIS_COL_EXTERNAL_IDS],
                          datum,
                          &sbrec_table_classes[SBREC_TABLE_HA_CHASSIS]);
}
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ha_chassis_index_set_priority(const struct sbrec_ha_chassis *row, int64_t priority)
{
    struct ovsdb_datum datum;
    union ovsdb_atom *key = xmalloc(sizeof(union ovsdb_atom));
    ...
    key->integer = priority;
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &sbrec_ha_chassis_columns[ SBREC_HA_CHASSIS_COL_PRIORITY ], &datum, &sbrec_table_classes[SBREC_TABLE_HA_CHASSIS]);
}

struct ovsdb_idl_column sbrec_ha_chassis_columns[SBREC_HA_CHASSIS_N_COLUMNS];

void
sbrec_ha_chassis_set_condition(struct ovsdb_idl *idl, struct ovsdb_idl_condition *condition)
{
    ovsdb_idl_set_condition(idl, &sbrec_table_ha_chassis, condition);
}
...
struct ovsdb_idl_column sbrec_ha_chassis_columns[SBREC_HA_CHASSIS_N_COLUMNS] = {
    [SBREC_HA_CHASSIS_COL_CHASSIS] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = sbrec_ha_chassis_parse_chassis,
         .unparse = sbrec_ha_chassis_unparse_chassis,
    },
    ...
    [SBREC_HA_CHASSIS_COL_EXTERNAL_IDS] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = sbrec_ha_chassis_parse_external_ids,
         .unparse = sbrec_ha_chassis_unparse_external_ids,
    },
    ...
    [SBREC_HA_CHASSIS_COL_PRIORITY] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = sbrec_ha_chassis_parse_priority,
         .unparse = sbrec_ha_chassis_unparse_priority,
    },
...
sbrec_ha_chassis_group_table_get(const struct ovsdb_idl *idl)
{
    return (const struct sbrec_ha_chassis_group_table *) idl;
}

const struct sbrec_ha_chassis_group *
sbrec_ha_chassis_group_table_first(const struct sbrec_ha_chassis_group_table *table)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return sbrec_ha_chassis_group_first(idl);
}


const struct sbrec_ha_chassis_group *
sbrec_ha_chassis_group_table_track_get_first(const struct sbrec_ha_chassis_group_table *table)
{
    const struct ovsdb_idl *idl =  (const struct ovsdb_idl *) table;
    return sbrec_ha_chassis_group_track_get_first(idl);
}


static void
sbrec_ha_chassis_group_parse_external_ids(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct sbrec_ha_chassis_group *row = sbrec_ha_chassis_group_cast(row_);
    smap_init(&row->external_ids);
    for (size_t i = 0; i < datum->n; i++) {
        smap_add(&row->external_ids,
                 datum->keys[i].string,
...
static void
sbrec_ha_chassis_group_parse_ha_chassis(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct sbrec_ha_chassis_group *row = sbrec_ha_chassis_group_cast(row_);
    row->ha_chassis = NULL;
    row->n_ha_chassis = 0;
    for (size_t i = 0; i < datum->n; i++) {
        struct sbrec_ha_chassis *keyRow = sbrec_ha_chassis_cast(ovsdb_idl_get_row_arc(row_, &sbrec_table_ha_chassis, &datum->keys[i].uuid));
        if (!keyRow) {
            continue;
        }
        if (!row->n_ha_chassis) {
            row->ha_chassis = xmalloc(datum->n * sizeof *row->ha_chassis);
        }
        row->ha_chassis[row->n_ha_chassis] = keyRow;
        row->n_ha_chassis++;
    }
}

static void
sbrec_ha_chassis_group_parse_name(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct sbrec_ha_chassis_group *row = sbrec_ha_chassis_group_cast(row_);
...
static void
sbrec_ha_chassis_group_parse_ref_chassis(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct sbrec_ha_chassis_group *row = sbrec_ha_chassis_group_cast(row_);
    row->ref_chassis = NULL;
    row->n_ref_chassis = 0;
    for (size_t i = 0; i < datum->n; i++) {
        struct sbrec_chassis *keyRow = sbrec_chassis_cast(ovsdb_idl_get_row_arc(row_, &sbrec_table_chassis, &datum->keys[i].uuid));
        if (!keyRow) {
            continue;
        }
        if (!row->n_ref_chassis) {
            row->ref_chassis = xmalloc(datum->n * sizeof *row->ref_chassis);
        }
        row->ref_chassis[row->n_ref_chassis] = keyRow;
        row->n_ref_chassis++;
    }
}

static void
sbrec_ha_chassis_group_unparse_external_ids(struct ovsdb_idl_row *row_)
{
    struct sbrec_ha_chassis_group *row = sbrec_ha_chassis_group_cast(row_);
    smap_destroy(&row->external_ids);
}
...
static void
sbrec_ha_chassis_group_unparse_ha_chassis(struct ovsdb_idl_row *row_)
{
    struct sbrec_ha_chassis_group *row = sbrec_ha_chassis_group_cast(row_);
    free(row->ha_chassis);
}
...
static void
sbrec_ha_chassis_group_unparse_name(struct ovsdb_idl_row *row OVS_UNUSED)
{
    /* Nothing to do. */
}

static void
sbrec_ha_chassis_group_unparse_ref_chassis(struct ovsdb_idl_row *row_)
{
    struct sbrec_ha_chassis_group *row = sbrec_ha_chassis_group_cast(row_);
    free(row->ref_chassis);
}

static void
sbrec_ha_chassis_group_init__(struct ovsdb_idl_row *row)
{
    sbrec_ha_chassis_group_init(sbrec_ha_chassis_group_cast(row));
}

/* Clears the contents of 'row' in table "HA_Chassis_Group". */
void
sbrec_ha_chassis_group_init(struct sbrec_ha_chassis_group *row)
{
    memset(row, 0, sizeof *row); 
    smap_init(&row->external_ids);
    row->name = "";
...
/* Searches table "HA_Chassis_Group" in 'idl' for a row with UUID 'uuid'.  Returns
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct sbrec_ha_chassis_group *
sbrec_ha_chassis_group_get_for_uuid(const struct ovsdb_idl *idl, const struct uuid *uuid)
{
    return sbrec_ha_chassis_group_cast(ovsdb_idl_get_row_for_uuid(idl, &sbrec_table_ha_chassis_group, uuid));
}

/* Searches table "HA_Chassis_Group" for a row with UUID 'uuid'.  Returns
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct sbrec_ha_chassis_group *
sbrec_ha_chassis_group_table_get_for_uuid(const struct sbrec_ha_chassis_group_table *table, const struct uuid *uuid)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return sbrec_ha_chassis_group_get_for_uuid(idl, uuid);
}

/* Returns a row in table "HA_Chassis_Group" in 'idl', or a null pointer if that
 * table is empty.
 *
 * Database tables are internally maintained as hash tables, so adding or
 * removing rows while traversing the same table can cause some rows to be
 * visited twice or not at apply. */
const struct sbrec_ha_chassis_group *
sbrec_ha_chassis_group_first(const struct ovsdb_idl *idl)
{
    return sbrec_ha_chassis_group_cast(ovsdb_idl_first_row(idl, &sbrec_table_ha_chassis_group));
}

/* Returns a row following 'row' within its table, or a null pointer if 'row'
 * is the last row in its table. */
const struct sbrec_ha_chassis_group *
sbrec_ha_chassis_group_next(const struct sbrec_ha_chassis_group *row)
{
    return sbrec_ha_chassis_group_cast(ovsdb_idl_next_row(&row->header_));
}

unsigned int sbrec_ha_chassis_group_get_seqno(const struct ovsdb_idl *idl)
{
    return ovsdb_idl_table_get_seqno(idl, &sbrec_table_ha_chassis_group);
}

unsigned int sbrec_ha_chassis_group_row_get_seqno(const struct sbrec_ha_chassis_group *row, enum ovsdb_idl_change change)
{
    return ovsdb_idl_row_get_seqno(&row->header_, change);
}

const struct sbrec_ha_chassis_group *
sbrec_ha_chassis_group_track_get_first(const struct ovsdb_idl *idl)
{
    return sbrec_ha_chassis_group_cast(ovsdb_idl_track_get_first(idl, &sbrec_table_ha_chassis_group));
}

const struct sbrec_ha_chassis_group
*sbrec_ha_chassis_group_track_get_next(const struct sbrec_ha_chassis_group *row)
{
    return sbrec_ha_chassis_group_cast(ovsdb_idl_track_get_next(&row->header_));
}


/* Deletes 'row' from table "HA_Chassis_Group".  'row' may be freed, so it must not be
 * accessed afterward.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_ha_chassis_group_delete(const struct sbrec_ha_chassis_group *row)
{
    ovsdb_idl_txn_delete(&row->header_);
}

/* Inserts and returns a new row in the table "HA_Chassis_Group" in the database
 * with open transaction 'txn'.
 *
 * The new row is assigned a randomly generated provisional UUID.
 * ovsdb-server will assign a different UUID when 'txn' is committed,
...
sbrec_ha_chassis_group_insert(struct ovsdb_idl_txn *txn)
{
    return sbrec_ha_chassis_group_cast(ovsdb_idl_txn_insert(txn, &sbrec_table_ha_chassis_group, NULL));
}

bool
sbrec_ha_chassis_group_is_updated(const struct sbrec_ha_chassis_group *row, enum sbrec_ha_chassis_group_column_id column)
{
    return ovsdb_idl_track_is_updated(&row->header_, &sbrec_ha_chassis_group_columns[column]);
}
 ...
 *
 *   - If 'row' is a row created by the current transaction (returned by
 *     sbrec_ha_chassis_group_insert()).
 *
 *   - If "external_ids" has already been modified (with
 *     sbrec_ha_chassis_group_set_external_ids()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * sbrec_ha_chassis_group_set_external_ids() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_ha_chassis_group_verify_external_ids(const struct sbrec_ha_chassis_group *row)
{
    ovsdb_idl_txn_verify(&row->header_, &sbrec_ha_chassis_group_col_external_ids);
}

/* Causes the original contents of column "ha_chassis" in 'row' to be
 * verified as a prerequisite to completing the transaction.  That is, if
 * "ha_chassis" in 'row' changed (or if 'row' was deleted) between the
 * time that the IDL originally read its contents and the time that the
 * transaction aborts and ovsdb_idl_txn_commit() returns TXN_TRY_AGAIN.
 *
 * The intention is that, to ensure that no transaction commits based on dirty
 * reads, an application should call this function any time "ha_chassis" is
 * read as part of a read-modify-write operation.
 *
 * In some cases this function reduces to a no-op, because the current value
 * of "ha_chassis" is already known:
 *
 *   - If 'row' is a row created by the current transaction (returned by
 *     sbrec_ha_chassis_group_insert()).
 *
 *   - If "ha_chassis" has already been modified (with
 *     sbrec_ha_chassis_group_set_ha_chassis()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * sbrec_ha_chassis_group_set_ha_chassis() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_ha_chassis_group_verify_ha_chassis(const struct sbrec_ha_chassis_group *row)
{
    ovsdb_idl_txn_verify(&row->header_, &sbrec_ha_chassis_group_col_ha_chassis);
}
 ...
 *
 *   - If "name" has already been modified (with
 *     sbrec_ha_chassis_group_set_name()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * sbrec_ha_chassis_group_set_name() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_ha_chassis_group_verify_name(const struct sbrec_ha_chassis_group *row)
{
    ovsdb_idl_txn_verify(&row->header_, &sbrec_ha_chassis_group_col_name);
}

/* Causes the original contents of column "ref_chassis" in 'row' to be
 * verified as a prerequisite to completing the transaction.  That is, if
 * "ref_chassis" in 'row' changed (or if 'row' was deleted) between the
 * time that the IDL originally read its contents and the time that the
 * transaction aborts and ovsdb_idl_txn_commit() returns TXN_TRY_AGAIN.
 *
 * The intention is that, to ensure that no transaction commits based on dirty
 * reads, an application should call this function any time "ref_chassis" is
 * read as part of a read-modify-write operation.
 *
 * In some cases this function reduces to a no-op, because the current value
 * of "ref_chassis" is already known:
 *
 *   - If 'row' is a row created by the current transaction (returned by
 *     sbrec_ha_chassis_group_insert()).
 *
 *   - If "ref_chassis" has already been modified (with
 *     sbrec_ha_chassis_group_set_ref_chassis()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * sbrec_ha_chassis_group_set_ref_chassis() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_ha_chassis_group_verify_ref_chassis(const struct sbrec_ha_chassis_group *row)
{
    ovsdb_idl_txn_verify(&row->header_, &sbrec_ha_chassis_group_col_ref_chassis);
}

/* Returns the "external_ids" column's value from the "HA_Chassis_Group" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "external_ids" member in sbrec_ha_chassis_group. */
const struct ovsdb_datum *
sbrec_ha_chassis_group_get_external_ids(const struct sbrec_ha_chassis_group *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED,
	enum ovsdb_atomic_type value_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_STRING);
    ovs_assert(value_type == OVSDB_TYPE_STRING);
    return ovsdb_idl_read(&row->header_, &sbrec_ha_chassis_group_col_external_ids);
}

/* Returns the "ha_chassis" column's value from the "HA_Chassis_Group" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 *
 * 'key_type' must be OVSDB_TYPE_UUID.
 * (This helps to avoid silent bugs if someone changes ha_chassis's
 * type without updating the caller.)
 *
 * The caller must not modify or free the returned value.
 *
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "ha_chassis" member in sbrec_ha_chassis_group. */
const struct ovsdb_datum *
sbrec_ha_chassis_group_get_ha_chassis(const struct sbrec_ha_chassis_group *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_UUID);
    return ovsdb_idl_read(&row->header_, &sbrec_ha_chassis_group_col_ha_chassis);
}

/* Returns the "name" column's value from the "HA_Chassis_Group" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "name" member in sbrec_ha_chassis_group. */
const struct ovsdb_datum *
sbrec_ha_chassis_group_get_name(const struct sbrec_ha_chassis_group *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_STRING);
    return ovsdb_idl_read(&row->header_, &sbrec_ha_chassis_group_col_name);
}

/* Returns the "ref_chassis" column's value from the "HA_Chassis_Group" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 *
 * 'key_type' must be OVSDB_TYPE_UUID.
 * (This helps to avoid silent bugs if someone changes ref_chassis's
 * type without updating the caller.)
 *
 * The caller must not modify or free the returned value.
 *
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "ref_chassis" member in sbrec_ha_chassis_group. */
const struct ovsdb_datum *
sbrec_ha_chassis_group_get_ref_chassis(const struct sbrec_ha_chassis_group *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_UUID);
    return ovsdb_idl_read(&row->header_, &sbrec_ha_chassis_group_col_ref_chassis);
}

/* Sets the "external_ids" column's value from the "HA_Chassis_Group" table in 'row'
 * to 'external_ids'.
 *
 * The caller retains ownership of 'external_ids' and everything in it. */
void
sbrec_ha_chassis_group_set_external_ids(const struct sbrec_ha_chassis_group *row, const struct smap *external_ids)
{
    struct ovsdb_datum datum;
...
/* Sets the "ha_chassis" column from the "HA_Chassis_Group" table in 'row' to
 * the 'ha_chassis' set with 'n_ha_chassis' entries.
 *
 * The caller retains ownership of the arguments. */
void
sbrec_ha_chassis_group_set_ha_chassis(const struct sbrec_ha_chassis_group *row, struct sbrec_ha_chassis **ha_chassis, size_t n_ha_chassis)
{
    struct ovsdb_datum datum;

    datum.n = n_ha_chassis;
    datum.keys = n_ha_chassis ? xmalloc(n_ha_chassis * sizeof *datum.keys) : NULL;
    datum.values = NULL;
    for (size_t i = 0; i < n_ha_chassis; i++) {
        datum.keys[i].uuid = ha_chassis[i]->header_.uuid;
    }
    ovsdb_idl_txn_write(&row->header_, &sbrec_ha_chassis_group_col_ha_chassis, &datum);
}

/* Sets the "name" column from the "HA_Chassis_Group" table in 'row' to
 * 'name'.
 *
 * The caller retains ownership of the arguments. */
void
sbrec_ha_chassis_group_set_name(const struct sbrec_ha_chassis_group *row, const char *name)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    key.string = CONST_CAST(char *, name);
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &sbrec_ha_chassis_group_col_name, &datum);
}

/* Sets the "ref_chassis" column from the "HA_Chassis_Group" table in 'row' to
 * the 'ref_chassis' set with 'n_ref_chassis' entries.
 *
 * The caller retains ownership of the arguments. */
void
sbrec_ha_chassis_group_set_ref_chassis(const struct sbrec_ha_chassis_group *row, struct sbrec_chassis **ref_chassis, size_t n_ref_chassis)
{
    struct ovsdb_datum datum;

    datum.n = n_ref_chassis;
    datum.keys = n_ref_chassis ? xmalloc(n_ref_chassis * sizeof *datum.keys) : NULL;
    datum.values = NULL;
    for (size_t i = 0; i < n_ref_chassis; i++) {
        datum.keys[i].uuid = ref_chassis[i]->header_.uuid;
    }
    ovsdb_idl_txn_write(&row->header_, &sbrec_ha_chassis_group_col_ref_chassis, &datum);
}

/* Sets an element of the "external_ids" map column from the "HA_Chassis_Group" table in 'row'
 * to 'new_value' given the key value 'new_key'.
 *
 */
void
sbrec_ha_chassis_group_update_external_ids_setkey(const struct sbrec_ha_chassis_group *row, const char *new_key, const char *new_value)
{
    struct ovsdb_datum *datum;
...
/* Deletes an element of the "external_ids" map column from the "HA_Chassis_Group" table in 'row'
 * given the key value 'delete_key'.
 *
 */
void
sbrec_ha_chassis_group_update_external_ids_delkey(const struct sbrec_ha_chassis_group *row, const char *delete_key)
{
    struct ovsdb_datum *datum;
...
/* Adds the value 'new_value' to the "ha_chassis" set column from the "HA_Chassis_Group" table
 * in 'row'.
 *
 */
void
sbrec_ha_chassis_group_update_ha_chassis_addvalue(const struct sbrec_ha_chassis_group *row, const struct sbrec_ha_chassis *new_value)
{
    struct ovsdb_datum *datum;
...
/* Deletes the value 'delete_value' from the "ha_chassis" set column from the
 * "HA_Chassis_Group" table in 'row'.
 *
 */
void
sbrec_ha_chassis_group_update_ha_chassis_delvalue(const struct sbrec_ha_chassis_group *row, const struct sbrec_ha_chassis *delete_value)
{
    struct ovsdb_datum *datum;
...
/* Adds the value 'new_value' to the "ref_chassis" set column from the "HA_Chassis_Group" table
 * in 'row'.
 *
 */
void
sbrec_ha_chassis_group_update_ref_chassis_addvalue(const struct sbrec_ha_chassis_group *row, const struct sbrec_chassis *new_value)
{
    struct ovsdb_datum *datum;
...
/* Deletes the value 'delete_value' from the "ref_chassis" set column from the
 * "HA_Chassis_Group" table in 'row'.
 *
 */
void
sbrec_ha_chassis_group_update_ref_chassis_delvalue(const struct sbrec_ha_chassis_group *row, const struct sbrec_chassis *delete_value)
{
    struct ovsdb_datum *datum;
 ...
 * The caller retains ownership of 'external_ids' and everything in it. */
void
sbrec_ha_chassis_group_add_clause_external_ids(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const struct smap *external_ids)
{
    struct ovsdb_datum datum;
    ...
    ovsdb_datum_destroy(&datum, &sbrec_ha_chassis_group_col_external_ids.type);
}
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ha_chassis_group_add_clause_ha_chassis(struct ovsdb_idl_condition *cond, enum ovsdb_function function, struct uuid **ha_chassis, size_t n_ha_chassis)
{
    struct ovsdb_datum datum;
    datum.n = n_ha_chassis;
    datum.keys = n_ha_chassis ? xmalloc(n_ha_chassis * sizeof *datum.keys) : NULL;
    datum.values = NULL;
    for (size_t i = 0; i < n_ha_chassis; i++) {
        datum.keys[i].uuid = *ha_chassis[i];
    }
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_UUID, OVSDB_TYPE_VOID);
    ovsdb_idl_condition_add_clause(cond,
                          function,
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ha_chassis_group_add_clause_name(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const char *name)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ha_chassis_group_add_clause_ref_chassis(struct ovsdb_idl_condition *cond, enum ovsdb_function function, struct uuid **ref_chassis, size_t n_ref_chassis)
{
    struct ovsdb_datum datum;
    datum.n = n_ref_chassis;
    datum.keys = n_ref_chassis ? xmalloc(n_ref_chassis * sizeof *datum.keys) : NULL;
    datum.values = NULL;
    for (size_t i = 0; i < n_ref_chassis; i++) {
        datum.keys[i].uuid = *ref_chassis[i];
    }
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_UUID, OVSDB_TYPE_VOID);
    ovsdb_idl_condition_add_clause(cond,
                          function,
...
/* Destroy 'row' of kind "HA_Chassis_Group". The row must have been
 * created with ovsdb_idl_index_init_row.
 */
void
sbrec_ha_chassis_group_index_destroy_row(const struct sbrec_ha_chassis_group *row)
{
    ovsdb_idl_index_destroy_row(&row->header_);
}
        

/* Creates a new row of kind "HA_Chassis_Group". */
struct sbrec_ha_chassis_group *
sbrec_ha_chassis_group_index_init_row(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &sbrec_table_ha_chassis_group);
    return ALIGNED_CAST(struct sbrec_ha_chassis_group *, ovsdb_idl_index_init_row(index));
}

struct sbrec_ha_chassis_group *
sbrec_ha_chassis_group_index_find(struct ovsdb_idl_index *index, const struct sbrec_ha_chassis_group *target)
{
    ovs_assert(index->table->class_ == &sbrec_table_ha_chassis_group);
    return sbrec_ha_chassis_group_cast(ovsdb_idl_index_find(index, &target->header_));
}

/* Compares 'a' to 'b' and returns a strcmp()-type result. */
int
sbrec_ha_chassis_group_index_compare(
    struct ovsdb_idl_index *index, 
...
struct ovsdb_idl_cursor
sbrec_ha_chassis_group_cursor_first(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &sbrec_table_ha_chassis_group);
    return ovsdb_idl_cursor_first(index);
}
...
struct ovsdb_idl_cursor
sbrec_ha_chassis_group_cursor_first_eq(
    struct ovsdb_idl_index *index, const struct sbrec_ha_chassis_group *target)
{
    ovs_assert(index->table->class_ == &sbrec_table_ha_chassis_group);
    return ovsdb_idl_cursor_first_eq(index, &target->header_);
}
...
struct ovsdb_idl_cursor
sbrec_ha_chassis_group_cursor_first_ge(
    struct ovsdb_idl_index *index, const struct sbrec_ha_chassis_group *target)
{
    ovs_assert(index->table->class_ == &sbrec_table_ha_chassis_group);
    return ovsdb_idl_cursor_first_ge(index, &target->header_);
...
struct sbrec_ha_chassis_group *
sbrec_ha_chassis_group_cursor_data(struct ovsdb_idl_cursor *cursor)
{
    return sbrec_ha_chassis_group_cast(ovsdb_idl_cursor_data(cursor));
}
 ...
 * The caller retains ownership of 'external_ids' and everything in it. */
void
sbrec_ha_chassis_group_index_set_external_ids(const struct sbrec_ha_chassis_group *row, const struct smap *external_ids)
{
    struct ovsdb_datum *datum = xmalloc(sizeof(struct ovsdb_datum));
    ...
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_),
                          &sbrec_ha_chassis_group_columns[SBREC_HA_CHASSIS_GROUP_COL_EXTERNAL_IDS],
                          datum,
                          &sbrec_table_classes[SBREC_TABLE_HA_CHASSIS_GROUP]);
}
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ha_chassis_group_index_set_ha_chassis(const struct sbrec_ha_chassis_group *row, struct sbrec_ha_chassis **ha_chassis, size_t n_ha_chassis)
{
    struct ovsdb_datum datum;
    size_t i;
    ...
    datum.keys = n_ha_chassis ? xmalloc(n_ha_chassis * sizeof *datum.keys) : NULL;
    datum.values = NULL;
    for (i = 0; i < n_ha_chassis; i++) {
        datum.keys[i].uuid = ha_chassis[i]->header_.uuid;
    }
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_UUID, OVSDB_TYPE_VOID);
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &sbrec_ha_chassis_group_columns[ SBREC_HA_CHASSIS_GROUP_COL_HA_CHASSIS ], &datum, &sbrec_table_classes[SBREC_TABLE_HA_CHASSIS_GROUP]);
}
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ha_chassis_group_index_set_name(const struct sbrec_ha_chassis_group *row, const char *name)
{
    struct ovsdb_datum datum;
    union ovsdb_atom *key = xmalloc(sizeof(union ovsdb_atom));
    ...
    key->string = CONST_CAST(char *, name);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &sbrec_ha_chassis_group_columns[ SBREC_HA_CHASSIS_GROUP_COL_NAME ], &datum, &sbrec_table_classes[SBREC_TABLE_HA_CHASSIS_GROUP]);
}
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ha_chassis_group_index_set_ref_chassis(const struct sbrec_ha_chassis_group *row, struct sbrec_chassis **ref_chassis, size_t n_ref_chassis)
{
    struct ovsdb_datum datum;
    size_t i;
    ...
    datum.keys = n_ref_chassis ? xmalloc(n_ref_chassis * sizeof *datum.keys) : NULL;
    datum.values = NULL;
    for (i = 0; i < n_ref_chassis; i++) {
        datum.keys[i].uuid = ref_chassis[i]->header_.uuid;
    }
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_UUID, OVSDB_TYPE_VOID);
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &sbrec_ha_chassis_group_columns[ SBREC_HA_CHASSIS_GROUP_COL_REF_CHASSIS ], &datum, &sbrec_table_classes[SBREC_TABLE_HA_CHASSIS_GROUP]);
}

struct ovsdb_idl_column sbrec_ha_chassis_group_columns[SBREC_HA_CHASSIS_GROUP_N_COLUMNS];

void
sbrec_ha_chassis_group_set_condition(struct ovsdb_idl *idl, struct ovsdb_idl_condition *condition)
{
    ovsdb_idl_set_condition(idl, &sbrec_table_ha_chassis_group, condition);
}
...
struct ovsdb_idl_column sbrec_ha_chassis_group_columns[SBREC_HA_CHASSIS_GROUP_N_COLUMNS] = {
    [SBREC_HA_CHASSIS_GROUP_COL_EXTERNAL_IDS] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = sbrec_ha_chassis_group_parse_external_ids,
         .unparse = sbrec_ha_chassis_group_unparse_external_ids,
    },
    ...
    [SBREC_HA_CHASSIS_GROUP_COL_HA_CHASSIS] = {
         ...
         .type = {
            .key = {
               .type = OVSDB_TYPE_UUID,
               .uuid = { .refTableName = "HA_Chassis", .refType = OVSDB_REF_STRONG },
            },
            .value = OVSDB_BASE_VOID_INIT,
            .n_min = 0,
            .n_max = UINT_MAX,
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = sbrec_ha_chassis_group_parse_ha_chassis,
         .unparse = sbrec_ha_chassis_group_unparse_ha_chassis,
    },
    ...
    [SBREC_HA_CHASSIS_GROUP_COL_NAME] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = sbrec_ha_chassis_group_parse_name,
         .unparse = sbrec_ha_chassis_group_unparse_name,
    },
    ...
    [SBREC_HA_CHASSIS_GROUP_COL_REF_CHASSIS] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = sbrec_ha_chassis_group_parse_ref_chassis,
         .unparse = sbrec_ha_chassis_group_unparse_ref_chassis,
    },
...
sbrec_igmp_group_table_get(const struct ovsdb_idl *idl)
{
    return (const struct sbrec_igmp_group_table *) idl;
}

const struct sbrec_igmp_group *
sbrec_igmp_group_table_first(const struct sbrec_igmp_group_table *table)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return sbrec_igmp_group_first(idl);
}


const struct sbrec_igmp_group *
sbrec_igmp_group_table_track_get_first(const struct sbrec_igmp_group_table *table)
{
    const struct ovsdb_idl *idl =  (const struct ovsdb_idl *) table;
    return sbrec_igmp_group_track_get_first(idl);
}


static void
sbrec_igmp_group_parse_address(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct sbrec_igmp_group *row = sbrec_igmp_group_cast(row_);

    if (datum->n >= 1) {
        row->address = datum->keys[0].string;
    } else {
        row->address = "";
    }
}
...
static void
sbrec_igmp_group_parse_chassis(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct sbrec_igmp_group *row = sbrec_igmp_group_cast(row_);
...
static void
sbrec_igmp_group_parse_datapath(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct sbrec_igmp_group *row = sbrec_igmp_group_cast(row_);
...
static void
sbrec_igmp_group_parse_ports(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct sbrec_igmp_group *row = sbrec_igmp_group_cast(row_);
    row->ports = NULL;
    row->n_ports = 0;
...
static void
sbrec_igmp_group_unparse_address(struct ovsdb_idl_row *row OVS_UNUSED)
{
    /* Nothing to do. */
}

static void
sbrec_igmp_group_unparse_chassis(struct ovsdb_idl_row *row OVS_UNUSED)
{
    /* Nothing to do. */
}

static void
sbrec_igmp_group_unparse_datapath(struct ovsdb_idl_row *row OVS_UNUSED)
{
    /* Nothing to do. */
}

static void
sbrec_igmp_group_unparse_ports(struct ovsdb_idl_row *row_)
{
    struct sbrec_igmp_group *row = sbrec_igmp_group_cast(row_);
    free(row->ports);
}
...
static void
sbrec_igmp_group_init__(struct ovsdb_idl_row *row)
{
    sbrec_igmp_group_init(sbrec_igmp_group_cast(row));
}

/* Clears the contents of 'row' in table "IGMP_Group". */
void
sbrec_igmp_group_init(struct sbrec_igmp_group *row)
{
    memset(row, 0, sizeof *row); 
    row->address = "";
}

/* Searches table "IGMP_Group" in 'idl' for a row with UUID 'uuid'.  Returns
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct sbrec_igmp_group *
sbrec_igmp_group_get_for_uuid(const struct ovsdb_idl *idl, const struct uuid *uuid)
{
    return sbrec_igmp_group_cast(ovsdb_idl_get_row_for_uuid(idl, &sbrec_table_igmp_group, uuid));
}

/* Searches table "IGMP_Group" for a row with UUID 'uuid'.  Returns
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct sbrec_igmp_group *
sbrec_igmp_group_table_get_for_uuid(const struct sbrec_igmp_group_table *table, const struct uuid *uuid)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return sbrec_igmp_group_get_for_uuid(idl, uuid);
}

/* Returns a row in table "IGMP_Group" in 'idl', or a null pointer if that
 * table is empty.
 *
 * Database tables are internally maintained as hash tables, so adding or
 * removing rows while traversing the same table can cause some rows to be
 * visited twice or not at apply. */
const struct sbrec_igmp_group *
sbrec_igmp_group_first(const struct ovsdb_idl *idl)
{
    return sbrec_igmp_group_cast(ovsdb_idl_first_row(idl, &sbrec_table_igmp_group));
}

/* Returns a row following 'row' within its table, or a null pointer if 'row'
 * is the last row in its table. */
const struct sbrec_igmp_group *
sbrec_igmp_group_next(const struct sbrec_igmp_group *row)
{
    return sbrec_igmp_group_cast(ovsdb_idl_next_row(&row->header_));
}

unsigned int sbrec_igmp_group_get_seqno(const struct ovsdb_idl *idl)
{
    return ovsdb_idl_table_get_seqno(idl, &sbrec_table_igmp_group);
}

unsigned int sbrec_igmp_group_row_get_seqno(const struct sbrec_igmp_group *row, enum ovsdb_idl_change change)
{
    return ovsdb_idl_row_get_seqno(&row->header_, change);
}

const struct sbrec_igmp_group *
sbrec_igmp_group_track_get_first(const struct ovsdb_idl *idl)
{
    return sbrec_igmp_group_cast(ovsdb_idl_track_get_first(idl, &sbrec_table_igmp_group));
}

const struct sbrec_igmp_group
*sbrec_igmp_group_track_get_next(const struct sbrec_igmp_group *row)
{
    return sbrec_igmp_group_cast(ovsdb_idl_track_get_next(&row->header_));
}


/* Deletes 'row' from table "IGMP_Group".  'row' may be freed, so it must not be
 * accessed afterward.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_igmp_group_delete(const struct sbrec_igmp_group *row)
{
    ovsdb_idl_txn_delete(&row->header_);
}

/* Inserts and returns a new row in the table "IGMP_Group" in the database
 * with open transaction 'txn'.
 *
 * The new row is assigned a randomly generated provisional UUID.
 * ovsdb-server will assign a different UUID when 'txn' is committed,
...
sbrec_igmp_group_insert(struct ovsdb_idl_txn *txn)
{
    return sbrec_igmp_group_cast(ovsdb_idl_txn_insert(txn, &sbrec_table_igmp_group, NULL));
}

bool
sbrec_igmp_group_is_updated(const struct sbrec_igmp_group *row, enum sbrec_igmp_group_column_id column)
{
    return ovsdb_idl_track_is_updated(&row->header_, &sbrec_igmp_group_columns[column]);
}

/* Causes the original contents of column "address" in 'row' to be
 * verified as a prerequisite to completing the transaction.  That is, if
 * "address" in 'row' changed (or if 'row' was deleted) between the
 * time that the IDL originally read its contents and the time that the
 * transaction aborts and ovsdb_idl_txn_commit() returns TXN_TRY_AGAIN.
 *
 * The intention is that, to ensure that no transaction commits based on dirty
 * reads, an application should call this function any time "address" is
 * read as part of a read-modify-write operation.
 *
 * In some cases this function reduces to a no-op, because the current value
 * of "address" is already known:
 *
 *   - If 'row' is a row created by the current transaction (returned by
 *     sbrec_igmp_group_insert()).
 *
 *   - If "address" has already been modified (with
 *     sbrec_igmp_group_set_address()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * sbrec_igmp_group_set_address() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_igmp_group_verify_address(const struct sbrec_igmp_group *row)
{
    ovsdb_idl_txn_verify(&row->header_, &sbrec_igmp_group_col_address);
}
 ...
 *
 *   - If "chassis" has already been modified (with
 *     sbrec_igmp_group_set_chassis()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * sbrec_igmp_group_set_chassis() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_igmp_group_verify_chassis(const struct sbrec_igmp_group *row)
{
    ovsdb_idl_txn_verify(&row->header_, &sbrec_igmp_group_col_chassis);
}
 ...
 *     sbrec_igmp_group_set_datapath()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * sbrec_igmp_group_set_datapath() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_igmp_group_verify_datapath(const struct sbrec_igmp_group *row)
{
    ovsdb_idl_txn_verify(&row->header_, &sbrec_igmp_group_col_datapath);
}
 ...
 *     sbrec_igmp_group_set_ports()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * sbrec_igmp_group_set_ports() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_igmp_group_verify_ports(const struct sbrec_igmp_group *row)
{
    ovsdb_idl_txn_verify(&row->header_, &sbrec_igmp_group_col_ports);
}

/* Returns the "address" column's value from the "IGMP_Group" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 *
 * 'key_type' must be OVSDB_TYPE_STRING.
 * (This helps to avoid silent bugs if someone changes address's
 * type without updating the caller.)
 *
 * The caller must not modify or free the returned value.
 *
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "address" member in sbrec_igmp_group. */
const struct ovsdb_datum *
sbrec_igmp_group_get_address(const struct sbrec_igmp_group *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_STRING);
    return ovsdb_idl_read(&row->header_, &sbrec_igmp_group_col_address);
}

/* Returns the "chassis" column's value from the "IGMP_Group" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "chassis" member in sbrec_igmp_group. */
const struct ovsdb_datum *
sbrec_igmp_group_get_chassis(const struct sbrec_igmp_group *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_UUID);
    return ovsdb_idl_read(&row->header_, &sbrec_igmp_group_col_chassis);
}

/* Returns the "datapath" column's value from the "IGMP_Group" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "datapath" member in sbrec_igmp_group. */
const struct ovsdb_datum *
sbrec_igmp_group_get_datapath(const struct sbrec_igmp_group *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_UUID);
    return ovsdb_idl_read(&row->header_, &sbrec_igmp_group_col_datapath);
}

/* Returns the "ports" column's value from the "IGMP_Group" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "ports" member in sbrec_igmp_group. */
const struct ovsdb_datum *
sbrec_igmp_group_get_ports(const struct sbrec_igmp_group *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_UUID);
    return ovsdb_idl_read(&row->header_, &sbrec_igmp_group_col_ports);
}

/* Sets the "address" column from the "IGMP_Group" table in 'row' to
 * 'address'.
 *
 * The caller retains ownership of the arguments. */
void
sbrec_igmp_group_set_address(const struct sbrec_igmp_group *row, const char *address)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    datum.n = 1;
    datum.keys = &key;
    key.string = CONST_CAST(char *, address);
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &sbrec_igmp_group_col_address, &datum);
}

/* Sets the "chassis" column from the "IGMP_Group" table in 'row' to
 * the 'chassis' set.
 *
 * If "chassis" is null, the column will be the empty set,
 * otherwise it will contain the specified value.
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_igmp_group_set_chassis(const struct sbrec_igmp_group *row, const struct sbrec_chassis *chassis)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &sbrec_igmp_group_col_chassis, &datum);
}

/* Sets the "datapath" column from the "IGMP_Group" table in 'row' to
 * the 'datapath' set.
 *
 * If "datapath" is null, the column will be the empty set,
 * otherwise it will contain the specified value.
 *
 * The caller retains ownership of the arguments. */
void
sbrec_igmp_group_set_datapath(const struct sbrec_igmp_group *row, const struct sbrec_datapath_binding *datapath)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &sbrec_igmp_group_col_datapath, &datum);
}

/* Sets the "ports" column from the "IGMP_Group" table in 'row' to
 * the 'ports' set with 'n_ports' entries.
 *
 * The caller retains ownership of the arguments. */
void
sbrec_igmp_group_set_ports(const struct sbrec_igmp_group *row, struct sbrec_port_binding **ports, size_t n_ports)
{
    struct ovsdb_datum datum;
    ...
    ovsdb_idl_txn_write(&row->header_, &sbrec_igmp_group_col_ports, &datum);
}

/* Adds the value 'new_value' to the "chassis" set column from the "IGMP_Group" table
 * in 'row'.
 *
 */
void
sbrec_igmp_group_update_chassis_addvalue(const struct sbrec_igmp_group *row, const struct sbrec_chassis *new_value)
{
    struct ovsdb_datum *datum;
...
/* Deletes the value 'delete_value' from the "chassis" set column from the
 * "IGMP_Group" table in 'row'.
 *
 */
void
sbrec_igmp_group_update_chassis_delvalue(const struct sbrec_igmp_group *row, const struct sbrec_chassis *delete_value)
{
    struct ovsdb_datum *datum;
...
/* Adds the value 'new_value' to the "datapath" set column from the "IGMP_Group" table
 * in 'row'.
 *
 */
void
sbrec_igmp_group_update_datapath_addvalue(const struct sbrec_igmp_group *row, const struct sbrec_datapath_binding *new_value)
{
    struct ovsdb_datum *datum;
...
/* Deletes the value 'delete_value' from the "datapath" set column from the
 * "IGMP_Group" table in 'row'.
 *
 */
void
sbrec_igmp_group_update_datapath_delvalue(const struct sbrec_igmp_group *row, const struct sbrec_datapath_binding *delete_value)
{
    struct ovsdb_datum *datum;
...
/* Adds the value 'new_value' to the "ports" set column from the "IGMP_Group" table
 * in 'row'.
 *
 */
void
sbrec_igmp_group_update_ports_addvalue(const struct sbrec_igmp_group *row, const struct sbrec_port_binding *new_value)
{
    struct ovsdb_datum *datum;
 ...
 */
void
sbrec_igmp_group_update_ports_delvalue(const struct sbrec_igmp_group *row, const struct sbrec_port_binding *delete_value)
{
    struct ovsdb_datum *datum;
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_igmp_group_add_clause_address(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const char *address)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_igmp_group_add_clause_chassis(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const struct uuid *chassis)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_igmp_group_add_clause_datapath(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const struct uuid *datapath)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_igmp_group_add_clause_ports(struct ovsdb_idl_condition *cond, enum ovsdb_function function, struct uuid **ports, size_t n_ports)
{
    struct ovsdb_datum datum;
    datum.n = n_ports;
    datum.keys = n_ports ? xmalloc(n_ports * sizeof *datum.keys) : NULL;
...
/* Destroy 'row' of kind "IGMP_Group". The row must have been
 * created with ovsdb_idl_index_init_row.
 */
void
sbrec_igmp_group_index_destroy_row(const struct sbrec_igmp_group *row)
{
    ovsdb_idl_index_destroy_row(&row->header_);
}
        

/* Creates a new row of kind "IGMP_Group". */
struct sbrec_igmp_group *
sbrec_igmp_group_index_init_row(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &sbrec_table_igmp_group);
    return ALIGNED_CAST(struct sbrec_igmp_group *, ovsdb_idl_index_init_row(index));
}

struct sbrec_igmp_group *
sbrec_igmp_group_index_find(struct ovsdb_idl_index *index, const struct sbrec_igmp_group *target)
{
    ovs_assert(index->table->class_ == &sbrec_table_igmp_group);
    return sbrec_igmp_group_cast(ovsdb_idl_index_find(index, &target->header_));
}

/* Compares 'a' to 'b' and returns a strcmp()-type result. */
int
sbrec_igmp_group_index_compare(
    struct ovsdb_idl_index *index, 
    const struct sbrec_igmp_group *a, 
    const struct sbrec_igmp_group *b)
{
    return ovsdb_idl_index_compare(index, &a->header_, &b->header_);
}

struct ovsdb_idl_cursor
sbrec_igmp_group_cursor_first(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &sbrec_table_igmp_group);
    return ovsdb_idl_cursor_first(index);
}
...
struct ovsdb_idl_cursor
sbrec_igmp_group_cursor_first_eq(
    struct ovsdb_idl_index *index, const struct sbrec_igmp_group *target)
{
    ovs_assert(index->table->class_ == &sbrec_table_igmp_group);
    return ovsdb_idl_cursor_first_eq(index, &target->header_);
}
...
struct ovsdb_idl_cursor
sbrec_igmp_group_cursor_first_ge(
    struct ovsdb_idl_index *index, const struct sbrec_igmp_group *target)
{
    ovs_assert(index->table->class_ == &sbrec_table_igmp_group);
    return ovsdb_idl_cursor_first_ge(index, &target->header_);
...
struct sbrec_igmp_group *
sbrec_igmp_group_cursor_data(struct ovsdb_idl_cursor *cursor)
{
    return sbrec_igmp_group_cast(ovsdb_idl_cursor_data(cursor));
}
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_igmp_group_index_set_address(const struct sbrec_igmp_group *row, const char *address)
{
    struct ovsdb_datum datum;
    union ovsdb_atom *key = xmalloc(sizeof(union ovsdb_atom));
    ...
    key->string = CONST_CAST(char *, address);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &sbrec_igmp_group_columns[ SBREC_IGMP_GROUP_COL_ADDRESS ], &datum, &sbrec_table_classes[SBREC_TABLE_IGMP_GROUP]);
}
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_igmp_group_index_set_chassis(const struct sbrec_igmp_group *row, const struct sbrec_chassis *chassis)
{
    struct ovsdb_datum datum;
    union ovsdb_atom *key = xmalloc(sizeof (union ovsdb_atom));
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &sbrec_igmp_group_columns[ SBREC_IGMP_GROUP_COL_CHASSIS ], &datum, &sbrec_table_classes[SBREC_TABLE_IGMP_GROUP]);
}
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_igmp_group_index_set_datapath(const struct sbrec_igmp_group *row, const struct sbrec_datapath_binding *datapath)
{
    struct ovsdb_datum datum;
    union ovsdb_atom *key = xmalloc(sizeof (union ovsdb_atom));
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &sbrec_igmp_group_columns[ SBREC_IGMP_GROUP_COL_DATAPATH ], &datum, &sbrec_table_classes[SBREC_TABLE_IGMP_GROUP]);
}
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_igmp_group_index_set_ports(const struct sbrec_igmp_group *row, struct sbrec_port_binding **ports, size_t n_ports)
{
    struct ovsdb_datum datum;
    size_t i;
    ...
    ovsdb_datum_sort_unique(&datum, OVSDB_TYPE_UUID, OVSDB_TYPE_VOID);
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &sbrec_igmp_group_columns[ SBREC_IGMP_GROUP_COL_PORTS ], &datum, &sbrec_table_classes[SBREC_TABLE_IGMP_GROUP]);
}

struct ovsdb_idl_column sbrec_igmp_group_columns[SBREC_IGMP_GROUP_N_COLUMNS];

void
sbrec_igmp_group_set_condition(struct ovsdb_idl *idl, struct ovsdb_idl_condition *condition)
{
    ovsdb_idl_set_condition(idl, &sbrec_table_igmp_group, condition);
}
...
struct ovsdb_idl_column sbrec_igmp_group_columns[SBREC_IGMP_GROUP_N_COLUMNS] = {
    [SBREC_IGMP_GROUP_COL_ADDRESS] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = sbrec_igmp_group_parse_address,
         .unparse = sbrec_igmp_group_unparse_address,
    },
    ...
    [SBREC_IGMP_GROUP_COL_CHASSIS] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = sbrec_igmp_group_parse_chassis,
         .unparse = sbrec_igmp_group_unparse_chassis,
    },
    ...
    [SBREC_IGMP_GROUP_COL_DATAPATH] = {
         ...
         .type = {
            .key = {
               .type = OVSDB_TYPE_UUID,
               .uuid = { .refTableName = "Datapath_Binding", .refType = OVSDB_REF_WEAK },
            },
            .value = OVSDB_BASE_VOID_INIT,
            .n_min = 0,
            .n_max = 1,
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = sbrec_igmp_group_parse_datapath,
         .unparse = sbrec_igmp_group_unparse_datapath,
    },
    ...
    [SBREC_IGMP_GROUP_COL_PORTS] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = sbrec_igmp_group_parse_ports,
         .unparse = sbrec_igmp_group_unparse_ports,
    },
...
const struct sbrec_ip_multicast_table *
sbrec_ip_multicast_table_get(const struct ovsdb_idl *idl)
{
    return (const struct sbrec_ip_multicast_table *) idl;
}

const struct sbrec_ip_multicast *
sbrec_ip_multicast_table_first(const struct sbrec_ip_multicast_table *table)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return sbrec_ip_multicast_first(idl);
}


const struct sbrec_ip_multicast *
sbrec_ip_multicast_table_track_get_first(const struct sbrec_ip_multicast_table *table)
{
    const struct ovsdb_idl *idl =  (const struct ovsdb_idl *) table;
    return sbrec_ip_multicast_track_get_first(idl);
}


static void
sbrec_ip_multicast_parse_datapath(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct sbrec_ip_multicast *row = sbrec_ip_multicast_cast(row_);
...
static void
sbrec_ip_multicast_parse_enabled(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct sbrec_ip_multicast *row = sbrec_ip_multicast_cast(row_);
    size_t n = MIN(1, datum->n);
    row->enabled = NULL;
...
static void
sbrec_ip_multicast_parse_eth_src(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct sbrec_ip_multicast *row = sbrec_ip_multicast_cast(row_);

    if (datum->n >= 1) {
        row->eth_src = datum->keys[0].string;
    } else {
        row->eth_src = "";
    }
}
...
static void
sbrec_ip_multicast_parse_idle_timeout(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct sbrec_ip_multicast *row = sbrec_ip_multicast_cast(row_);
    size_t n = MIN(1, datum->n);
    row->idle_timeout = NULL;
    row->n_idle_timeout = 0;
    for (size_t i = 0; i < n; i++) {
        if (!row->n_idle_timeout) {
            row->idle_timeout = xmalloc(n * sizeof *row->idle_timeout);
        }
        row->idle_timeout[row->n_idle_timeout] = datum->keys[i].integer;
        row->n_idle_timeout++;
    }
}

static void
sbrec_ip_multicast_parse_ip4_src(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct sbrec_ip_multicast *row = sbrec_ip_multicast_cast(row_);

    if (datum->n >= 1) {
        row->ip4_src = datum->keys[0].string;
    } else {
        row->ip4_src = "";
    }
}
...
static void
sbrec_ip_multicast_parse_querier(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct sbrec_ip_multicast *row = sbrec_ip_multicast_cast(row_);
    size_t n = MIN(1, datum->n);
    row->querier = NULL;
    ...
    for (size_t i = 0; i < n; i++) {
        if (!row->n_querier) {
            row->querier = xmalloc(n * sizeof *row->querier);
        }
        row->querier[row->n_querier] = datum->keys[i].boolean;
        row->n_querier++;
    }
}

static void
sbrec_ip_multicast_parse_query_interval(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct sbrec_ip_multicast *row = sbrec_ip_multicast_cast(row_);
    size_t n = MIN(1, datum->n);
    row->query_interval = NULL;
    row->n_query_interval = 0;
    for (size_t i = 0; i < n; i++) {
        if (!row->n_query_interval) {
            row->query_interval = xmalloc(n * sizeof *row->query_interval);
        }
        row->query_interval[row->n_query_interval] = datum->keys[i].integer;
        row->n_query_interval++;
    }
}

static void
sbrec_ip_multicast_parse_query_max_resp(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct sbrec_ip_multicast *row = sbrec_ip_multicast_cast(row_);
    size_t n = MIN(1, datum->n);
    row->query_max_resp = NULL;
    row->n_query_max_resp = 0;
    for (size_t i = 0; i < n; i++) {
        if (!row->n_query_max_resp) {
            row->query_max_resp = xmalloc(n * sizeof *row->query_max_resp);
        }
        row->query_max_resp[row->n_query_max_resp] = datum->keys[i].integer;
        row->n_query_max_resp++;
    }
}

static void
sbrec_ip_multicast_parse_seq_no(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct sbrec_ip_multicast *row = sbrec_ip_multicast_cast(row_);

    if (datum->n >= 1) {
        row->seq_no = datum->keys[0].integer;
    } else {
        row->seq_no = 0;
    }
}
...
static void
sbrec_ip_multicast_parse_table_size(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct sbrec_ip_multicast *row = sbrec_ip_multicast_cast(row_);
    size_t n = MIN(1, datum->n);
    row->table_size = NULL;
    ...
    for (size_t i = 0; i < n; i++) {
        if (!row->n_table_size) {
            row->table_size = xmalloc(n * sizeof *row->table_size);
        }
        row->table_size[row->n_table_size] = datum->keys[i].integer;
        row->n_table_size++;
    }
}

static void
sbrec_ip_multicast_unparse_datapath(struct ovsdb_idl_row *row OVS_UNUSED)
{
    /* Nothing to do. */
}

static void
sbrec_ip_multicast_unparse_enabled(struct ovsdb_idl_row *row_)
{
    struct sbrec_ip_multicast *row = sbrec_ip_multicast_cast(row_);
    free(row->enabled);
}
...
static void
sbrec_ip_multicast_unparse_eth_src(struct ovsdb_idl_row *row OVS_UNUSED)
{
    /* Nothing to do. */
}

static void
sbrec_ip_multicast_unparse_idle_timeout(struct ovsdb_idl_row *row_)
{
    struct sbrec_ip_multicast *row = sbrec_ip_multicast_cast(row_);
    free(row->idle_timeout);
}

static void
sbrec_ip_multicast_unparse_ip4_src(struct ovsdb_idl_row *row OVS_UNUSED)
{
    /* Nothing to do. */
}

static void
sbrec_ip_multicast_unparse_querier(struct ovsdb_idl_row *row_)
{
    struct sbrec_ip_multicast *row = sbrec_ip_multicast_cast(row_);
    free(row->querier);
}
...
static void
sbrec_ip_multicast_unparse_query_interval(struct ovsdb_idl_row *row_)
{
    struct sbrec_ip_multicast *row = sbrec_ip_multicast_cast(row_);
    free(row->query_interval);
}

static void
sbrec_ip_multicast_unparse_query_max_resp(struct ovsdb_idl_row *row_)
{
    struct sbrec_ip_multicast *row = sbrec_ip_multicast_cast(row_);
    free(row->query_max_resp);
}

static void
sbrec_ip_multicast_unparse_seq_no(struct ovsdb_idl_row *row OVS_UNUSED)
{
    /* Nothing to do. */
}

static void
sbrec_ip_multicast_unparse_table_size(struct ovsdb_idl_row *row_)
{
    struct sbrec_ip_multicast *row = sbrec_ip_multicast_cast(row_);
    free(row->table_size);
}
...
static void
sbrec_ip_multicast_init__(struct ovsdb_idl_row *row)
{
    sbrec_ip_multicast_init(sbrec_ip_multicast_cast(row));
}

/* Clears the contents of 'row' in table "IP_Multicast". */
void
sbrec_ip_multicast_init(struct sbrec_ip_multicast *row)
{
    memset(row, 0, sizeof *row); 
    row->eth_src = "";
    row->ip4_src = "";
...
/* Searches table "IP_Multicast" in 'idl' for a row with UUID 'uuid'.  Returns
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct sbrec_ip_multicast *
sbrec_ip_multicast_get_for_uuid(const struct ovsdb_idl *idl, const struct uuid *uuid)
{
    return sbrec_ip_multicast_cast(ovsdb_idl_get_row_for_uuid(idl, &sbrec_table_ip_multicast, uuid));
}

/* Searches table "IP_Multicast" for a row with UUID 'uuid'.  Returns
 * a pointer to the row if there is one, otherwise a null pointer.  */
const struct sbrec_ip_multicast *
sbrec_ip_multicast_table_get_for_uuid(const struct sbrec_ip_multicast_table *table, const struct uuid *uuid)
{
    const struct ovsdb_idl *idl = (const struct ovsdb_idl *) table;
    return sbrec_ip_multicast_get_for_uuid(idl, uuid);
}

/* Returns a row in table "IP_Multicast" in 'idl', or a null pointer if that
 * table is empty.
 *
 * Database tables are internally maintained as hash tables, so adding or
 * removing rows while traversing the same table can cause some rows to be
 * visited twice or not at apply. */
const struct sbrec_ip_multicast *
sbrec_ip_multicast_first(const struct ovsdb_idl *idl)
{
    return sbrec_ip_multicast_cast(ovsdb_idl_first_row(idl, &sbrec_table_ip_multicast));
}

/* Returns a row following 'row' within its table, or a null pointer if 'row'
 * is the last row in its table. */
const struct sbrec_ip_multicast *
sbrec_ip_multicast_next(const struct sbrec_ip_multicast *row)
{
    return sbrec_ip_multicast_cast(ovsdb_idl_next_row(&row->header_));
}

unsigned int sbrec_ip_multicast_get_seqno(const struct ovsdb_idl *idl)
{
    return ovsdb_idl_table_get_seqno(idl, &sbrec_table_ip_multicast);
}

unsigned int sbrec_ip_multicast_row_get_seqno(const struct sbrec_ip_multicast *row, enum ovsdb_idl_change change)
{
    return ovsdb_idl_row_get_seqno(&row->header_, change);
}

const struct sbrec_ip_multicast *
sbrec_ip_multicast_track_get_first(const struct ovsdb_idl *idl)
{
    return sbrec_ip_multicast_cast(ovsdb_idl_track_get_first(idl, &sbrec_table_ip_multicast));
}

const struct sbrec_ip_multicast
*sbrec_ip_multicast_track_get_next(const struct sbrec_ip_multicast *row)
{
    return sbrec_ip_multicast_cast(ovsdb_idl_track_get_next(&row->header_));
}


/* Deletes 'row' from table "IP_Multicast".  'row' may be freed, so it must not be
 * accessed afterward.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_ip_multicast_delete(const struct sbrec_ip_multicast *row)
{
    ovsdb_idl_txn_delete(&row->header_);
}

/* Inserts and returns a new row in the table "IP_Multicast" in the database
 * with open transaction 'txn'.
 *
 * The new row is assigned a randomly generated provisional UUID.
 * ovsdb-server will assign a different UUID when 'txn' is committed,
 * but the IDL will replace any uses of the provisional UUID in the
 * data to be to be committed by the UUID assigned by ovsdb-server. */
struct sbrec_ip_multicast *
sbrec_ip_multicast_insert(struct ovsdb_idl_txn *txn)
{
    return sbrec_ip_multicast_cast(ovsdb_idl_txn_insert(txn, &sbrec_table_ip_multicast, NULL));
}

bool
sbrec_ip_multicast_is_updated(const struct sbrec_ip_multicast *row, enum sbrec_ip_multicast_column_id column)
{
    return ovsdb_idl_track_is_updated(&row->header_, &sbrec_ip_multicast_columns[column]);
}
 ...
 *
 *   - If 'row' is a row created by the current transaction (returned by
 *     sbrec_ip_multicast_insert()).
 *
 *   - If "datapath" has already been modified (with
 *     sbrec_ip_multicast_set_datapath()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * sbrec_ip_multicast_set_datapath() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_ip_multicast_verify_datapath(const struct sbrec_ip_multicast *row)
{
    ovsdb_idl_txn_verify(&row->header_, &sbrec_ip_multicast_col_datapath);
}
 ...
 *     sbrec_ip_multicast_set_enabled()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * sbrec_ip_multicast_set_enabled() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_ip_multicast_verify_enabled(const struct sbrec_ip_multicast *row)
{
    ovsdb_idl_txn_verify(&row->header_, &sbrec_ip_multicast_col_enabled);
}

/* Causes the original contents of column "eth_src" in 'row' to be
 * verified as a prerequisite to completing the transaction.  That is, if
 * "eth_src" in 'row' changed (or if 'row' was deleted) between the
 * time that the IDL originally read its contents and the time that the
 * transaction aborts and ovsdb_idl_txn_commit() returns TXN_TRY_AGAIN.
 *
 * The intention is that, to ensure that no transaction commits based on dirty
 * reads, an application should call this function any time "eth_src" is
 * read as part of a read-modify-write operation.
 *
 * In some cases this function reduces to a no-op, because the current value
 * of "eth_src" is already known:
 *
 *   - If 'row' is a row created by the current transaction (returned by
 *     sbrec_ip_multicast_insert()).
 *
 *   - If "eth_src" has already been modified (with
 *     sbrec_ip_multicast_set_eth_src()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * sbrec_ip_multicast_set_eth_src() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_ip_multicast_verify_eth_src(const struct sbrec_ip_multicast *row)
{
    ovsdb_idl_txn_verify(&row->header_, &sbrec_ip_multicast_col_eth_src);
}

/* Causes the original contents of column "idle_timeout" in 'row' to be
 * verified as a prerequisite to completing the transaction.  That is, if
 * "idle_timeout" in 'row' changed (or if 'row' was deleted) between the
 * time that the IDL originally read its contents and the time that the
 * transaction aborts and ovsdb_idl_txn_commit() returns TXN_TRY_AGAIN.
 *
 * The intention is that, to ensure that no transaction commits based on dirty
 * reads, an application should call this function any time "idle_timeout" is
 * read as part of a read-modify-write operation.
 *
 * In some cases this function reduces to a no-op, because the current value
 * of "idle_timeout" is already known:
 *
 *   - If 'row' is a row created by the current transaction (returned by
 *     sbrec_ip_multicast_insert()).
 *
 *   - If "idle_timeout" has already been modified (with
 *     sbrec_ip_multicast_set_idle_timeout()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * sbrec_ip_multicast_set_idle_timeout() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_ip_multicast_verify_idle_timeout(const struct sbrec_ip_multicast *row)
{
    ovsdb_idl_txn_verify(&row->header_, &sbrec_ip_multicast_col_idle_timeout);
}

/* Causes the original contents of column "ip4_src" in 'row' to be
 * verified as a prerequisite to completing the transaction.  That is, if
 * "ip4_src" in 'row' changed (or if 'row' was deleted) between the
 * time that the IDL originally read its contents and the time that the
 * transaction aborts and ovsdb_idl_txn_commit() returns TXN_TRY_AGAIN.
 *
 * The intention is that, to ensure that no transaction commits based on dirty
 * reads, an application should call this function any time "ip4_src" is
 * read as part of a read-modify-write operation.
 *
 * In some cases this function reduces to a no-op, because the current value
 * of "ip4_src" is already known:
 *
 *   - If 'row' is a row created by the current transaction (returned by
 *     sbrec_ip_multicast_insert()).
 *
 *   - If "ip4_src" has already been modified (with
 *     sbrec_ip_multicast_set_ip4_src()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * sbrec_ip_multicast_set_ip4_src() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_ip_multicast_verify_ip4_src(const struct sbrec_ip_multicast *row)
{
    ovsdb_idl_txn_verify(&row->header_, &sbrec_ip_multicast_col_ip4_src);
}

/* Causes the original contents of column "querier" in 'row' to be
 * verified as a prerequisite to completing the transaction.  That is, if
 * "querier" in 'row' changed (or if 'row' was deleted) between the
 * time that the IDL originally read its contents and the time that the
 * transaction aborts and ovsdb_idl_txn_commit() returns TXN_TRY_AGAIN.
 *
 * The intention is that, to ensure that no transaction commits based on dirty
 * reads, an application should call this function any time "querier" is
 * read as part of a read-modify-write operation.
 *
 * In some cases this function reduces to a no-op, because the current value
 * of "querier" is already known:
 *
 *   - If 'row' is a row created by the current transaction (returned by
 *     sbrec_ip_multicast_insert()).
 *
 *   - If "querier" has already been modified (with
 *     sbrec_ip_multicast_set_querier()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * sbrec_ip_multicast_set_querier() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_ip_multicast_verify_querier(const struct sbrec_ip_multicast *row)
{
    ovsdb_idl_txn_verify(&row->header_, &sbrec_ip_multicast_col_querier);
}

/* Causes the original contents of column "query_interval" in 'row' to be
 * verified as a prerequisite to completing the transaction.  That is, if
 * "query_interval" in 'row' changed (or if 'row' was deleted) between the
 * time that the IDL originally read its contents and the time that the
 * transaction aborts and ovsdb_idl_txn_commit() returns TXN_TRY_AGAIN.
 *
 * The intention is that, to ensure that no transaction commits based on dirty
 * reads, an application should call this function any time "query_interval" is
 * read as part of a read-modify-write operation.
 *
 * In some cases this function reduces to a no-op, because the current value
 * of "query_interval" is already known:
 *
 *   - If 'row' is a row created by the current transaction (returned by
 *     sbrec_ip_multicast_insert()).
 *
 *   - If "query_interval" has already been modified (with
 *     sbrec_ip_multicast_set_query_interval()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * sbrec_ip_multicast_set_query_interval() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_ip_multicast_verify_query_interval(const struct sbrec_ip_multicast *row)
{
    ovsdb_idl_txn_verify(&row->header_, &sbrec_ip_multicast_col_query_interval);
}

/* Causes the original contents of column "query_max_resp" in 'row' to be
 * verified as a prerequisite to completing the transaction.  That is, if
 * "query_max_resp" in 'row' changed (or if 'row' was deleted) between the
 * time that the IDL originally read its contents and the time that the
 * transaction aborts and ovsdb_idl_txn_commit() returns TXN_TRY_AGAIN.
 *
 * The intention is that, to ensure that no transaction commits based on dirty
 * reads, an application should call this function any time "query_max_resp" is
 * read as part of a read-modify-write operation.
 *
 * In some cases this function reduces to a no-op, because the current value
 * of "query_max_resp" is already known:
 *
 *   - If 'row' is a row created by the current transaction (returned by
 *     sbrec_ip_multicast_insert()).
 *
 *   - If "query_max_resp" has already been modified (with
 *     sbrec_ip_multicast_set_query_max_resp()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * sbrec_ip_multicast_set_query_max_resp() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_ip_multicast_verify_query_max_resp(const struct sbrec_ip_multicast *row)
{
    ovsdb_idl_txn_verify(&row->header_, &sbrec_ip_multicast_col_query_max_resp);
}

/* Causes the original contents of column "seq_no" in 'row' to be
 * verified as a prerequisite to completing the transaction.  That is, if
 * "seq_no" in 'row' changed (or if 'row' was deleted) between the
 * time that the IDL originally read its contents and the time that the
 * transaction aborts and ovsdb_idl_txn_commit() returns TXN_TRY_AGAIN.
 *
 * The intention is that, to ensure that no transaction commits based on dirty
 * reads, an application should call this function any time "seq_no" is
 * read as part of a read-modify-write operation.
 *
 * In some cases this function reduces to a no-op, because the current value
 * of "seq_no" is already known:
 *
 *   - If 'row' is a row created by the current transaction (returned by
 *     sbrec_ip_multicast_insert()).
 *
 *   - If "seq_no" has already been modified (with
 *     sbrec_ip_multicast_set_seq_no()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * sbrec_ip_multicast_set_seq_no() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_ip_multicast_verify_seq_no(const struct sbrec_ip_multicast *row)
{
    ovsdb_idl_txn_verify(&row->header_, &sbrec_ip_multicast_col_seq_no);
}

/* Causes the original contents of column "table_size" in 'row' to be
 * verified as a prerequisite to completing the transaction.  That is, if
 * "table_size" in 'row' changed (or if 'row' was deleted) between the
 * time that the IDL originally read its contents and the time that the
 * transaction aborts and ovsdb_idl_txn_commit() returns TXN_TRY_AGAIN.
 *
 * The intention is that, to ensure that no transaction commits based on dirty
 * reads, an application should call this function any time "table_size" is
 * read as part of a read-modify-write operation.
 *
 * In some cases this function reduces to a no-op, because the current value
 * of "table_size" is already known:
 *
 *   - If 'row' is a row created by the current transaction (returned by
 *     sbrec_ip_multicast_insert()).
 *
 *   - If "table_size" has already been modified (with
 *     sbrec_ip_multicast_set_table_size()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * sbrec_ip_multicast_set_table_size() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_ip_multicast_verify_table_size(const struct sbrec_ip_multicast *row)
{
    ovsdb_idl_txn_verify(&row->header_, &sbrec_ip_multicast_col_table_size);
}

/* Returns the "datapath" column's value from the "IP_Multicast" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "datapath" member in sbrec_ip_multicast. */
const struct ovsdb_datum *
sbrec_ip_multicast_get_datapath(const struct sbrec_ip_multicast *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_UUID);
    return ovsdb_idl_read(&row->header_, &sbrec_ip_multicast_col_datapath);
}

/* Returns the "enabled" column's value from the "IP_Multicast" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "enabled" member in sbrec_ip_multicast. */
const struct ovsdb_datum *
sbrec_ip_multicast_get_enabled(const struct sbrec_ip_multicast *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_BOOLEAN);
    return ovsdb_idl_read(&row->header_, &sbrec_ip_multicast_col_enabled);
}

/* Returns the "eth_src" column's value from the "IP_Multicast" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 *
 * 'key_type' must be OVSDB_TYPE_STRING.
 * (This helps to avoid silent bugs if someone changes eth_src's
 * type without updating the caller.)
 *
 * The caller must not modify or free the returned value.
 *
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "eth_src" member in sbrec_ip_multicast. */
const struct ovsdb_datum *
sbrec_ip_multicast_get_eth_src(const struct sbrec_ip_multicast *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_STRING);
    return ovsdb_idl_read(&row->header_, &sbrec_ip_multicast_col_eth_src);
}

/* Returns the "idle_timeout" column's value from the "IP_Multicast" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 *
 * 'key_type' must be OVSDB_TYPE_INTEGER.
 * (This helps to avoid silent bugs if someone changes idle_timeout's
 * type without updating the caller.)
 *
 * The caller must not modify or free the returned value.
 *
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "idle_timeout" member in sbrec_ip_multicast. */
const struct ovsdb_datum *
sbrec_ip_multicast_get_idle_timeout(const struct sbrec_ip_multicast *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_INTEGER);
    return ovsdb_idl_read(&row->header_, &sbrec_ip_multicast_col_idle_timeout);
}

/* Returns the "ip4_src" column's value from the "IP_Multicast" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 *
 * 'key_type' must be OVSDB_TYPE_STRING.
 * (This helps to avoid silent bugs if someone changes ip4_src's
 * type without updating the caller.)
 *
 * The caller must not modify or free the returned value.
 *
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "ip4_src" member in sbrec_ip_multicast. */
const struct ovsdb_datum *
sbrec_ip_multicast_get_ip4_src(const struct sbrec_ip_multicast *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_STRING);
    return ovsdb_idl_read(&row->header_, &sbrec_ip_multicast_col_ip4_src);
}

/* Returns the "querier" column's value from the "IP_Multicast" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 *
 * 'key_type' must be OVSDB_TYPE_BOOLEAN.
 * (This helps to avoid silent bugs if someone changes querier's
 * type without updating the caller.)
 *
 * The caller must not modify or free the returned value.
 *
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "querier" member in sbrec_ip_multicast. */
const struct ovsdb_datum *
sbrec_ip_multicast_get_querier(const struct sbrec_ip_multicast *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_BOOLEAN);
    return ovsdb_idl_read(&row->header_, &sbrec_ip_multicast_col_querier);
}

/* Returns the "query_interval" column's value from the "IP_Multicast" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 *
 * 'key_type' must be OVSDB_TYPE_INTEGER.
 * (This helps to avoid silent bugs if someone changes query_interval's
 * type without updating the caller.)
 *
 * The caller must not modify or free the returned value.
 *
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "query_interval" member in sbrec_ip_multicast. */
const struct ovsdb_datum *
sbrec_ip_multicast_get_query_interval(const struct sbrec_ip_multicast *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_INTEGER);
    return ovsdb_idl_read(&row->header_, &sbrec_ip_multicast_col_query_interval);
}

/* Returns the "query_max_resp" column's value from the "IP_Multicast" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 *
 * 'key_type' must be OVSDB_TYPE_INTEGER.
 * (This helps to avoid silent bugs if someone changes query_max_resp's
 * type without updating the caller.)
 *
 * The caller must not modify or free the returned value.
 *
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "query_max_resp" member in sbrec_ip_multicast. */
const struct ovsdb_datum *
sbrec_ip_multicast_get_query_max_resp(const struct sbrec_ip_multicast *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_INTEGER);
    return ovsdb_idl_read(&row->header_, &sbrec_ip_multicast_col_query_max_resp);
}

/* Returns the "seq_no" column's value from the "IP_Multicast" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 *
 * 'key_type' must be OVSDB_TYPE_INTEGER.
 * (This helps to avoid silent bugs if someone changes seq_no's
 * type without updating the caller.)
 *
 * The caller must not modify or free the returned value.
 *
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "seq_no" member in sbrec_ip_multicast. */
const struct ovsdb_datum *
sbrec_ip_multicast_get_seq_no(const struct sbrec_ip_multicast *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_INTEGER);
    return ovsdb_idl_read(&row->header_, &sbrec_ip_multicast_col_seq_no);
}

/* Returns the "table_size" column's value from the "IP_Multicast" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 *
 * 'key_type' must be OVSDB_TYPE_INTEGER.
 * (This helps to avoid silent bugs if someone changes table_size's
 * type without updating the caller.)
 *
 * The caller must not modify or free the returned value.
 *
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "table_size" member in sbrec_ip_multicast. */
const struct ovsdb_datum *
sbrec_ip_multicast_get_table_size(const struct sbrec_ip_multicast *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_INTEGER);
    return ovsdb_idl_read(&row->header_, &sbrec_ip_multicast_col_table_size);
}

/* Sets the "datapath" column from the "IP_Multicast" table in 'row' to
 * 'datapath'.
 *
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_set_datapath(const struct sbrec_ip_multicast *row, const struct sbrec_datapath_binding *datapath)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    key.uuid = datapath->header_.uuid;
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &sbrec_ip_multicast_col_datapath, &datum);
}

/* Sets the "enabled" column from the "IP_Multicast" table in 'row' to
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_set_enabled(const struct sbrec_ip_multicast *row, const bool *enabled, size_t n_enabled)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &sbrec_ip_multicast_col_enabled, &datum);
}

/* Sets the "eth_src" column from the "IP_Multicast" table in 'row' to
 * 'eth_src'.
 *
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_set_eth_src(const struct sbrec_ip_multicast *row, const char *eth_src)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    datum.n = 1;
    datum.keys = &key;
    key.string = CONST_CAST(char *, eth_src);
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &sbrec_ip_multicast_col_eth_src, &datum);
}

/* Sets the "idle_timeout" column from the "IP_Multicast" table in 'row' to
 * the 'idle_timeout' set with 'n_idle_timeout' entries.
 *
 * 'n_idle_timeout' may be 0 or 1; if it is 0, then 'idle_timeout'
 * may be NULL.
 *
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_set_idle_timeout(const struct sbrec_ip_multicast *row, const int64_t *idle_timeout, size_t n_idle_timeout)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    if (n_idle_timeout) {
        datum.n = 1;
        datum.keys = &key;
        key.integer = *idle_timeout;
    } else {
        datum.n = 0;
        datum.keys = NULL;
    }
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &sbrec_ip_multicast_col_idle_timeout, &datum);
}

/* Sets the "ip4_src" column from the "IP_Multicast" table in 'row' to
 * 'ip4_src'.
 *
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_set_ip4_src(const struct sbrec_ip_multicast *row, const char *ip4_src)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    datum.n = 1;
    datum.keys = &key;
    key.string = CONST_CAST(char *, ip4_src);
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &sbrec_ip_multicast_col_ip4_src, &datum);
}

/* Sets the "querier" column from the "IP_Multicast" table in 'row' to
 * the 'querier' set with 'n_querier' entries.
 *
 * 'n_querier' may be 0 or 1; if it is 0, then 'querier'
 * may be NULL.
 *
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_set_querier(const struct sbrec_ip_multicast *row, const bool *querier, size_t n_querier)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &sbrec_ip_multicast_col_querier, &datum);
}

/* Sets the "query_interval" column from the "IP_Multicast" table in 'row' to
 * the 'query_interval' set with 'n_query_interval' entries.
 *
 * 'n_query_interval' may be 0 or 1; if it is 0, then 'query_interval'
 * may be NULL.
 *
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_set_query_interval(const struct sbrec_ip_multicast *row, const int64_t *query_interval, size_t n_query_interval)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    if (n_query_interval) {
        datum.n = 1;
        datum.keys = &key;
        key.integer = *query_interval;
    } else {
        datum.n = 0;
        datum.keys = NULL;
    }
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &sbrec_ip_multicast_col_query_interval, &datum);
}

/* Sets the "query_max_resp" column from the "IP_Multicast" table in 'row' to
 * the 'query_max_resp' set with 'n_query_max_resp' entries.
 *
 * 'n_query_max_resp' may be 0 or 1; if it is 0, then 'query_max_resp'
 * may be NULL.
 *
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_set_query_max_resp(const struct sbrec_ip_multicast *row, const int64_t *query_max_resp, size_t n_query_max_resp)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    if (n_query_max_resp) {
        datum.n = 1;
        datum.keys = &key;
        key.integer = *query_max_resp;
    } else {
        datum.n = 0;
        datum.keys = NULL;
    }
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &sbrec_ip_multicast_col_query_max_resp, &datum);
}

/* Sets the "seq_no" column from the "IP_Multicast" table in 'row' to
 * 'seq_no'.
 *
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_set_seq_no(const struct sbrec_ip_multicast *row, int64_t seq_no)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    key.integer = seq_no;
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &sbrec_ip_multicast_col_seq_no, &datum);
}

/* Sets the "table_size" column from the "IP_Multicast" table in 'row' to
 * the 'table_size' set with 'n_table_size' entries.
 *
 * 'n_table_size' may be 0 or 1; if it is 0, then 'table_size'
 * may be NULL.
 *
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_set_table_size(const struct sbrec_ip_multicast *row, const int64_t *table_size, size_t n_table_size)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    if (n_table_size) {
        datum.n = 1;
        datum.keys = &key;
        key.integer = *table_size;
    } else {
        datum.n = 0;
        datum.keys = NULL;
    }
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &sbrec_ip_multicast_col_table_size, &datum);
}

/* Adds the value 'new_value' to the "enabled" set column from the "IP_Multicast" table
 * in 'row'.
 *
 */
void
sbrec_ip_multicast_update_enabled_addvalue(const struct sbrec_ip_multicast *row, bool new_value)
{
    struct ovsdb_datum *datum;
 ...
 * "IP_Multicast" table in 'row'.
 *
 */
void
sbrec_ip_multicast_update_enabled_delvalue(const struct sbrec_ip_multicast *row, bool delete_value)
{
    struct ovsdb_datum *datum;
...
/* Adds the value 'new_value' to the "idle_timeout" set column from the "IP_Multicast" table
 * in 'row'.
 *
 */
void
sbrec_ip_multicast_update_idle_timeout_addvalue(const struct sbrec_ip_multicast *row, int64_t new_value)
{
    struct ovsdb_datum *datum;
...
/* Deletes the value 'delete_value' from the "idle_timeout" set column from the
 * "IP_Multicast" table in 'row'.
 *
 */
void
sbrec_ip_multicast_update_idle_timeout_delvalue(const struct sbrec_ip_multicast *row, int64_t delete_value)
{
    struct ovsdb_datum *datum;
...
/* Adds the value 'new_value' to the "querier" set column from the "IP_Multicast" table
 * in 'row'.
 *
 */
void
sbrec_ip_multicast_update_querier_addvalue(const struct sbrec_ip_multicast *row, bool new_value)
{
    struct ovsdb_datum *datum;
...
/* Deletes the value 'delete_value' from the "querier" set column from the
 * "IP_Multicast" table in 'row'.
 *
 */
void
sbrec_ip_multicast_update_querier_delvalue(const struct sbrec_ip_multicast *row, bool delete_value)
{
    struct ovsdb_datum *datum;
...
/* Adds the value 'new_value' to the "query_interval" set column from the "IP_Multicast" table
 * in 'row'.
 *
 */
void
sbrec_ip_multicast_update_query_interval_addvalue(const struct sbrec_ip_multicast *row, int64_t new_value)
{
    struct ovsdb_datum *datum;
...
/* Deletes the value 'delete_value' from the "query_interval" set column from the
 * "IP_Multicast" table in 'row'.
 *
 */
void
sbrec_ip_multicast_update_query_interval_delvalue(const struct sbrec_ip_multicast *row, int64_t delete_value)
{
    struct ovsdb_datum *datum;
...
/* Adds the value 'new_value' to the "query_max_resp" set column from the "IP_Multicast" table
 * in 'row'.
 *
 */
void
sbrec_ip_multicast_update_query_max_resp_addvalue(const struct sbrec_ip_multicast *row, int64_t new_value)
{
    struct ovsdb_datum *datum;
...
/* Deletes the value 'delete_value' from the "query_max_resp" set column from the
 * "IP_Multicast" table in 'row'.
 *
 */
void
sbrec_ip_multicast_update_query_max_resp_delvalue(const struct sbrec_ip_multicast *row, int64_t delete_value)
{
    struct ovsdb_datum *datum;
...
/* Adds the value 'new_value' to the "table_size" set column from the "IP_Multicast" table
 * in 'row'.
 *
 */
void
sbrec_ip_multicast_update_table_size_addvalue(const struct sbrec_ip_multicast *row, int64_t new_value)
{
    struct ovsdb_datum *datum;
...
/* Deletes the value 'delete_value' from the "table_size" set column from the
 * "IP_Multicast" table in 'row'.
 *
 */
void
sbrec_ip_multicast_update_table_size_delvalue(const struct sbrec_ip_multicast *row, int64_t delete_value)
{
    struct ovsdb_datum *datum;
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_add_clause_datapath(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const struct uuid *datapath)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_add_clause_enabled(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const bool *enabled, size_t n_enabled)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_add_clause_eth_src(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const char *eth_src)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_add_clause_idle_timeout(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const int64_t *idle_timeout, size_t n_idle_timeout)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_add_clause_ip4_src(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const char *ip4_src)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_add_clause_querier(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const bool *querier, size_t n_querier)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_add_clause_query_interval(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const int64_t *query_interval, size_t n_query_interval)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_add_clause_query_max_resp(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const int64_t *query_max_resp, size_t n_query_max_resp)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_add_clause_seq_no(struct ovsdb_idl_condition *cond, enum ovsdb_function function, int64_t seq_no)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_add_clause_table_size(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const int64_t *table_size, size_t n_table_size)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
...
/* Destroy 'row' of kind "IP_Multicast". The row must have been
 * created with ovsdb_idl_index_init_row.
 */
void
sbrec_ip_multicast_index_destroy_row(const struct sbrec_ip_multicast *row)
{
    ovsdb_idl_index_destroy_row(&row->header_);
}
        

/* Creates a new row of kind "IP_Multicast". */
struct sbrec_ip_multicast *
sbrec_ip_multicast_index_init_row(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &sbrec_table_ip_multicast);
    return ALIGNED_CAST(struct sbrec_ip_multicast *, ovsdb_idl_index_init_row(index));
}

struct sbrec_ip_multicast *
sbrec_ip_multicast_index_find(struct ovsdb_idl_index *index, const struct sbrec_ip_multicast *target)
{
    ovs_assert(index->table->class_ == &sbrec_table_ip_multicast);
    return sbrec_ip_multicast_cast(ovsdb_idl_index_find(index, &target->header_));
}

/* Compares 'a' to 'b' and returns a strcmp()-type result. */
int
sbrec_ip_multicast_index_compare(
    struct ovsdb_idl_index *index, 
    const struct sbrec_ip_multicast *a, 
    const struct sbrec_ip_multicast *b)
{
    return ovsdb_idl_index_compare(index, &a->header_, &b->header_);
}

struct ovsdb_idl_cursor
sbrec_ip_multicast_cursor_first(struct ovsdb_idl_index *index)
{
    ovs_assert(index->table->class_ == &sbrec_table_ip_multicast);
    return ovsdb_idl_cursor_first(index);
}
...
struct ovsdb_idl_cursor
sbrec_ip_multicast_cursor_first_eq(
    struct ovsdb_idl_index *index, const struct sbrec_ip_multicast *target)
{
    ovs_assert(index->table->class_ == &sbrec_table_ip_multicast);
    return ovsdb_idl_cursor_first_eq(index, &target->header_);
}
...
struct ovsdb_idl_cursor
sbrec_ip_multicast_cursor_first_ge(
    struct ovsdb_idl_index *index, const struct sbrec_ip_multicast *target)
{
    ovs_assert(index->table->class_ == &sbrec_table_ip_multicast);
    return ovsdb_idl_cursor_first_ge(index, &target->header_);
...
struct sbrec_ip_multicast *
sbrec_ip_multicast_cursor_data(struct ovsdb_idl_cursor *cursor)
{
    return sbrec_ip_multicast_cast(ovsdb_idl_cursor_data(cursor));
}
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_index_set_datapath(const struct sbrec_ip_multicast *row, const struct sbrec_datapath_binding *datapath)
{
    struct ovsdb_datum datum;
    union ovsdb_atom *key = xmalloc(sizeof(union ovsdb_atom));
    ...
    key->uuid = datapath->header_.uuid;
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &sbrec_ip_multicast_columns[ SBREC_IP_MULTICAST_COL_DATAPATH ], &datum, &sbrec_table_classes[SBREC_TABLE_IP_MULTICAST]);
}
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_index_set_enabled(const struct sbrec_ip_multicast *row, const bool *enabled, size_t n_enabled)
{
    struct ovsdb_datum datum;
    union ovsdb_atom *key = xmalloc(sizeof(union ovsdb_atom));
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &sbrec_ip_multicast_columns[ SBREC_IP_MULTICAST_COL_ENABLED ], &datum, &sbrec_table_classes[SBREC_TABLE_IP_MULTICAST]);
}
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_index_set_eth_src(const struct sbrec_ip_multicast *row, const char *eth_src)
{
    struct ovsdb_datum datum;
    union ovsdb_atom *key = xmalloc(sizeof(union ovsdb_atom));
    ...
    key->string = CONST_CAST(char *, eth_src);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &sbrec_ip_multicast_columns[ SBREC_IP_MULTICAST_COL_ETH_SRC ], &datum, &sbrec_table_classes[SBREC_TABLE_IP_MULTICAST]);
}
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_index_set_idle_timeout(const struct sbrec_ip_multicast *row, const int64_t *idle_timeout, size_t n_idle_timeout)
{
    struct ovsdb_datum datum;
    union ovsdb_atom *key = xmalloc(sizeof(union ovsdb_atom));
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &sbrec_ip_multicast_columns[ SBREC_IP_MULTICAST_COL_IDLE_TIMEOUT ], &datum, &sbrec_table_classes[SBREC_TABLE_IP_MULTICAST]);
}
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_index_set_ip4_src(const struct sbrec_ip_multicast *row, const char *ip4_src)
{
    struct ovsdb_datum datum;
    union ovsdb_atom *key = xmalloc(sizeof(union ovsdb_atom));
    ...
    key->string = CONST_CAST(char *, ip4_src);
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &sbrec_ip_multicast_columns[ SBREC_IP_MULTICAST_COL_IP4_SRC ], &datum, &sbrec_table_classes[SBREC_TABLE_IP_MULTICAST]);
}
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_index_set_querier(const struct sbrec_ip_multicast *row, const bool *querier, size_t n_querier)
{
    struct ovsdb_datum datum;
    union ovsdb_atom *key = xmalloc(sizeof(union ovsdb_atom));
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &sbrec_ip_multicast_columns[ SBREC_IP_MULTICAST_COL_QUERIER ], &datum, &sbrec_table_classes[SBREC_TABLE_IP_MULTICAST]);
}
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_index_set_query_interval(const struct sbrec_ip_multicast *row, const int64_t *query_interval, size_t n_query_interval)
{
    struct ovsdb_datum datum;
    union ovsdb_atom *key = xmalloc(sizeof(union ovsdb_atom));
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &sbrec_ip_multicast_columns[ SBREC_IP_MULTICAST_COL_QUERY_INTERVAL ], &datum, &sbrec_table_classes[SBREC_TABLE_IP_MULTICAST]);
}
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_index_set_query_max_resp(const struct sbrec_ip_multicast *row, const int64_t *query_max_resp, size_t n_query_max_resp)
{
    struct ovsdb_datum datum;
    union ovsdb_atom *key = xmalloc(sizeof(union ovsdb_atom));
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &sbrec_ip_multicast_columns[ SBREC_IP_MULTICAST_COL_QUERY_MAX_RESP ], &datum, &sbrec_table_classes[SBREC_TABLE_IP_MULTICAST]);
}
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_index_set_seq_no(const struct sbrec_ip_multicast *row, int64_t seq_no)
{
    struct ovsdb_datum datum;
    union ovsdb_atom *key = xmalloc(sizeof(union ovsdb_atom));
    ...
    key->integer = seq_no;
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &sbrec_ip_multicast_columns[ SBREC_IP_MULTICAST_COL_SEQ_NO ], &datum, &sbrec_table_classes[SBREC_TABLE_IP_MULTICAST]);
}
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_ip_multicast_index_set_table_size(const struct sbrec_ip_multicast *row, const int64_t *table_size, size_t n_table_size)
{
    struct ovsdb_datum datum;
    union ovsdb_atom *key = xmalloc(sizeof(union ovsdb_atom));
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &sbrec_ip_multicast_columns[ SBREC_IP_MULTICAST_COL_TABLE_SIZE ], &datum, &sbrec_table_classes[SBREC_TABLE_IP_MULTICAST]);
}

struct ovsdb_idl_column sbrec_ip_multicast_columns[SBREC_IP_MULTICAST_N_COLUMNS];

void
sbrec_ip_multicast_set_condition(struct ovsdb_idl *idl, struct ovsdb_idl_condition *condition)
{
    ovsdb_idl_set_condition(idl, &sbrec_table_ip_multicast, condition);
}
...
struct ovsdb_idl_column sbrec_ip_multicast_columns[SBREC_IP_MULTICAST_N_COLUMNS] = {
    [SBREC_IP_MULTICAST_COL_DATAPATH] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = sbrec_ip_multicast_parse_datapath,
         .unparse = sbrec_ip_multicast_unparse_datapath,
    },
    ...
    [SBREC_IP_MULTICAST_COL_ENABLED] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = sbrec_ip_multicast_parse_enabled,
         .unparse = sbrec_ip_multicast_unparse_enabled,
    },
    ...
    [SBREC_IP_MULTICAST_COL_ETH_SRC] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = sbrec_ip_multicast_parse_eth_src,
         .unparse = sbrec_ip_multicast_unparse_eth_src,
    },
    ...
    [SBREC_IP_MULTICAST_COL_IDLE_TIMEOUT] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = sbrec_ip_multicast_parse_idle_timeout,
         .unparse = sbrec_ip_multicast_unparse_idle_timeout,
    },
    ...
    [SBREC_IP_MULTICAST_COL_IP4_SRC] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = sbrec_ip_multicast_parse_ip4_src,
         .unparse = sbrec_ip_multicast_unparse_ip4_src,
    },
    ...
    [SBREC_IP_MULTICAST_COL_QUERIER] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = sbrec_ip_multicast_parse_querier,
         .unparse = sbrec_ip_multicast_unparse_querier,
    },
    ...
    [SBREC_IP_MULTICAST_COL_QUERY_INTERVAL] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = sbrec_ip_multicast_parse_query_interval,
         .unparse = sbrec_ip_multicast_unparse_query_interval,
    },
    ...
    [SBREC_IP_MULTICAST_COL_QUERY_MAX_RESP] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = sbrec_ip_multicast_parse_query_max_resp,
         .unparse = sbrec_ip_multicast_unparse_query_max_resp,
    },
    ...
    [SBREC_IP_MULTICAST_COL_SEQ_NO] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = sbrec_ip_multicast_parse_seq_no,
         .unparse = sbrec_ip_multicast_unparse_seq_no,
    },
    ...
    [SBREC_IP_MULTICAST_COL_TABLE_SIZE] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = sbrec_ip_multicast_parse_table_size,
         .unparse = sbrec_ip_multicast_unparse_table_size,
    },
...
static void
sbrec_port_binding_parse_ha_chassis_group(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct sbrec_port_binding *row = sbrec_port_binding_cast(row_);

    if (datum->n >= 1) {
        row->ha_chassis_group = sbrec_ha_chassis_group_cast(ovsdb_idl_get_row_arc(row_, &sbrec_table_ha_chassis_group, &datum->keys[0].uuid));
    } else {
        row->ha_chassis_group = NULL;
    }
}
...
static void
sbrec_port_binding_parse_virtual_parent(struct ovsdb_idl_row *row_, const struct ovsdb_datum *datum)
{
    struct sbrec_port_binding *row = sbrec_port_binding_cast(row_);

    if (datum->n >= 1) {
        row->virtual_parent = datum->keys[0].string;
    } else {
        row->virtual_parent = NULL;
    }
}
...
static void
sbrec_port_binding_unparse_ha_chassis_group(struct ovsdb_idl_row *row OVS_UNUSED)
{
    /* Nothing to do. */
}
...
static void
sbrec_port_binding_unparse_virtual_parent(struct ovsdb_idl_row *row OVS_UNUSED)
{
    /* Nothing to do. */
}
...
/* Causes the original contents of column "ha_chassis_group" in 'row' to be
 * verified as a prerequisite to completing the transaction.  That is, if
 * "ha_chassis_group" in 'row' changed (or if 'row' was deleted) between the
 * time that the IDL originally read its contents and the time that the
 * transaction aborts and ovsdb_idl_txn_commit() returns TXN_TRY_AGAIN.
 *
 * The intention is that, to ensure that no transaction commits based on dirty
 * reads, an application should call this function any time "ha_chassis_group" is
 * read as part of a read-modify-write operation.
 *
 * In some cases this function reduces to a no-op, because the current value
 * of "ha_chassis_group" is already known:
 *
 *   - If 'row' is a row created by the current transaction (returned by
 *     sbrec_port_binding_insert()).
 *
 *   - If "ha_chassis_group" has already been modified (with
 *     sbrec_port_binding_set_ha_chassis_group()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * sbrec_port_binding_set_ha_chassis_group() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_port_binding_verify_ha_chassis_group(const struct sbrec_port_binding *row)
{
    ovsdb_idl_txn_verify(&row->header_, &sbrec_port_binding_col_ha_chassis_group);
}
...
/* Causes the original contents of column "virtual_parent" in 'row' to be
 * verified as a prerequisite to completing the transaction.  That is, if
 * "virtual_parent" in 'row' changed (or if 'row' was deleted) between the
 * time that the IDL originally read its contents and the time that the
 * transaction aborts and ovsdb_idl_txn_commit() returns TXN_TRY_AGAIN.
 *
 * The intention is that, to ensure that no transaction commits based on dirty
 * reads, an application should call this function any time "virtual_parent" is
 * read as part of a read-modify-write operation.
 *
 * In some cases this function reduces to a no-op, because the current value
 * of "virtual_parent" is already known:
 *
 *   - If 'row' is a row created by the current transaction (returned by
 *     sbrec_port_binding_insert()).
 *
 *   - If "virtual_parent" has already been modified (with
 *     sbrec_port_binding_set_virtual_parent()) within the current transaction.
 *
 * Because of the latter property, always call this function *before*
 * sbrec_port_binding_set_virtual_parent() for a given read-modify-write.
 *
 * The caller must have started a transaction with ovsdb_idl_txn_create(). */
void
sbrec_port_binding_verify_virtual_parent(const struct sbrec_port_binding *row)
{
    ovsdb_idl_txn_verify(&row->header_, &sbrec_port_binding_col_virtual_parent);
}
...
/* Returns the "ha_chassis_group" column's value from the "Port_Binding" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 *
 * 'key_type' must be OVSDB_TYPE_UUID.
 * (This helps to avoid silent bugs if someone changes ha_chassis_group's
 * type without updating the caller.)
 *
 * The caller must not modify or free the returned value.
 *
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "ha_chassis_group" member in sbrec_port_binding. */
const struct ovsdb_datum *
sbrec_port_binding_get_ha_chassis_group(const struct sbrec_port_binding *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_UUID);
    return ovsdb_idl_read(&row->header_, &sbrec_port_binding_col_ha_chassis_group);
}
...
/* Returns the "virtual_parent" column's value from the "Port_Binding" table in 'row'
 * as a struct ovsdb_datum.  This is useful occasionally: for example,
 * ovsdb_datum_find_key() is an easier and more efficient way to search
 * for a given key than implementing the same operation on the "cooked"
 * form in 'row'.
 *
 * 'key_type' must be OVSDB_TYPE_STRING.
 * (This helps to avoid silent bugs if someone changes virtual_parent's
 * type without updating the caller.)
 *
 * The caller must not modify or free the returned value.
 *
 ...
 *
 * This function is rarely useful, since it is easier to access the value
 * directly through the "virtual_parent" member in sbrec_port_binding. */
const struct ovsdb_datum *
sbrec_port_binding_get_virtual_parent(const struct sbrec_port_binding *row,
	enum ovsdb_atomic_type key_type OVS_UNUSED)
{
    ovs_assert(key_type == OVSDB_TYPE_STRING);
    return ovsdb_idl_read(&row->header_, &sbrec_port_binding_col_virtual_parent);
}
...
/* Sets the "ha_chassis_group" column from the "Port_Binding" table in 'row' to
 * the 'ha_chassis_group' set.
 *
 * If "ha_chassis_group" is null, the column will be the empty set,
 * otherwise it will contain the specified value.
 *
 * The caller retains ownership of the arguments. */
void
sbrec_port_binding_set_ha_chassis_group(const struct sbrec_port_binding *row, const struct sbrec_ha_chassis_group *ha_chassis_group)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    if (ha_chassis_group) {
        datum.n = 1;
        datum.keys = &key;
        key.uuid = ha_chassis_group->header_.uuid;
    } else {
        datum.n = 0;
        datum.keys = NULL;
    }
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &sbrec_port_binding_col_ha_chassis_group, &datum);
}
...
/* Sets the "virtual_parent" column from the "Port_Binding" table in 'row' to
 * the 'virtual_parent' set.
 *
 * If "virtual_parent" is null, the column will be the empty set,
 * otherwise it will contain the specified value.
 *
 * The caller retains ownership of the arguments. */
void
sbrec_port_binding_set_virtual_parent(const struct sbrec_port_binding *row, const char *virtual_parent)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
    ...
    if (virtual_parent) {
        datum.n = 1;
        datum.keys = &key;
        key.string = CONST_CAST(char *, virtual_parent);
    } else {
        datum.n = 0;
        datum.keys = NULL;
    }
    datum.values = NULL;
    ovsdb_idl_txn_write_clone(&row->header_, &sbrec_port_binding_col_virtual_parent, &datum);
}
...
/* Adds the value 'new_value' to the "ha_chassis_group" set column from the "Port_Binding" table
 * in 'row'.
 *
 */
void
sbrec_port_binding_update_ha_chassis_group_addvalue(const struct sbrec_port_binding *row, const struct sbrec_ha_chassis_group *new_value)
{
    struct ovsdb_datum *datum;
...
/* Deletes the value 'delete_value' from the "ha_chassis_group" set column from the
 * "Port_Binding" table in 'row'.
 *
 */
void
sbrec_port_binding_update_ha_chassis_group_delvalue(const struct sbrec_port_binding *row, const struct sbrec_ha_chassis_group *delete_value)
{
    struct ovsdb_datum *datum;
...
/* Adds the value 'new_value' to the "virtual_parent" set column from the "Port_Binding" table
 * in 'row'.
 *
 */
void
sbrec_port_binding_update_virtual_parent_addvalue(const struct sbrec_port_binding *row, const char *new_value)
{
    struct ovsdb_datum *datum;
...
/* Deletes the value 'delete_value' from the "virtual_parent" set column from the
 * "Port_Binding" table in 'row'.
 *
 */
void
sbrec_port_binding_update_virtual_parent_delvalue(const struct sbrec_port_binding *row, const char *delete_value)
{
    struct ovsdb_datum *datum;
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_port_binding_add_clause_ha_chassis_group(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const struct uuid *ha_chassis_group)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_port_binding_add_clause_virtual_parent(struct ovsdb_idl_condition *cond, enum ovsdb_function function, const char *virtual_parent)
{
    struct ovsdb_datum datum;
    union ovsdb_atom key;
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_port_binding_index_set_ha_chassis_group(const struct sbrec_port_binding *row, const struct sbrec_ha_chassis_group *ha_chassis_group)
{
    struct ovsdb_datum datum;
    union ovsdb_atom *key = xmalloc(sizeof (union ovsdb_atom));
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &sbrec_port_binding_columns[ SBREC_PORT_BINDING_COL_HA_CHASSIS_GROUP ], &datum, &sbrec_table_classes[SBREC_TABLE_PORT_BINDING]);
}
 ...
 * The caller retains ownership of the arguments. */
void
sbrec_port_binding_index_set_virtual_parent(const struct sbrec_port_binding *row, const char *virtual_parent)
{
    struct ovsdb_datum datum;
    union ovsdb_atom *key = xmalloc(sizeof (union ovsdb_atom));
    ...
    datum.values = NULL;
    ovsdb_idl_index_write(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &sbrec_port_binding_columns[ SBREC_PORT_BINDING_COL_VIRTUAL_PARENT ], &datum, &sbrec_table_classes[SBREC_TABLE_PORT_BINDING]);
}
...
struct ovsdb_idl_column sbrec_port_binding_columns[SBREC_PORT_BINDING_N_COLUMNS] = {
    ...
    [SBREC_PORT_BINDING_COL_HA_CHASSIS_GROUP] = {
         .name = "ha_chassis_group",
         .type = {
            .key = {
               .type = OVSDB_TYPE_UUID,
               .uuid = { .refTableName = "HA_Chassis_Group", .refType = OVSDB_REF_STRONG },
            },
            .value = OVSDB_BASE_VOID_INIT,
            .n_min = 0,
            .n_max = 1,
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = sbrec_port_binding_parse_ha_chassis_group,
         .unparse = sbrec_port_binding_unparse_ha_chassis_group,
    },
    ...
    [SBREC_PORT_BINDING_COL_VIRTUAL_PARENT] = {
         ...
         .is_mutable = true,
         .is_synthetic = false,
         .parse = sbrec_port_binding_parse_virtual_parent,
         .unparse = sbrec_port_binding_unparse_virtual_parent,
    },
...
struct ovsdb_idl_table_class sbrec_table_classes[SBREC_N_TABLES] = {
    ...
    {"Controller_Event", true, false,
     sbrec_controller_event_columns, ARRAY_SIZE(sbrec_controller_event_columns),
     sizeof(struct sbrec_controller_event), sbrec_controller_event_init__},
    ...
    {"HA_Chassis", false, false,
     sbrec_ha_chassis_columns, ARRAY_SIZE(sbrec_ha_chassis_columns),
     sizeof(struct sbrec_ha_chassis), sbrec_ha_chassis_init__},
    {"HA_Chassis_Group", true, false,
     sbrec_ha_chassis_group_columns, ARRAY_SIZE(sbrec_ha_chassis_group_columns),
     sizeof(struct sbrec_ha_chassis_group), sbrec_ha_chassis_group_init__},
    {"IGMP_Group", true, false,
     sbrec_igmp_group_columns, ARRAY_SIZE(sbrec_igmp_group_columns),
     sizeof(struct sbrec_igmp_group), sbrec_igmp_group_init__},
    {"IP_Multicast", true, false,
     sbrec_ip_multicast_columns, ARRAY_SIZE(sbrec_ip_multicast_columns),
     sizeof(struct sbrec_ip_multicast), sbrec_ip_multicast_init__},
