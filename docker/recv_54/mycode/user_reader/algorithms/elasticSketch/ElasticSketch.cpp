...
template<int bucket_num, int tot_memory_in_bytes>
int ElasticSketch<bucket_num, tot_memory_in_bytes>::insert(uint8_t *key, int keylen, struct Val f)
{
    uint8_t swap_key[KEY_LENGTH_4];
    uint32_t swap_val_pkts = 0;
    uint32_t swap_val_sizes = 0;
    uint32_t swap_val_delay = 0;
    // 0: 无需插入light part, match | empty,   
    // 1: heavy 中的流需要插入 light，驱逐小流 | heavy溢出  
    // 2: 只插入light part，小流
    int result = heavy_part.insert(key, swap_key, swap_val_pkts, swap_val_sizes, swap_val_delay, f);
    switch(result)
    {
        case 0: break;
        case 1:{
            if(HIGHEST_BIT_IS_1(swap_val_pkts))
                light_part.insert(swap_key, GetCounterVal(swap_val_pkts), swap_val_sizes, swap_val_delay);
            else
                light_part.swap_insert(swap_key, swap_val_sizes, swap_val_delay, swap_val_pkts);
            break;
        }
        case 2: light_part.insert(key, f.sizes, f.delay, 1);  break;
        default:
...
{
    int heavy_result = heavy_part.query(key).pkts;
    if(heavy_result == 0 || HIGHEST_BIT_IS_1(heavy_result))
    {
        // int light_result = light_part.query(key)[0];
        ...
        return (int)GetCounterVal(heavy_result) + light_result_sizes;
    }
    return heavy_result;
}

template<int bucket_num, int tot_memory_in_bytes>
struct Val ElasticSketch<bucket_num, tot_memory_in_bytes>::query(uint8_t *key, int keylen)
{
